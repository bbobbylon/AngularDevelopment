...
data.address.city;   // now it's just objects — dot into the nesting`;
  ...
  /**
   * What actually crosses the wire when a proxy or load balancer fails before
   * your own server does — the literal bytes a client receives when the real
   * failure happens one layer up. `res.ok` is `false` here, but nothing stops
   * code from calling `.json()` on it anyway.
   */
  protected readonly badGatewaySample = `HTTP/1.1 502 Bad Gateway
Content-Type: text/html

<!DOCTYPE html>
<h1>502 Bad Gateway</h1>`;

  /** Line-by-line walkthrough of {@link badGatewaySample}. */
  protected readonly badGatewayNotes: CodeNote[] = [
    {
      line: 1,
      text: 'The status line. `502` means the proxy in front of the real server got a bad answer from it — or none at all. This is still a **completed** HTTP exchange, so `fetch` resolves normally; it does not reject.',
    },
    {
      line: 2,
      text: 'The giveaway, if anything reads it: `Content-Type: text/html`, not `application/json`. Error pages like this one are generated by the proxy itself, which has no idea your app was expecting JSON.',
    },
    {
      line: 4,
      text: 'The body opens with `<!DOCTYPE html>` — an ordinary web page. `res.json()` never looks at `Content-Type` before parsing; it just runs the parser on whatever text arrived, and this text starts with `<`.',
    },
  ];

  /** The exact, googleable error `JSON.parse` throws on an HTML body. */
  protected readonly jsonParseErrorMessage = `SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON`;

  /** The naive call — no `res.ok` check, so `.json()` runs on any body at all. */
  protected readonly blindJsonSample = `const res = await fetch(url);
const data = await res.json();   // body starts with "<" — this throws`;

  /** The defensive version: branch on `res.ok`, and read the failure path as text. */
  protected readonly safeJsonSample = `const res = await fetch(url);

if (!res.ok) {
  const body = await res.text();     // read as TEXT — .json() would throw here
  console.error('HTTP ' + res.status + ':', body.slice(0, 120));
  throw new Error('Request failed: ' + res.status);
}

const data = await res.json();       // only parsed once the status is known good`;

  /** Query string built the naive way — no escaping at all. */
  protected readonly concatQuerySample = `const url = '/search?q=' + term + '&page=' + page;
// term = 'C++ & Java'  →  the URL now has a stray, unescaped "&" in it`;

  /** The same query, built so special characters survive the trip. */
  protected readonly urlSearchParamsSample = `const params = new URLSearchParams({ q: term, page: String(page) });
const url = '/search?' + params.toString();
// term = 'C++ & Java'  →  'q=C%2B%2B+%26+Java&page=2' — every character escaped`;
