...
type Second = Tuple[1];               // string — tuples index per position</pre></div>
      ...
      <div class="demo">
        <p class="demo__title">Live — <code>T[number]</code>, indexing an array by position</p>
        <div class="row" style="margin-bottom:10px">
          @for (r of roles; track $index) {
            <button [class.ghost]="roleIndex() !== $index" (click)="roleIndex.set($index)">{{ r }}</button>
          }
        </div>
        <p class="row">
          <code>roles[{{ roleIndex() }}] = '{{ role() }}'</code>
        </p>
        <div class="code"><pre>const roles = ['admin', 'editor', 'viewer'] as const;
type Role = (typeof roles)[number];   // 'admin' | 'editor' | 'viewer'
//                       └── same trick as User['roles'][number] above,
//                           just starting from the array's own typeof</pre></div>
        <p style="color:var(--text-muted);font-size:.85rem">
          Add a fourth role to the array and the <code>Role</code> union — and
          these buttons — pick it up with zero other edits. Same single-source-of-truth
          idea as the THEME demo below, applied to a tuple-like array instead of an object.
        </p>
      </div>
...
function go(to: RouteKey) {{ '{' }} router.navigateByUrl(ROUTES[to]); {{ '}' }}</pre></div>
      ...
      <p>
        None of this exists once your code runs — that's worth sitting with,
        because it explains both the power and the limits of these operators.
      </p>
      <ul>
        <li>
          <strong>Full erasure, zero runtime cost.</strong> The compiler resolves
          <code>typeof</code>/<code>keyof</code>/<code>T[K]</code> entirely during
          type-checking, then strips every type annotation before emitting JS. The
          buttons in the demo above are generated by the real runtime call
          <code>Object.keys(THEME)</code> — <code>ThemeKey</code> itself never
          appears in the compiled output, not even as a string array.
        </li>
        <li>
          <strong>Resolution is structural, not name-based.</strong> <code>keyof T</code>
          doesn't look up a declaration by name; the compiler walks T's actual member
          list (however it was built — an interface, an inferred object literal, a
          mapped type) and unions the property keys it finds. That's why
          <code>keyof typeof THEME</code> works on a plain <code>const</code> with no
          <code>interface</code> in sight.
        </li>
        <li>
          <strong>Index signatures widen the union.</strong> <code>keyof</code> on a type
          with a <code>[key: string]: V</code> index signature includes
          <code>string</code> (and <code>number</code>, since numeric property access is
          stringified) in the key union — the compiler can't enumerate an open-ended
          signature's members, so it falls back to "any string/number key is valid."
        </li>
        <li>
          <strong>This is why "add a key, everything updates" actually holds.</strong>
          Because the union is recomputed from T's structure on every compile rather
          than cached from a prior declaration, there is no second place that can go
          stale — the const object <em>is</em> the type, one build behind.
        </li>
      </ul>
