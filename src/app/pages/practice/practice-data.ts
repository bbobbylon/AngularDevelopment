...
<!-- goal: "She replied" / "He replied" / "They replied" -->`,
  ...
  },
  {
    ...
    options: [
      'A special compiler mode with no equivalent longhand',
      'A property binding plus an event binding: [ngModel]="name" (ngModelChange)="name = $event" — data flows in via the input and out via the change event, which is why you can split them to intercept or transform the outgoing value',
      'Two-way binding directly to the DOM value attribute',
  ...
  },
  {
    ...
    options: [
      'Immediately on page load, so users know the rules upfront',
      'Typically once the control is touched (blurred) or dirty, e.g. @if (email.invalid && (email.touched || email.dirty)) — showing "Email is required" before the user ever reached the field reads as shouting at them for nothing',
      'Only after the form is submitted successfully',
  ...
  },
  {
    ...
    options: [
      'Bind [disabled]="isDisabled" on the input like any other property',
      'Use the API: new FormControl({ value: "", disabled: true }) at creation or control.disable()/enable() at runtime — binding [disabled] on a formControlName input conflicts with the form model (Angular warns) because the FormControl owns that state',
      'Set control.readonly = true',
  ...
  },
  {
    ...
    options: [
      'FormControl, FormGroup, FormArray — a single value (one input), a fixed named collection of controls (an object), and a variable-length list of controls (an array), all sharing the AbstractControl API (value, valid, valueChanges…) and nesting arbitrarily',
      'Input, Form, Button — the three HTML elements Angular enhances',
      'Model, View, Controller — the MVC triad applied to forms',
  ...
  },
  {
    ...
    options: [
      'Only one validator is allowed per control',
      'Pass an array: new FormControl("", [Validators.required, Validators.email]) or new FormControl("", [Validators.required, Validators.minLength(8)]) — each failing validator contributes its own key to control.errors, so you can show per-rule messages',
      'Chain them: Validators.required.email.minLength(8)',
  ...
  },
  {
    ...
    options: [
      'ngSubmit is required or Angular throws a runtime error',
      'ngSubmit fires for EVERY way a form submits — clicking the submit button AND pressing Enter in a field — and Angular prevents the default full-page post; a click handler misses keyboard submission and screen-reader form semantics',
      'click handlers cannot call component methods',
  ...
  },
  {
    ...
    options: [
      'form.address.street — controls become properties automatically',
      'form.get("address.street") with a dot path (or form.controls.address.controls.street for full typing) — get() walks nested groups/arrays; the template mirrors the nesting with formGroupName="address" around formControlName="street"',
      'form.find("street") searches all levels by name',
  ...
  },
  {
    ...
    options: [
      'Call the API in (keyup) directly — simplest is best',
      'searchCtrl.valueChanges.pipe(debounceTime(300), distinctUntilChanged(), switchMap(q => this.api.search(q))) — wait for a typing pause, skip unchanged values, and cancel the in-flight request when a newer query arrives',
      'Poll the API every second and filter client-side',
...
// usage: new FormControl('', [Validators.required, noSpaces])`,
    options: [
      'control.value.includes(" ") — returning a boolean pass/fail',
      'control.value?.includes(" ") ? { noSpaces: true } : null — a validator returns a ValidationErrors object (keyed so the template can read errors?.["noSpaces"]) when INVALID and null when valid',
      'throw new ValidationError("spaces") when invalid',
  ...
  },
  {
    ...
    options: [
      'They are interchangeable in any context',
      'formControlName is a STRING key resolved against the nearest parent [formGroup]/formGroupName; [formControl] binds a standalone FormControl INSTANCE directly and needs no surrounding group — use it for lone controls like a search box',
      '[formControl] is deprecated in favor of formControlName',
  ...
  <input formControlName="email"
    ...
    options: [
      'It works — ngModel and formControlName sync automatically',
      'Mixing ngModel with reactive directives on the same control is unsupported: Angular 6 deprecated it and newer versions throw/warn, because two owners (template-driven state and the FormControl) fight over one value. Pick one system: drop ngModel and read form.get("email")?.value instead',
      'ngModel silently wins and the FormControl is detached',
  ...
  },
  {
    ...
    options: [
      'Nothing — it is an alias for valueChanges',
      'A single typed stream of ALL control events — ValueChangeEvent, StatusChangeEvent, TouchedChangeEvent, PristineChangeEvent (and submit/reset on the root form) — including TOUCHED and PRISTINE transitions, which previously had no observable at all, filterable with instanceof',
      'It replaces validators with event handlers',
  ...
  },
  {
    ...
    options: [
      'Order is irrelevant — Angular picks the most specific match automatically',
      'The router uses FIRST-MATCH-WINS, scanning top to bottom — so specific/static routes go before parameterized ones (products/new before products/:id) and the ** wildcard goes LAST, because it matches everything and would swallow all routes placed after it',
      'Routes are matched alphabetically by path',
  ...
  },
  {
    ...
    options: [
      'pathMatch is decorative and can be omitted',
      'The default pathMatch is "prefix", and EVERY url starts with the empty string — so an empty-path redirect without "full" matches everything and causes an infinite-redirect error; "full" says: redirect only when the ENTIRE remaining url is empty',
      'pathMatch: "full" makes the redirect permanent (301)',
  ...
  <a ____>{{ product.name }}</a>
    ...
    options: [
      ...
      '[routerLink]="[\'/products\', product.id]" — the array form builds the url from segments, handling encoding, and routerLink keeps navigation inside the SPA',
      'routerLink="/products/product.id" — the string is evaluated automatically',
  ...
  },
  {
    ...
    options: [
      'href is forbidden inside Angular templates',
      'href performs a full browser navigation — the app reloads, all in-memory state is lost, and every bundle re-downloads; routerLink intercepts the click and routes client-side, swapping components instantly while still rendering a real <a href> for accessibility, SEO, and open-in-new-tab',
      'routerLink is only needed for lazy-loaded routes',
  ...
  },
  {
    ...
    options: [
      'It renders the routed component INSIDE itself like a container div',
      'It is a marker/placeholder: the router creates the matched route\'s component as a SIBLING right after the outlet element in the DOM — the app shell (nav, footer) around it stays put across navigations, and child routes need their own outlet in the parent\'s template',
      'It fetches the component\'s data before display',
  ...
  },
  {
    ...
    options: [
      'Query params always survive navigation automatically',
      'Pass queryParamsHandling: "preserve" to carry the CURRENT params unchanged, or "merge" to combine them with newly provided queryParams (new keys win) — e.g. router.navigate(["details"], { queryParams: { tab: "specs" }, queryParamsHandling: "merge" }); default handling DROPS them',
      'Store the params in localStorage and restore manually',
  ...
  },
  {
    ...
    options: [
      'They are identical; navigateByUrl is legacy',
      'navigate() takes a COMMANDS ARRAY (segments, outlets, relative navigation via relativeTo) and composes the url; navigateByUrl() takes a complete absolute url STRING/UrlTree and replaces the whole url — use navigate for programmatic composition, navigateByUrl when you already hold the full path (e.g. a stored returnUrl)',
  ...
  },
  {
    ...
    options: [
      'They are two names for the same map of everything in the url',
      'paramMap holds ROUTE parameters defined in the path pattern (products/:id → id = "42", scoped to that route); queryParamMap holds the ?key=value query string (sort = "price", global to the whole url) — both expose values as STRINGS via get()/getAll() and exist as snapshot or observable',
      'queryParamMap only works with HashLocationStrategy',
  ...
  },
  {
    ...
    options: [
      'Set document.title in every component constructor',
      'Use the route title property — { path: "products", title: "Products · Shop" } (or a resolver function for dynamic titles) — and optionally a custom TitleStrategy to decorate all titles with a suffix in one place',
      'Titles can only be set in index.html once',
  ...
  },
  {
    ...
    options: [
      'The Angular router is broken in production builds',
      'With the default PathLocationStrategy the server receives a request for /products/42, and no such FILE exists — the server must be configured to rewrite unknown paths to index.html so the SPA boots and its router takes over; the alternative is HashLocationStrategy (/#/products/42), where everything after # never reaches the server',
      'Deep links require SSR in all cases',
  ...
  },
  {
    ...
    options: [
      'Reuse one component class for several route paths',
      'Control whether the router DESTROYS or DETACHES-AND-STORES a route\'s component subtree on navigation — e.g. cache a heavy search-results page (shouldDetach/store) and reattach it with scroll position and state intact when the user navigates back (shouldAttach/retrieve)',
  ...
  },
  {
    ...
    options: [
      'This is impossible — same-url navigation is always a no-op',
      'onSameUrlNavigation: "reload" makes the router process navigation to the identical url instead of skipping it, and runGuardsAndResolvers (e.g. "always") on the route controls when its guards/resolvers re-execute — combine them so a same-url navigate() re-runs the resolver and refreshes the data',
      'Call window.location.reload() — that is the intended API',
