...
export const CHALLENGES: Challenge[] = [
  // --- COMPONENTS ---
  {
    ...
    answer: 1,
    explanation:
      '@Component marks a class as an Angular component and configures its template, styles, and selector. It creates a self-contained, reusable UI building block. Why others fail: @NgModule groups multiple components and services into a feature or shared module. @Injectable enables any class (services, guards, resolvers) to be injected; not specific to components. @Directive adds reusable behavior to existing elements without managing its own view.',
...
export class GreetingComponent {
  name = 'World';
    ...
    answer: 3,
    explanation:
      'Modern Angular uses standalone-first architecture. In projects without NgModules, components must declare standalone: true in their @Component decorator, either explicitly or implicitly through schema assumptions. The template syntax {{ name }} is correct. Why others fail: Selectors can use any prefix; "app-" is conventional. Interpolation is correct syntax. OnInit is only required if you need lifecycle hooks.',
  },
  {
    id: 3,
    type: 'predict-output',
    ...
    category: 'components',
    question:
      'What does fixture.nativeElement.querySelector(".count").textContent contain after this test runs?',
    code: `@Component({
  standalone: true,
...
fixture.detectChanges();`,
    ...
    answer: 2,
    explanation:
      'Each click() increments count by 1. Two clicks make count = 2. fixture.detectChanges() triggers Angular\'s change detection and re-renders the template with the updated value, so the span text becomes "2". Why others fail: Clicks happen before detectChanges. detectChanges renders the final state. The span displays the interpolated value once detectChanges runs.',
  },
  {
    id: 4,
    type: 'multiple-choice',
    ...
    answer: 2,
    explanation:
      'ngOnChanges fires before ngOnInit on the first render (if the component has @Input properties), then fires again whenever any @Input reference changes. It receives a SimpleChanges object showing previous and current values. Note: it does NOT fire for signal inputs — those are reactive by default and do not use ngOnChanges. Why others fail: Fires on first render but also on subsequent @Input changes. Fires on @Input changes, not on every CD pass. Fires throughout the component lifecycle, not just at end.',
  },
...
count.update(n => n * 2);`,
    ...
    answer: 2,
    explanation:
      'signal(0) creates a signal with initial value 0. set(5) replaces the value with 5. update(n => n * 2) reads the current value (5) and updates it to 5 * 2 = 10. So count() returns 10. Why others fail: 0 is the initial state before modifications. 5 was the intermediate value after set(), but before update() runs. The callback multiplies by 2, not squares the value.',
    topicPath: 'signals',
  },
  {
    id: 6,
...
const total = computed(() => {
  return price + tax;  // ???
    ...
    answer: 1,
    explanation:
      'Signals are getter functions — to read their current value you must call them: price() and tax(). Writing price + tax tries to add the signal function objects together (resulting in NaN), not their values. The fix is: return price() + price() * tax() for the calculated total. Why others fail: const is the correct declaration for computed. Addition works on numbers regardless of integer or decimal type. computed() returns a Signal<T>, not an Observable.',
  },
  {
    id: 7,
    type: 'multiple-choice',
    ...
    answer: 0,
    explanation:
      'computed() derives a new signal value from other signals — it is lazy (only runs when read) and memoized (cached until dependencies change), returning Signal<T>. effect() runs arbitrary side-effect code (logging, DOM writes, API calls) whenever its reactive dependencies change. Use computed() for derived state; use effect() for imperative side effects. Why others fail: They serve different purposes. Both can be used in any context. Both return their respective types.',
  },
  {
    id: 8,
    type: 'spot-the-bug',
...
cartStore.items.set([]);   // can mutate store directly`,
    ...
    answer: 1,
    explanation:
      'Exposing a writable signal publicly breaks encapsulation — any component can mutate store state directly, bypassing validation or business logic. The fix: make it private readonly _items = signal([]) and expose readonly items = this._items.asReadonly(). All mutations should go through explicit methods like add(item) and remove(id). Why others fail: Signals work perfectly in services. computed() can reference signals from the same class. Signals are a simpler alternative to NgRx.',
  },
  ...
  // --- RxJS ---
  {
    ...
    answer: 0,
    explanation:
      'map() transforms each emitted value using a synchronous function (like Array.map for streams). switchMap() projects each value to a new inner Observable and automatically cancels/unsubscribes from the previous inner Observable, "switching" to the new one. This makes switchMap() essential for HTTP requests where you want to discard stale responses (e.g., typeahead search). Why others fail: switchMap() is not for filtering; use filter() for that. They are completely different operators with distinct use cases. Both work with Observables; switchMap() is actually designed for Observable-returning functions.',
...
export class DataComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    answer: 1,
    explanation:
      'When the component is destroyed, the subscription to users$ keeps running, creating a memory leak and potential errors when updating a destroyed component. Fix options: (1) Use takeUntilDestroyed(inject(DestroyRef)) in the pipe. (2) Use toSignal(this.userService.users$) which auto-unsubscribes. (3) Use the async pipe in the template instead of manual subscribe. Why others fail: Signals can be set in subscribe callbacks. The async pipe is an alternative, not a requirement. Manual subscriptions are fine if properly cleaned up.',
  },
  {
    id: 11,
    type: 'multiple-choice',
    ...
    category: 'rxjs',
    question:
      'Which flattening operator should you use for an HTTP search that cancels stale requests?',
    options: [
    ...
    answer: 2,
    explanation:
      'switchMap cancels the previous inner Observable when a new value arrives — perfect for typeahead search where you want to discard stale HTTP requests. If the user types again, the old request is cancelled and the new one runs. Why others fail: mergeMap runs all concurrently, so stale responses can arrive late and overwrite newer data. concatMap queues them sequentially, which is too slow for search. exhaustMap ignores new requests while one is pending, which prevents rapid searches from working correctly.',
  },
  {
    id: 12,
    type: 'predict-output',
  ...
  map(n => n * 10)
    ...
    options: [
      '1, 2, 3 — all values are emitted unchanged',
      '10, 20, 30 — all values are multiplied by 10',
      '10, 30 — odd numbers filtered then multiplied',
      '1, 3 — filter keeps odds but map is not applied',
    ],
    answer: 2,
    explanation:
      'filter(n => n % 2 !== 0) keeps only odd numbers: 1 and 3. map(n => n * 10) multiplies each by 10: 10 and 30. So console.log is called twice with 10, then 30. Why others fail: Filter removes the even number (2). Filter removes 2 before map runs. Map is definitely applied to filtered values.',
  },
  ...
  // --- FORMS ---
  {
    ...
    answer: 1,
    explanation:
      'Template-driven forms (FormsModule, ngModel) let the template drive the form model — easier for simple forms but harder to test and validate. Reactive forms (ReactiveFormsModule, FormControl/FormGroup/FormBuilder) define the structure in the component class — explicit, testable, and required for complex forms with dynamic controls. Why others fail: Speed is similar; power/flexibility is the real difference. They are architecturally different approaches. Both can work with signals or zones.',
  },
  {
    id: 14,
    type: 'spot-the-bug',
...
// <input [formControl]="form.get('email')" />`,
    ...
    answer: 1,
    explanation:
      'form.get("email") returns AbstractControl | null (could be undefined), but [formControl] expects FormControl<T>. This type mismatch causes binding issues. Fix: create a typed control variable: emailCtrl = this.form.get("email") as FormControl; then use [formControl]="emailCtrl". Better yet, use FormBuilder which is type-safe. Why others fail: FormBuilder is recommended but direct constructor works. Validators.email is built-in. Validation runs regardless of ngSubmit.',
  },
  ...
  // --- ROUTING ---
  {
    ...
    answer: 1,
    explanation:
      'CanActivateFn returns: true (or Promise/Observable of true) to allow navigation, false to block it, or a UrlTree (created via Router.createUrlTree()) to redirect. Returning false just blocks the current route; returning a UrlTree redirects to another route — this is the correct way to send unauthenticated users to /login. Why others fail: null/undefined is falsy and blocks. void is not a valid guard return. It can return an Observable, but not required.',
  },
  {
    id: 16,
    type: 'spot-the-bug',
  ...
  // missing return!
    ...
    answer: 1,
    explanation:
      'When isLoggedIn() is false, navigate() is called but the function falls off the end and returns undefined. Angular treats undefined as allowing navigation (falsy but not explicitly false). The fix: return inject(Router).createUrlTree(["/login"]) which both redirects AND explicitly blocks. Never call navigate() in a guard — return a UrlTree. Why others fail: inject() works fine in function guards. createUrlTree() is synchronous. It can return Observable but sync boolean is fine.',
  },
  ...
  // --- TESTING ---
  {
    ...
    answer: 1,
    explanation:
      'httpMock.verify() asserts that every HTTP request made during the test was explicitly handled (flushed or errored). If a request was made but never handled, verify() throws, catching silent test failures. Without verify(), unhandled requests pass silently, hiding bugs. Place it in afterEach() to run after every test. Why others fail: It does not close connections; that is handled by unsubscribe. It does not reset state. It is required for all request types, not just POST/PUT.',
  },
  {
    id: 18,
    type: 'spot-the-bug',
  ...
  expect(users![0].name).toBe('Ada');
    ...
    answer: 1,
    explanation:
      'This test is correct! HttpTestingController.flush() is synchronous — it immediately delivers the mocked response and resolves the Observable. The subscribe callback runs synchronously, setting users, and then expect() correctly sees the value. The test passes for the right reasons. Why others fail: flush() is explicitly synchronous. expectOne() is correct for single request. fakeAsync is not required for HttpTestingController tests.',
  },
...
class ProductList {
  ...
  getFilteredItems() {
    ...
    answer: 1,
    explanation:
      'getFilteredItems() is a method call in the template. Angular invokes it on every change-detection pass — potentially dozens of times per second. Each call filters the entire array. Fix: replace with a computed signal: readonly filteredItems = computed(() => this.items().filter(i => i.active)). computed() is memoized — only re-runs when items changes. Why others fail: track item.id is correct for identity tracking. @for can call methods. Signals and methods coexist fine.',
  },
  {
    id: 20,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'NgOptimizedImage (ngSrc) with the priority attribute tells Angular to add both fetchpriority="high" and a <link rel="preload"> in the document head — both critical for LCP. This is the optimized, recommended approach. Why others fail: Raw fetchpriority works but misses the preload hint and Angular\'s optimization. [fetchpriority] property binding does not work because it is not a DOM property. Angular has full support via NgOptimizedImage.',
  },
...
const result = identity(42);`,
    options: [
      'any — no type information is available',
      'number — inferred from the argument type',
      'T — the generic type variable',
      'unknown — TypeScript cannot determine the type',
    ],
    answer: 1,
    explanation:
      'TypeScript infers the generic type parameter T from the argument. identity(42) passes the number literal 42, so T is inferred as number. result has type number (not T, which is a placeholder resolved at compile time). Why others fail: T is inferred, not any. T is a type parameter resolved at call time; result is number. T is known through inference.',
  },
  {
    id: 22,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'Both describe object shapes, but differ: interface supports declaration merging (redefine same name to extend it — useful for augmenting third-party types) and uses extends for inheritance. type supports unions (A | B), intersections (A & B), mapped types, and conditional types. In Angular: use interface for data models, type for unions/utility types. Why others fail: They have distinct capabilities. Both work with primitives and objects. Both are current; interface is not deprecated.',
  },
  {
    id: 23,
    type: 'spot-the-bug',
...
type ReadonlyUser = Readonly<User>;`,
    ...
    answer: 1,
    explanation:
      'Mapped types iterate keys with [K in keyof T]. This type adds readonly to every property. ReadonlyUser becomes { readonly id: number; readonly name: string }. Attempting user.id = 5 is a compile error. This is the actual TypeScript Readonly utility type. Why others fail: readonly is applied. readonly modifies properties, not wraps them. readonly works on any property type.',
  },
  {
    id: 25,
    type: 'multiple-choice',
    ...
    answer: 3,
    explanation:
      'OnPush components only re-check when: @Input references change, events fire, signals change, or async pipe emits. Mutating an array/object in place (push, splice, obj.prop = x) does NOT change the reference — Angular sees the same object and skips re-check. Always update immutably: this.items = [...this.items, newItem]. Why others fail: Reference change triggers re-check. Events trigger re-check. Signal changes trigger re-check.',
  },
  ...
  // --- MORE COMPONENTS ---
  {
    ...
    answer: 1,
    explanation:
      '"on idle" triggers when the browser fires requestIdleCallback — waits for a quiet moment. "on immediate" loads instantly (no deferral). "on viewport" loads when placeholder scrolls into view. "on interaction" loads on first user interaction. Use "on idle" for helpful content not critical to first paint. Why others fail: immediate has no deferral. viewport is for visibility-based loading. interaction requires user action.',
  },
  {
    id: 27,
    type: 'spot-the-bug',
...
export class EditorComponent {
  ...
  save() {
    ...
    answer: 1,
    explanation:
      'output<number>() declares the event emits a number. Calling emit("done") passes a string — a compile-time type error. TypeScript will catch this. Fix: emit the correct type, e.g., this.saved.emit(42). The output() API (Angular 17+) is fully typed, unlike EventEmitter which accepts any. Why others fail: output() is the modern API. output() is called in class body; emit() is in methods. No lifecycle requirement.',
  },
  {
    id: 28,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      '@Component extends @Directive under the hood. Every component is a directive that additionally has a template and encapsulated view. @Directive augments an existing element (adds classes, listens to events, DOM manipulation) without creating new DOM. @Component creates and renders a self-contained view. Use @Directive for reusable behaviors; @Component for UI building blocks. Why others fail: They have an inheritance relationship. @Directive is not simplified; directives are powerful. They are not aliases.',
  },
  ...
  // --- MORE SIGNALS ---
  {
    ...
    category: 'signals',
    question:
      'What does toSignal(obs$, { initialValue: [] }) guarantee that plain toSignal(obs$) does not?',
    options: [
    ...
    answer: 1,
    explanation:
      'ToSignal() without initialValue returns Signal<T | undefined> because the Observable may not have emitted yet. Reading the signal before first emission returns undefined. With { initialValue: [] }, the return type is Signal<T[]> (no undefined) and the signal immediately holds the initial value. Use initialValue when your template cannot handle undefined. Why others fail: Both subscribe eagerly. Neither auto-unsubscribes after one emission. Neither handles multicasting by default.',
  },
  {
    id: 30,
    type: 'spot-the-bug',
...
export class Counter {
  ...
  constructor() {
    ...
    answer: 1,
    explanation:
      'The effect reads count() (registering a dependency) then calls count.set(), which triggers the effect again → infinite loop. Angular detects this and throws. Fix: if you need derived state use computed(). If you must write in an effect, wrap the read in untracked(() => this.count()) so it is not a dependency. Never write to a signal that the same effect reads. Why others fail: effect() works in constructors. computed() is read-only. set() is allowed.',
  },
  {
    id: 31,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'computed() is always read-only. linkedSignal() is BOTH reactive AND writable: when source changes, it resets to the derived value; but you can also call .set() to override it independently. Example: selectedItem defaults to items()[0] but the user can pick a different item. computed() cannot model this (no write path). Why others fail: It is not permanent; resets on source change. It is not about multicasting. linkedSignal() stays as a signal.',
  },
...
export class UserService {
  ...
  constructor(private http: HttpClient) {}
    ...
    options: [
      ...
      '3 requests total — a cold Observable re-executes its producer per subscription',
      '0 requests — cold observables never execute',
      'Depends on Zone.js timer configuration',
    ],
    answer: 1,
    explanation:
      'http.get() returns a cold Observable — each subscription starts a new HTTP request from scratch. Three subscriptions = three separate requests. Fix: add shareReplay(1): users$ = this.http.get(...).pipe(shareReplay(1)). This executes once, multicasts the response to all subscribers, and replays it to future subscribers. Why others fail: Without shareReplay it is cold. Cold observables DO execute when subscribed. Zone.js does not affect this.',
  },
  {
    id: 33,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'combineLatest([a$, b$]) emits whenever ANY source emits, using the latest value from each. If a$ emits 10 times and b$ emits once, you get 10 emissions. zip([a$, b$]) waits for BOTH sources to emit their Nth value before emitting the Nth pair — steps through in lockstep. Use combineLatest for reactive state; use zip for one-to-one event pairing. Why others fail: They have different emission strategies. Both work with Observables. Both are current.',
  },
  {
    id: 34,
    type: 'spot-the-bug',
...
export class PollingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    answer: 1,
    explanation:
      'When the component is destroyed, the interval Observable keeps emitting every 5 seconds. Each tick fires an HTTP request and updates a destroyed component — a leak. Fix: add takeUntilDestroyed(inject(DestroyRef)) to auto-unsubscribe: interval(5000).pipe(switchMap(...), takeUntilDestroyed(inject(DestroyRef))).subscribe(...). Or use resource() which ties lifetime to the component. Why others fail: interval() works fine in components. switchMap() is correct for polling. Manual subscriptions are fine if cleaned up.',
  },
  ...
  // --- MORE FORMS ---
  {
    ...
    answer: 2,
    explanation:
      'FormArray.push() appends a control AND triggers value/status notifications. Pushing directly to .controls bypasses the notification mechanism — form state does not update. Always use FormArray methods: push() to append, insert(index, control) for position-specific, removeAt(index) to remove. Never mutate .controls directly. Why others fail: Bypasses notifications. add() does not exist. append() does not exist.',
  },
  {
    id: 36,
    type: 'spot-the-bug',
...
loadProfile(profile: UserProfile) {
  this.form.setValue({
    ...
    answer: 1,
    explanation:
      'setValue() is strict — throws "Must supply a value for form control with name: \'lastName\'" if any control is omitted. Use patchValue() for partial updates: it updates only provided keys, ignoring missing ones. Rule: use setValue() when loading a complete model (all keys guaranteed); use patchValue() for partial updates. Why others fail: setValue() is correct. FormBuilder is recommended but direct constructor works. Object structure must match form structure, not template.',
  },
  {
    id: 37,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'Async validators return Observable<ValidationErrors | null> or Promise<ValidationErrors | null>. While running, the FormControl.status is "PENDING". On completion: null = valid, ValidationErrors object = invalid. In templates: check control.status === "PENDING" to show a loading spinner. Apply debounceTime inside validators to avoid hammering the server on every keystroke. Why others fail: Returns observable, not null. Status is a string. No sentinel needed.',
  },
  ...
  // --- MORE ROUTING ---
  {
    ...
    answer: 1,
    explanation:
      'CanMatchFn returning false makes the router skip this route definition and try the next matching route — enables "role-based routing" with multiple routes at the same path. CanActivateFn returning false blocks the already-matched route; no fallback route is tried. Use canMatch for "different components, same URL"; use canActivate for "block access or redirect to login". Why others fail: They behave differently. CanMatchFn works on all routes. Both run, but CanMatchFn runs first.',
  },
  {
    id: 39,
    type: 'spot-the-bug',
    ...
    code: `export const routes: Routes = [
  {
    ...
    answer: 1,
    explanation:
      'loadComponent is for lazy-loading standalone components. It expects the promise to resolve to a component class, not an NgModule. Providing SettingsModule causes a runtime error. Fix: for modules use loadChildren: () => import("./settings/settings.module").then(m => m.SettingsModule); for standalone routes use loadChildren: () => import("./settings/settings.routes").then(m => m.SETTINGS_ROUTES). Why others fail: import() works fine. Paths do not need slashes. Guards are optional.',
  },
  ...
  // --- MORE TESTING ---
  {
    ...
    answer: 1,
    explanation:
      "fakeAsync() patches all timer APIs (setTimeout, setInterval, Promises) to use a virtual clock. Inside the test, call tick(500) to advance time 500ms synchronously — this runs any scheduled callbacks. Use flushMicrotasks() to drain pending Promises. This makes debounce, throttle, and delay logic testable without actually waiting. Without fakeAsync you must use async/await or done() callbacks. Why others fail: fakeAsync runs in the same thread. Does not affect setup. That is HttpTestingController's job.",
  },
  {
    id: 41,
    type: 'spot-the-bug',
  ...
  expect(el.textContent).toContain('Alice');
    ...
    answer: 1,
    explanation:
      'Assigning component.user changes the TypeScript value but does not run change detection. The DOM still shows whatever was rendered from the previous detectChanges() call. The test passes only if the initial render happens to contain "Alice". Fix: call fixture.detectChanges() after setting the property to trigger re-render. This updates the DOM before the assertion. Why others fail: querySelector works in tests. Plain objects work; signals are optional. toContain is correct for substring matching.',
  },
  {
    id: 42,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'querySelector tests are brittle — changing div to button, renaming a CSS class, or restructuring DOM breaks them even if behavior is unchanged. Harnesses wrap the component behind a semantic behavioral API. When you refactor the DOM, only the harness implementation changes; tests stay the same. Angular Material ships harnesses for every component. For custom components, extend ComponentHarness. Test behaviors, not implementation. Why others fail: Speed is similar. Harnesses work with any component. Harnesses do not add attributes.',
  },
  ...
  // --- MORE PERFORMANCE ---
  {
    ...
    answer: 1,
    explanation:
      '@defer separates "when to prefetch" from "when to render". Example: @defer (on interaction; prefetch on idle) downloads the chunk during idle time before the user interacts, so interaction renders immediately without delay. Without prefetch, interaction has latency while downloading. This is Angular\'s equivalent of <link rel="prefetch"> scoped to a component. Combine with "on viewport" for below-fold content. Why others fail: Renders only when trigger fires. Prefetch happens regardless of connection. Prefetch does not inline.',
  },
  {
    id: 44,
    type: 'spot-the-bug',
    ...
    answer: 1,
    explanation:
      'NgOptimizedImage requires explicit width and height on every image. These establish the aspect ratio so the browser reserves exact layout space before the image loads — preventing CLS (Cumulative Layout Shift). Without them, the image loads, DOM shifts, causing visual jank. NgOptimizedImage throws a dev-mode error if width/height are missing. Fix: <img ngSrc="/assets/hero.webp" width="1200" height="400" alt="Hero banner" priority />. Width/height set intrinsic dimensions, not CSS display size. Why others fail: Relative paths work fine. priority is required. .webp is supported.',
  },
  {
    id: 45,
    type: 'multiple-choice',
    ...
    answer: 1,
    explanation:
      'Route-level code splitting has the highest ROI. Each lazy route becomes a separate chunk downloaded only when the user navigates there. The initial load includes only the app shell and entry route. Everything else defers. Additional techniques: @defer for below-fold components, tree-shaking unused exports, custom preload strategy to load important routes in background. Why others fail: Optimization improves performance. Types are stripped at compile time. OnPush improves runtime performance, not bundle size.',
  },
...
type Keys = keyof typeof config;`,
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'Without "as const", ROLES has type string[] — TypeScript only knows it is an array of strings, not which strings. With "as const", the type is readonly ["admin", "editor", "viewer"] — every literal is preserved. This lets you derive a union: type Role = typeof ROLES[number] gives "admin" | "editor" | "viewer". Without "as const" you would get string. This pattern is more tree-shake-friendly than const enums and transparent at runtime. Why others fail: Does not make runtime immutable. Not a const enum. Does not prevent tree-shaking.',
  },
  ...
  // --- ARCHITECTURE ---
  {
    ...
    category: 'components',
    question:
      'If a service is in BOTH root injector and component.providers, which instance do descendants get?',
    options: [
...
export class RouterService {
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'CanMatchFn returning false causes the router to skip that route entirely and try the next route definition. So: [{ path: "dashboard", canMatch: [isAdmin], loadComponent: () => AdminDashboard }, { path: "dashboard", loadComponent: () => UserDashboard }]. Admins match the first route; non-admins fall through to the second. CanMatchFn is specifically designed for "different components, same URL" routing. Why others fail: CanActivateFn blocks but does not try other routes. CanLoadFn is for chunk loading, not routing. Resolvers are for data, not component selection.',
  },
...
<ul>
  ...
  },
  {
    ...
    category: 'components',
    question:
      'When is `ViewContainerRef.createEmbeddedView(templateRef, context)` used over `ngTemplateOutlet`?',
    options: [
  ...
  },
  {
    ...
    category: 'components',
    question:
      'What is the key structural difference between an attribute directive and a structural directive?',
    options: [
...
export class ChartComponent implements OnInit {
  ...
  },
  {
    ...
    category: 'components',
    question:
      "A component's template has `{{ getUser() }}`. Why can this be a performance problem?",
    options: [
  ...
  },
  {
    ...
    category: 'components',
    question:
      'What is the Directive Composition API (`hostDirectives`) and what problem does it solve?',
    options: [
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'What does `untracked(() => this.sideEffectValue())` do inside a `computed()` or `effect()`?',
    options: [
    ...
    answer: 0,
    explanation:
      '`untracked()` executes a function outside the reactive tracking context. Any signals read inside `untracked()` are NOT registered as dependencies. This is useful when you need a signal\'s value for a side-effect purpose but do not want the computed/effect to re-run when that signal changes. The "logs to DevTools" idea is wrong — no DevTools logging occurs. The "one-time snapshot" idea is wrong — it does not snapshot; the value is live if read again later. The "only valid in effects" idea is wrong — `untracked` works in both `computed` and `effect`.',
  },
  {
    id: 70,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      "`ResourceStatus` is a union of six lowercase string literals — `'idle'` (no request yet), `'loading'` (first fetch in flight), `'reloading'` (re-fetching while a value is already present), `'resolved'` (fetch succeeded), `'error'` (fetch failed), `'local'` (value was set directly via `.set()`/`.update()` rather than fetched). It is a string-literal type, not a TypeScript `enum` — there is no capitalized `Idle`/`Loading`/etc. Check `resource.isLoading()` for a convenience boolean instead of comparing `status()` yourself. A and D under-count and use the wrong names; C invents an enum that does not exist.",
  },
  {
    id: 72,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'What happens when this component is used as `<app-counter>` without the `start` binding?',
    code: `@Component({ standalone: true, template: '{{ count() }}' })
export class CounterComponent {
  ...
  readonly count = computed(() => this.start() * 2);
    ...
    answer: 1,
    explanation:
      '`input.required<T>()` enforces that the parent must provide a value. Angular throws a runtime error at component initialisation if the binding is missing. The error message is "Required input \'start\' is not provided." The "renders 0" idea is wrong — there is no default-to-zero behaviour. The "renders NaN" idea is wrong — it never reaches rendering. The "TypeScript prevents compilation" idea is wrong — the check is at runtime, not compile time (though Angular\'s language service in strict mode may warn).',
  },
  {
    id: 74,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'What is the key difference between `signal()` state and `FormControl` reactive state?',
    options: [
...
// {{ result() }}`,
  ...
  },
  {
    ...
    category: 'rxjs',
    question:
      'In `catchError`, what is the difference between returning `of(null)` vs returning `EMPTY`?',
    options: [
...
of(1, 2, 3).pipe(
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      "`scan((acc, val) => acc + val, 0)` on `of(1, 2, 3)` emits 1, 3, 6 — unlike `reduce()` which only emits the final value. `scan()` emits after EVERY item, making it perfect for building a running total, an accumulated array (`scan((acc, item) => [...acc, item], [])`), or an event log. Describing it as scanning for a value and filtering non-matches is wrong — that's what `filter` does. Describing it as buffering everything and emitting one array at the end is wrong — that's `toArray()`. And it isn't simply reduce() restricted to hot Observables — `reduce()` emits only the final value (and only once the source completes) regardless of whether the source is hot or cold, whereas `scan()` emits every intermediate value as it goes.",
  },
  {
    id: 85,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'rxjs',
    question:
      'Which is the RECOMMENDED modern way to clean up a subscription in an Angular service?',
    options: [
    ...
    answer: 1,
    explanation:
      '`takeUntilDestroyed()` (Angular 16+) is the cleanest pattern for services and components. Inject `DestroyRef` (or use the overload `takeUntilDestroyed(inject(DestroyRef))`). The stream auto-completes when the context is destroyed — no `ngOnDestroy` boilerplate. A works but is verbose. C works for templates but not service-side subscriptions. D only works for single-emission Observables.',
  },
  {
    id: 90,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'rxjs',
    question:
      'In an Angular service, `http.get("/config")` is called in the constructor. What is the problem?',
    options: [
  ...
  },
  {
    ...
    category: 'rxjs',
    question:
      'What does `combineLatest([a$, b$]).pipe(filter(([a, b]) => a !== null && b !== null))` solve?',
    options: [
  ...
  },
  {
    ...
    category: 'forms',
    question:
      'How do you implement a cross-field validator that ensures `password === confirmPassword`?',
    options: [
  ...
  },
  {
    ...
    category: 'forms',
    question:
      'Which reactive forms approach allows you to listen to all value changes in a FormGroup?',
    options: [
  ...
  },
  {
    ...
    category: 'forms',
    question:
      'What is the benefit of the typed Reactive Forms API (`FormGroup<{ name: FormControl<string> }>`) introduced in Angular 14?',
    options: [
  ...
  },
  {
    ...
    category: 'routing',
    question:
      'What is the difference between `route.snapshot.paramMap.get("id")` and `route.paramMap.pipe(map(m => m.get("id")))`?',
    options: [
  ...
  },
  {
    ...
    category: 'routing',
    question:
      'What is the execution order when navigating to a route with both `canMatch` and `canActivate` guards?',
    options: [
  ...
  },
  {
    ...
    category: 'routing',
    question:
      'How do you implement a custom preloading strategy to preload only routes with `data: { preload: true }`?',
    options: [
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'What is the difference between `DebugElement.triggerEventHandler("click", event)` and `nativeElement.click()`?',
    options: [
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'In a unit test using `HttpClientTestingModule`, why do you call `httpMock.verify()` in `afterEach`?',
    options: [
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'How do you test that a component method called `this.router.navigate(["/home"])` actually navigates?',
    options: [
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'What is the advantage of `ComponentFixture.whenStable()` over using `tick()` in `fakeAsync()`?',
    options: [
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'You want to provide a mock for `AuthService` in a test. Which approach correctly replaces it in DI?',
    options: [
    ...
    answer: 1,
    explanation:
      '`{ provide: AuthService, useValue: mockAuthService }` replaces the real service in the test injector. Any component that injects `AuthService` receives your mock. `mockAuthService` can be a jasmine spy object: `jasmine.createSpyObj("AuthService", ["login", "logout"])`. Just importing `AuthService` and creating a plain mock object with no DI wiring does not replace it in DI — Angular\'s injector never learns about your mock. Adding it to `declarations` is wrong — `declarations` is for components/pipes/directives, not for providing services. And using `jest.mock("./auth.service")` mocks the module at the module-resolution level but does not integrate with Angular\'s own DI container.',
  },
  {
    id: 132,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      '`<cdk-virtual-scroll-viewport itemSize="50" style="height: 400px"><div *cdkVirtualFor="let item of items">...</div></cdk-virtual-scroll-viewport>`. The CDK only renders the visible items plus a small buffer, keeping the DOM to ~20 nodes regardless of list size. Plain CSS overflow scrolling still renders every DOM node — the browser just clips what\'s visible, so it doesn\'t reduce DOM size. The `*ngFor` with a `[limit]` claim is wrong — `*ngFor` has no `[limit]` input. Virtual scrolling being built into `@for` is also wrong — it still requires the separate CDK package.',
  },
  {
    id: 137,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'How do you analyse which third-party packages are contributing most to your Angular bundle size?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'What is the purpose of `isPlatformBrowser(this.platformId)` in an Angular component?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'What causes "Change Detection running N times per second" and how do you diagnose it?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'What does the `loading="lazy"` attribute on `<img>` do vs Angular\'s `NgOptimizedImage`?',
    options: [
  ...
  // ─── TYPESCRIPT 143-150 ──────────────────────────────────────────────────────
  {
    ...
    answer: 1,
    explanation:
      "`Partial<T>` maps every property of `T` to `T[K] | undefined` with `?`. Example: `Partial<{ name: string; age: number }>` becomes `{ name?: string; age?: number }`. Perfect for PATCH request payloads or update functions where you only supply changed fields. The claim that it only keeps the first half of User's properties is wrong — Partial retains every property, it just marks each one optional rather than dropping any of them. Removing all optional properties to leave only the required ones describes something closer to the inverse of `Required<T>`, not what `Partial` does. And treating it as roughly equivalent to `User | undefined` is wrong too — that would make the whole object nullable as a unit, not each property individually; `Partial` operates property-by-property, not on the object as a whole.",
  },
  {
    id: 144,
    type: 'multiple-choice',
    ...
    category: 'typescript',
    question:
      'What is the difference between `Pick<User, "name" | "email">` and `Omit<User, "password">`?',
    options: [
  ...
  },
  {
    ...
    category: 'typescript',
    question:
      'What does `type IsString<T> = T extends string ? "yes" : "no"` evaluate to for `IsString<"hello">` vs `IsString<number>`?',
    options: [
  ...
  },
  {
    ...
    category: 'typescript',
    question:
      'What does `type ReturnOf<T> = T extends (...args: never[]) => infer R ? R : never` do?',
    options: [
...
export class TimerComponent implements OnInit {
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'What does `spectator.detectChanges()` do differently from `fixture.detectChanges()` in Angular testing?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'Why does `ChangeDetectionStrategy.OnPush` significantly reduce unnecessary re-renders?',
    options: [
  ...
  },
  {
    ...
    explanation:
      "`[formGroup]=\"myForm\"` sets up the reactive forms context. It bridges the FormGroup model in the component class with the `<form>` element in the template. Child `formControlName=\"email\"` directives then look up their controls in `myForm`. Without `[formGroup]`, `formControlName` has no context to look controls up in and throws. Binding the form's submit event is wrong — that's what `(ngSubmit)` is for, a separate binding entirely. Importing the FormGroup class into the template is also wrong — `[formGroup]` doesn't import anything, it's a property binding to an existing FormGroup instance already created in the component class. And it doesn't automatically call `reset()` on submission — the value simply stays as the user left it unless the component explicitly resets it.",
  },
  {
    id: 168,
    type: 'predict-output',
...
const result = r === 'success' ? 'done' : r;`,
  ...
  },
  {
    ...
    category: 'components',
    question:
      'What is `provideExperimentalZonelessChangeDetection()` and what does enabling it require?',
    options: [
...
addItem(item: Item) {
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'What does the `trackBy` (or `track` in @for) function do for list rendering performance?',
    options: [
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'What problem does `resource()` solve compared to subscribing to an HTTP Observable in a constructor?',
    options: [
...
console.log(count());`,
    ...
    explanation:
      "`untracked(() => b())` reads b's CURRENT value but does NOT register b as a dependency. So `count` only recomputes when `a` changes. First read caches 1 + 10 = 11. `a.set(2)` invalidates count → it recomputes to 2 + untracked(b=10) = 12. `b.set(20)` is invisible to count (b is not a tracked dependency), so it never recomputes — `count()` returns the stale 12. This is the whole point of untracked: read a signal without subscribing to it. Why others fail: count never re-reads b reactively. untracked reads the live value, it does not freeze. a was updated to 2, not left at 1.",
  },
  {
    id: 202,
    type: 'multiple-choice',
  ...
  onCleanup(() => clearInterval(id));
    ...
    topicPath: 'signals-advanced',
    explanation:
      'The function you pass to `onCleanup` runs (1) immediately before the effect re-executes due to a dependency change, and (2) when the effect is torn down (its injection context is destroyed). This lets each run clean up after itself — clear timers, unsubscribe, abort fetches — so you never stack duplicate intervals or leak subscriptions. Why others fail: it also runs between re-runs, not only on final destroy. it does not stop the effect; the effect keeps reacting. side-effecting work (timers, listeners) absolutely needs cleanup.',
  },
  {
    id: 203,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'forms',
    question:
      'After reset(), value is `null` and breaks the string-typed consumer. Why, and how do you fix it?',
...
const upper: string = name.value.toUpperCase(); // 💥 runtime + type error`,
    ...
    topicPath: 'reactive-forms',
    explanation:
      "By default `new FormControl('Ada')` is typed `FormControl<string | null>` and `reset()` sets the value to `null` (not the initial value), so `name.value.toUpperCase()` throws at runtime and TypeScript flags the possible null. The fix is `new FormControl('Ada', { nonNullable: true })` — now the type is `FormControl<string>` and `reset()` returns to the initial `'Ada'` instead of null. Why others fail: there is no `clear()`; reset() is correct. values are nullable by default, which is the whole problem. passing `reset('')` works at runtime but the declared type stays nullable, so the type error remains.",
  },
  {
    id: 205,
    type: 'multiple-choice',
    ...
    topicPath: 'form-validation',
    explanation:
      "`updateOn` controls when the model syncs from the view. `'change'` (default) updates on every input event; `'blur'` updates only when the field loses focus; `'submit'` updates only when the parent form is submitted. Using `'blur'` is ideal for expensive async validators (e.g. \"is this email taken?\") because it fires one validation when the user leaves the field instead of one per keystroke. Why others fail: it does not disable anything. it is event-based, not a timer/debounce. it replaces change-based updating, it does not combine them.",
  },
  {
    id: 206,
    type: 'predict-output',
  ...
  scan((acc, n) => acc + n, 0)
    ...
    topicPath: 'rxjs-operators',
    explanation:
      '`scan` is "reduce that emits every step". Starting from seed 0: 0+1=1 (emit 1), 1+2=3 (emit 3), 3+6... 3+3=6 (emit 6). So it logs 1, 3, 6. Use `scan` for running totals / accumulating state over a stream. Why others fail: that is `reduce`, which emits only once on completion. the seed itself is not emitted — the first emission is seed+first value; prepend `startWith(0)` if you want the 0. scan transforms and accumulates; it does not pass values through.',
  },
  {
    id: 207,
    type: 'multiple-choice',
    ...
    topicPath: 'rxjs-advanced',
    explanation:
      'By default `shareReplay(1)` is NOT reference-counted: once it subscribes to the source it keeps that subscription open forever, so a hot source like `interval`/`fromEvent`/a WebSocket keeps running even after every consumer has unsubscribed — a leak. Passing `{ bufferSize: 1, refCount: true }` makes it drop the source subscription when the subscriber count hits zero (and re-subscribe when a new one arrives). Why others fail: bufferSize 1 already caps the buffer; the leak is the source subscription, not the buffer. sharing is exactly what it does; the issue is teardown. location is irrelevant to the refCount behavior.',
  },
  {
    id: 208,
    type: 'spot-the-bug',
  ...
  complete: () => console.log('done')
    ...
    topicPath: 'rxjs-operators',
    explanation:
      '`forkJoin` waits for every input observable to COMPLETE, then emits a single object of their LAST values. `interval(1000)` never completes, so forkJoin never emits `next` and never completes — the subscription just sits there. Use `combineLatest` if you want the latest values whenever any source emits, or make each source finite (e.g. `interval(1000).pipe(take(1))`). Why others fail: forkJoin needs completion, not just a first value. it never emits partial results. mixing finite/infinite is legal — it simply never fires.',
  },
  {
    id: 210,
    type: 'multiple-choice',
    ...
    category: 'components',
    question:
      'What does the transform enable here: `disabled = input(false, { transform: booleanAttribute })`?',
    options: [
    ...
    topicPath: 'inputs',
    explanation:
      'An input `transform` runs on every incoming value before it reaches the signal. `booleanAttribute` (built in, alongside `numberAttribute`) coerces the way native HTML boolean attributes do: a present-but-empty attribute (`<my-cmp disabled>`) and the string `"true"` become `true`; absence/`"false"` become `false`. This lets custom components accept `disabled` like real DOM elements. Why others fail: it coerces at runtime, not just type-checks. two-way binding is `model()`, not a transform. the transform runs on every value change, not once.',
  },
  {
    id: 211,
    type: 'spot-the-bug',
...
export class Card {
  ...
  constructor() {
    ...
    topicPath: 'view-queries',
    explanation:
      "Signal view queries are populated when Angular creates the component's view — which happens AFTER the constructor runs. So reading `this.box()` in the constructor returns `undefined`. Read it once the view exists: inside an `effect(() => { const el = this.box(); if (el) {...} })` (the effect re-runs when the query resolves), in `afterNextRender()`, or in `ngAfterViewInit`. Why others fail: viewChild finds elements, directives, and components. `.required()` throws if not found — it does not change WHEN the query resolves. `exportAs` is for directive instances; a template ref var does not need it.",
  },
  {
    id: 212,
    type: 'multiple-choice',
    ...
    topicPath: 'outputs',
    explanation:
      '`input()` is READ-ONLY inside the component. `model()` creates a WRITABLE signal AND, whenever you call `value.set(x)`/`value.update()`, Angular emits a matching `valueChange` output — which is exactly the contract `[(value)]="parentSignal"` relies on. It is the modern, signal-based replacement for the `@Input() x` + `@Output() xChange` pair. Why others fail: it does not add validation. use `model.required()` for required; plain `model()` is optional. it stays a signal, not an Observable.',
  },
  {
    id: 213,
    type: 'multiple-choice',
...
id = input.required<string>();   // ← bound from the :id param`,
    ...
    topicPath: 'route-params',
    explanation:
      '`withComponentInputBinding()` wires router data into `@Input()`/`input()` properties by name: a path param `:id`, a matching query param `?id=`, and resolver data keyed `id` all flow into an `id` input. It removes the boilerplate of injecting `ActivatedRoute` and subscribing to `paramMap` for simple cases. Why others fail: lazy loading is `loadComponent`/`loadChildren`, unrelated. there is no runtime type validation — params arrive as strings. it is one-way (URL → input), not two-way.',
  },
  {
    id: 214,
    type: 'multiple-choice',
    ...
    explanation:
      'Functional interceptors are plain functions of type `HttpInterceptorFn` — `(req, next) => next(modifiedReq)` — registered via `provideHttpClient(withInterceptors([authInterceptor, loggingInterceptor]))`. They run in array order, can use `inject()` inside for services, and are the standalone-first standard. Why others fail: the `HTTP_INTERCEPTORS` multi-provider is the OLD DI-based approach — still supported via `withInterceptorsFromDi()`, but not the modern default. there is no `@Interceptor()` decorator. interceptors are configured through `provideHttpClient`, not a bootstrap argument.',
  },
  {
    id: 215,
    type: 'predict-output',
...
const p = config.port;`,
    ...
    topicPath: 'ts-utility-types',
    explanation:
      '`satisfies` checks that the value is assignable to `Record<string, string | number>` but keeps the NARROW inferred type of the literal. So `config.port` stays `number` (and `config.host` stays `string`), giving you both validation and precise types. Contrast with a type ANNOTATION `const config: Record<string, string | number> = {...}`, which widens `config.port` to `string | number`. Why others fail: that is what an annotation would do, not satisfies. satisfies never produces `any`. object property inference widens `3000` to `number` (only `as const` keeps the literal `3000`).',
  },
  {
    id: 216,
    type: 'multiple-choice',
  ...
  | { kind: 'square'; side: number };`,
    ...
    topicPath: 'ts-narrowing',
    explanation:
      "A common literal discriminant (`kind`) lets TypeScript narrow the union inside `switch (shape.kind)` so each branch sees the right member type. Add a `default` that assigns the value to a `never`: `const _exhaustive: never = shape;`. If someone later adds `{ kind: 'triangle'; ... }` and forgets a case, `shape` is no longer `never` in the default branch and the assignment fails to compile — a free exhaustiveness check. Why others fail: readonly affects mutability, not narrowing. an interface cannot express a union of shapes. an enum tag works too, but the enum itself is not what enables exhaustiveness — the never-assignment is.",
  },
  {
    id: 217,
    type: 'multiple-choice',
    ...
    topicPath: 'ts-utility-types',
    explanation:
      '`Partial<T>` maps every property to optional (`{ [K in keyof T]?: T[K] }`) — ideal for patch/update payloads and `patchValue`-style APIs. Know the family: `Required<T>` makes all properties required (the inverse); `Readonly<T>` makes them immutable; `Pick<T, K>` keeps only keys K; `Omit<T, K>` removes keys K; `Record<K, V>` builds an object type from a key set. Why others fail: Required is the opposite. Readonly changes mutability, not optionality. Pick selects a subset of keys, it does not make them optional.',
  },
  {
    id: 218,
    type: 'spot-the-bug',
  ...
  expect(text).toContain('Loaded');  // fails: still "Loading..."
    ...
    topicPath: 'testing-components',
    explanation:
      'The first `detectChanges()` runs `ngOnInit`, which kicks off an async fetch — but the promise/HTTP response has not resolved by the next synchronous line, so the DOM still shows the loading state. Await stabilization, then re-render: `await fixture.whenStable(); fixture.detectChanges(); expect(...)`. Alternatives: `fakeAsync` + `tick()`/`flush()`, or flush a mock via `HttpTestingController`. Why others fail: calling detectChanges twice does not make time pass for the pending async task. the query method is irrelevant to the timing bug. starting async work in ngOnInit is normal and correct.',
  },
  {
    id: 219,
    type: 'multiple-choice',
    ...
    topicPath: 'testing-services-http',
    explanation:
      "The standalone-first pattern is `TestBed.configureTestingModule({ providers: [provideHttpClient(), provideHttpClientTesting()] })`. `provideHttpClientTesting()` swaps in a backend you drive with `HttpTestingController` — `httpMock.expectOne(url).flush(data)` to answer requests and `httpMock.verify()` in afterEach to assert none were left unhandled. Why others fail: hitting the real network in a unit test is flaky and slow. `HttpClientModule` + fetch spying is neither the module you'd use nor how HttpClient works under the hood. TestBed does not auto-mock HttpClient — you must provide the testing backend.",
  },
  {
    id: 220,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'In a zoneless app (`provideZonelessChangeDetection()`), what triggers change detection?',
    options: [
    ...
    topicPath: 'zoneless',
    explanation:
      'Without Zone.js, Angular no longer monkey-patches setTimeout/Promise/addEventListener to guess when to re-render. Instead, change detection is scheduled by explicit reactive signals: a signal used in a template changing, the async pipe emitting, a template `(event)` firing, or `ChangeDetectorRef.markForCheck()`. The practical consequence: a bare `setTimeout(() => this.plainField = x)` will NOT update the view unless it writes to a signal or calls markForCheck. Why others fail: the listed reactive primitives schedule CD for you. it is not limited to HTTP. the whole point is that arbitrary async callbacks no longer auto-trigger CD.',
  },
  {
    id: 221,
    type: 'multiple-choice',
    ...
    topicPath: 'hydration',
    explanation:
      'Without hydration, Angular throws away the server-rendered DOM on bootstrap and re-creates it, causing a visible flash and layout shift. `provideClientHydration()` turns on non-destructive hydration: Angular walks the existing server DOM, attaches event listeners and state to it, and skips re-rendering — better LCP/CLS and no flicker. Add `withIncrementalHydration()` to hydrate `@defer` blocks lazily (e.g. on interaction/viewport). Why others fail: that is the pre-hydration destructive behavior it eliminates. it does not disable client JS. it hydrates DOM; it is not an HTTP cache.',
  },
  {
    id: 222,
    type: 'predict-output',
...
// component: this.items.push(newItem);  // mutate in place`,
  ...
  // ─── BATCH 225-248: SECURITY, TESTING DEPTH, A11Y, JUNIOR ON-RAMPS ───────────
  {
    ...
    category: 'security',
    question:
      "A user's comment contains an onerror script. Does this render a working XSS payload?",
...
export class CommentView {
  comment = '<img src=x onerror="stealCookies()">';
    ...
    topicPath: 'security',
    explanation:
      "Binding to `[innerHTML]` runs the value through Angular's sanitizer in the HTML security context, which removes dangerous constructs — inline event handlers like `onerror`, `<script>`, `javascript:` URLs — before the markup reaches the DOM. The image is inserted without the handler, so `stealCookies()` never fires. The real danger is deliberately DISABLING this with `DomSanitizer.bypassSecurityTrustHtml(userInput)`. Why others fail: (A/C) sanitization already neutralizes the payload without manual escaping. the tag renders; only the unsafe attribute is stripped.",
  },
  {
    id: 226,
    type: 'multiple-choice',
    ...
    topicPath: 'security',
    explanation:
      'Interpolation writes to `textContent`, not `innerHTML`. Characters like `<` and `>` are escaped and displayed as literal text, so `<script>` shows up on screen as harmless characters rather than executing. This is why interpolation is safe by default — the danger only appears when you switch to `[innerHTML]` and bypass sanitization, or write to the DOM directly. Why others fail: no third-party library is involved; it is plain text rendering. tags are shown, not blocked. interpolation is fundamentally safer than raw innerHTML.',
  },
  {
    id: 227,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    topicPath: 'security',
    explanation:
      '`[href]`, `[src]`, and similar URL bindings are sanitized in the URL security context. Dangerous schemes like `javascript:` are prefixed with `unsafe:`, producing an inert `unsafe:javascript:alert(1)` that the browser will not execute. Safe schemes (`http`, `https`, `mailto`, `tel`, relative paths) pass through unchanged. Why others fail: the scheme is neutralized before it reaches the DOM. it degrades gracefully, it does not crash. URL bindings ARE sanitized by default.',
  },
  {
    id: 229,
    type: 'multiple-choice',
...
// template: <div [innerHTML]="safeBio"></div>`,
    ...
    topicPath: 'security',
    explanation:
      '`bypassSecurityTrustHtml` explicitly turns OFF Angular\'s sanitizer for that value. Feeding it user-entered content means any `<script>`/`onerror` an attacker saved in their bio runs for every viewer — classic stored XSS. The fix is to NOT bypass: bind `[innerHTML]="userBio"` so Angular sanitizes it, or if you must allow rich text, sanitize on the server with a strict tag/attribute allowlist. Why others fail: `[innerHTML]` with sanitization is safe; bypassing is the actual bug. signal vs field is irrelevant to the vulnerability. bypassing does the opposite of sanitizing.',
  },
  {
    id: 231,
    type: 'multiple-choice',
    ...
    topicPath: 'security',
    explanation:
      'Assigning to `nativeElement.innerHTML` writes raw HTML straight to the DOM with NO sanitization, so any embedded user data becomes an XSS vector. It also assumes a real DOM exists, which breaks server-side rendering and other platforms. Prefer `[innerHTML]`/`[textContent]` bindings (sanitized) or `Renderer2` methods, which are platform-agnostic. Why others fail: performance is not the concern. ElementRef is not deprecated, just to be used carefully. the direct property assignment is NOT sanitized — only the `[innerHTML]` binding is.',
  },
  {
    id: 232,
    type: 'multiple-choice',
    ...
    topicPath: 'testing-components',
    explanation:
      '`fakeAsync` gives you a virtual clock. `tick(ms)` moves it forward by a precise amount and runs any timers/microtasks scheduled up to that point — use it to assert intermediate timing (e.g. a 300ms debounce). `flush()` repeatedly advances the clock until no timer macrotasks remain, settling everything at once — use it when you just want the async work to finish. `flushMicrotasks()` drains pending Promises without advancing timers. Why others fail: they behave differently. tick drains microtasks too. flush has no fixed duration.',
  },
  {
    id: 233,
    type: 'spot-the-bug',
    ...
    category: 'testing',
    question:
      'This fakeAsync test throws "1 periodic timer(s) still in the queue". Why, and how do you fix it?',
  ...
  expect(component.count).toBe(1);
    ...
    topicPath: 'testing-components',
    explanation:
      '`fakeAsync` asserts a clean timer queue at the end of the test to catch leaks. `setInterval` schedules a recurring (periodic) task that never completes on its own, so it is still queued when the test finishes → the error. Fix by tearing down the interval the way production would (`component.stop()` / `ngOnDestroy`), or explicitly call `discardPeriodicTasks()` to drop pending periodic timers before returning. Why others fail: `flush()` also leaves the periodic timer re-scheduling, so it still throws. fakeAsync works fine with intervals when you clean them up. where the interval is created does not change the pending-queue check.',
  },
  {
    id: 234,
    type: 'multiple-choice',
    ...
    topicPath: 'testing-services-http',
    explanation:
      "`TestScheduler` runs your stream against a VIRTUAL time frame described by marble strings: `-` is a frame of time, letters are emissions, `|` is complete, `#` is error. Inside `scheduler.run(({ cold, hot, expectObservable }) => ...)` you build inputs and assert the exact emission timing of `debounceTime`, `delay`, `retry`, etc. — all synchronously and deterministically, with no real clock. Why others fail: it does not use threads. HTTP mocking is HttpTestingController's job. it targets Observables and virtual time, not just Promises.",
  },
  {
    id: 235,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'What is the advantage of a Component Harness over `fixture.nativeElement.querySelector`?',
...
await button.click();`,
    ...
    topicPath: 'testing-components',
    explanation:
      "A harness wraps a component behind an intent-based API. Your test says `button.click()` / `await input.setValue('x')` instead of querying `.mat-button` and dispatching events. When the internal DOM structure or CSS classes change, only the harness implementation updates — the tests stay green because they assert behavior, not markup. Angular Material ships harnesses for every component; you can build your own by extending `ComponentHarness`. Why others fail: speed is comparable. harnesses work for any component, not just forms. harness methods trigger change detection internally but the concept is about stable, semantic queries.",
  },
  {
    id: 236,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'Why obtain a service with `TestBed.inject(UserService)` rather than `new UserService(...)` in a test?',
    options: [
    ...
    topicPath: 'testing-services-http',
    explanation:
      '`TestBed.inject` goes through the configured injector, so the service is constructed with its real (or overridden/mocked) dependencies and you get the SAME singleton the components in that TestBed use. `new UserService(...)` forces you to manually build every constructor dependency, ignores any `{ provide, useValue }` mocks you set up, and produces an instance disconnected from the component tree. Why others fail: `new` is legal, just usually wrong here. inject returns the DI singleton, not a fresh instance each call. they differ in dependency wiring and override behavior.',
  },
  {
    id: 237,
    type: 'multiple-choice',
    ...
    topicPath: 'testing-services-http',
    explanation:
      'Provider overriding is the DI-native way to inject test doubles. In `configureTestingModule({ providers: [{ provide: UserService, useValue: fake }] })` (or `useClass: MockUserService`), any component/service that injects `UserService` gets the fake — including nested dependencies you never touch directly. For already-built modules use `TestBed.overrideProvider(UserService, { useValue: fake })` before creating the component. Why others fail: reassigning a private field is brittle and misses nested injections. importing does not change what DI resolves. test doubles are the standard practice.',
  },
  {
    id: 238,
    type: 'spot-the-bug',
  ...
  },
  {
    ...
    topicPath: 'a11y',
    explanation:
      'A button with only an icon has no text for a screen reader to announce, so it reads as an unlabeled button. Provide an accessible name with `aria-label` (or a visually-hidden `<span>` with real text), and hide the purely decorative glyph from assistive tech with `aria-hidden="true"` on the icon. Why others fail: `title` on the SVG is unreliable across screen readers and not a substitute for an accessible name. a div wrapper adds nothing and loses native button semantics. screen readers do not narrate SVG path data.',
  },
  {
    id: 240,
    type: 'multiple-choice',
    ...
    topicPath: 'a11y',
    explanation:
      'An accessible modal takes focus on open, confines Tab/Shift+Tab to elements inside it so keyboard and screen-reader users cannot wander into the inert background, and returns focus to the element that opened it when it closes. Semantically it needs `role="dialog"` with `aria-modal="true"` and a label (`aria-labelledby`/`aria-label`). Angular CDK\'s `a11y` module (`cdkTrapFocus` / `FocusTrap`, `FocusMonitor`) implements this. Why others fail: a backdrop is visual only, not a focus boundary. disabling the keyboard breaks all keyboard users. a plain `<div>` gets none of this for free.',
  },
  {
    id: 241,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    topicPath: 'a11y',
    explanation:
      'The OS-level `prefers-reduced-motion` setting signals users who experience discomfort from motion. Respect it by wrapping non-essential animation CSS in `@media (prefers-reduced-motion: no-preference)` (so it only runs when motion is welcome), or by disabling large parallax/slide/zoom effects in Angular when the query matches. Keep essential feedback but drop gratuitous motion. Why others fail: shorter is still motion. motion sensitivity is not platform-specific. a custom toggle ignores the standard OS preference users already set.',
  },
  {
    id: 243,
    type: 'spot-the-bug',
...
export class Counter {
  count = signal(0);
    ...
    topicPath: 'signals',
    explanation:
      'Signals are accessed by CALLING them: `count()` returns the current value and registers the template as a reader so it updates on change. `{{ count }}` interpolates the signal function object itself (Angular stringifies it), not the number. The fix is `{{ count() }}`. This "forgot the parentheses" mistake is the most common signals gotcha. Why others fail: `signal(0)` is the correct creation syntax. signals render without CommonModule. readonly affects writability, not whether it displays.',
  },
  {
    id: 245,
    type: 'spot-the-bug',
...
export class NameField {
  name = '';
    ...
    explanation:
      "`ngModel` is provided by `FormsModule`. In a standalone component you must add it to `imports: [FormsModule]`; otherwise Angular does not recognize the `ngModel` directive and reports \"Can't bind to 'ngModel' since it isn't a known property of 'input'\". Why others fail: template-driven `ngModel` is fully supported, just not imported here. `ngModel` works with a plain string property. `[(ngModel)]` is valid banana-in-a-box sugar; splitting it does not fix the missing import.",
  },
  {
    id: 246,
    type: 'spot-the-bug',
  ...
  <li>{{ item.name }}</li>
    ...
    topicPath: 'control-flow-for',
    explanation:
      'Unlike the old `*ngFor`, the built-in `@for` block makes the track expression MANDATORY: `@for (item of items; track item.id) { … }`. `track` tells Angular how to identify each item so it can reuse DOM nodes efficiently across updates; omitting it is a compile-time error. Use a stable unique id when available, or `track item` / `track $index` for primitive lists. Why others fail: `@for` is the modern replacement for `*ngFor`, not the other way around. `@for` iterates any iterable, not only signals. `@empty` is optional; the missing `track` is the error.',
  },
  {
    id: 247,
    type: 'multiple-choice',
    ...
    topicPath: 'property-binding',
    explanation:
      '`prop="{{ expr }}"` (attribute interpolation) coerces the result to a STRING, which is fine for text but wrong for booleans, numbers, and objects. Property binding `[prop]="expr"` passes the real typed value to the DOM property or `@Input()`. Classic trap: `[disabled]="isOpen"` sets the boolean, whereas `disabled="{{ isOpen }}"` always yields a non-empty string that keeps the control disabled. For plain string values either form works. Why others fail: they diverge for non-string types. the rule applies to native elements too (disabled, hidden, value). it is unrelated to control-flow blocks.',
  },
  {
    id: 248,
    type: 'spot-the-bug',
...
export class Profile {
  ...
  user$ = this.http.get<{ name: string }>('/api/me');
    ...
    topicPath: 'rxjs-observables',
    explanation:
      '`http.get(...)` returns an Observable, and interpolating the Observable object itself just stringifies it (`[object Object]`). Use the `async` pipe to subscribe, unwrap the emitted value, and auto-unsubscribe on destroy: `{{ (user$ | async)?.name }}` — or assign it with `@let user = user$ | async`. Why others fail: HttpClient returns an Observable, not a Promise, and templates cannot await. the request is fine; the display is the bug. plain Observables have no synchronous `.value` (that is a BehaviorSubject).',
  },
  ...
  // ─── BATCH 249-272: STATE/ARCHITECTURE, RXJS & SIGNAL OUTPUTS, I18N, ANIMATION ─
  {
    ...
    explanation:
      '`signalStore()` builds a store by composing feature functions: `withState({...})` declares reactive state as signals, `withComputed(store => ({...}))` adds memoized derived signals, `withMethods(store => ({...}))` defines updaters and rxMethod-based effects, and `withHooks` runs onInit/onDestroy logic. The result is provided via DI (`providedIn` or a route/component provider) and gives fine-grained signal reactivity without the actions → reducers → effects → selectors ceremony of the classic Store. Why others fail: it is a standalone store, not an action/reducer adapter. persistence is a separate concern. it uses DI, it does not replace it.',
  },
  {
    id: 250,
    type: 'multiple-choice',
    ...
    topicPath: 'state-management',
    explanation:
      'NgRx enforces unidirectional data flow: an **action** is a plain event object (`{ type, payload }`); a **reducer** is a pure `(state, action) => newState` function with no side effects; a **selector** derives and memoizes a view of state for components; an **effect** listens to actions, performs impure work (HTTP, router, storage), and emits follow-up actions. Why others fail: it scrambles every responsibility. the layers have strict, distinct roles. reducers must be pure and selectors are read-only.',
  },
  {
    id: 251,
    type: 'multiple-choice',
    ...
    topicPath: 'state-management',
    explanation:
      'The container/presentational split keeps most components pure: presentational components take everything through `input()`/`@Input()`, report user intent through `output()`/`@Output()`, and avoid injecting services or touching global state — making them trivial to reuse and test with plain inputs. Smart/container components sit above them, inject stores/services, orchestrate state, and pass data down. Why others fail: that describes a smart/container component. presentational components absolutely have templates — that is their job. OnPush is a common pairing but not the defining trait.',
  },
  {
    id: 252,
    type: 'spot-the-bug',
  ...
  },
  {
    ...
    topicPath: 'state-management',
    explanation:
      '`createSelector(inputA, inputB, (a, b) => derive(a, b))` caches its last inputs and output. If the input selectors return the same references, it skips the projector and hands back the memoized value — so expensive derivations (filtering, sorting, joining slices) run only when their inputs actually change, and components bound to the selector do not re-render needlessly. Why others fail: it runs on the main thread. selectors CAN be plain functions; createSelector adds memoization. it does not persist anything.',
  },
  {
    id: 254,
    type: 'multiple-choice',
    ...
    topicPath: 'state-management',
    explanation:
      'A reducer is a pure function of `(state, action)`; putting an HTTP call in it would make it non-deterministic and un-testable and would break time-travel. Effects are the sanctioned home for impurity: an effect is an Observable that filters for a trigger action (`ofType`), does the async work, and dispatches result actions (`loadSuccess`/`loadFailure`) which reducers then handle purely. Why others fail: performance is not the reason. effects do NOT mutate state — they dispatch actions; only reducers change state. effects and reducers are complementary, not substitutes.',
  },
  {
    id: 255,
    type: 'multiple-choice',
    ...
    topicPath: 'state-management',
    explanation:
      'Choose by complexity, not fashion. Signal stores (a service with private writable signals + public readonly/computed + methods, or `@ngrx/signals`) cover most local and feature-level state with minimal ceremony and excellent ergonomics. The full NgRx Store earns its overhead when you need an auditable event log, time-travel debugging, elaborate coordinated effects, or a consistent structure imposed across a big team. Why others fail: NgRx Store is not deprecated. a root-provided signal service holds shared state fine. HTTP presence is orthogonal — you can do HTTP with either.',
  },
  {
    id: 256,
    type: 'spot-the-bug',
...
b$.next('b2');`,
    ...
    topicPath: 'rxjs-operators',
    explanation:
      "`combineLatest` emits once BOTH sources have produced a value, then re-emits whenever ANY source emits, always pairing the latest values. BehaviorSubjects start with values, so the initial emission is `a1b1`. `a$.next('a2')` → `a2b1`. `b$.next('b2')` → `a2b2`. Total: three emissions. Why others fail: it emits on every change, not only the last. it does not wait for simultaneous changes. it stays subscribed and keeps emitting; it does not complete after one.",
  },
  {
    id: 258,
    type: 'predict-output',
  ...
  concatMap(n => of(n).pipe(delay(n === 1 ? 30 : 0)))
    ...
    topicPath: 'rxjs-advanced',
    explanation:
      '`concatMap` processes inner observables one at a time IN ORDER: it fully subscribes to the inner observable for 1 (waiting the 30ms delay) before it even starts the inner observable for 2, then 3. So the output is strictly `1, 2, 3` regardless of per-item delays. Contrast `mergeMap`, which subscribes to all inners concurrently — there 2 and 3 (0ms) would emit before 1 (30ms), giving `2, 3, 1`. Why others fail: (A/C) that is mergeMap-style concurrency, not concatMap. the 30ms delay is honored; concatMap waits for it.',
  },
  {
    id: 259,
    type: 'predict-output',
  ...
  distinctUntilChanged()
    ...
    topicPath: 'rxjs-operators',
    explanation:
      '`distinctUntilChanged` compares each value to the PREVIOUS emitted one and drops it only if they are equal. So the runs `1,1` → `1`, `2,2` → `2`, then `3`, then a `1` again — which is NOT equal to the immediately preceding `3`, so it passes. Result: `1, 2, 3, 1`. To remove ALL duplicates ever seen (globally), you would need `distinct()` instead. Why others fail: that is `distinct()`, not distinctUntilChanged. consecutive dupes ARE removed. it filters, it does not stop the stream.',
  },
  {
    id: 260,
    type: 'predict-output',
  ...
  takeWhile(n => n % 2 === 0)
    ...
    topicPath: 'rxjs-operators',
    explanation:
      '`takeWhile(pred)` emits values while the predicate holds and COMPLETES as soon as one fails — it does not skip and continue like `filter`. At `7` the predicate is false, so the stream completes right there and `7` (and everything after) is not emitted: output `2, 4, 6`. Pass `takeWhile(pred, true)` (inclusive) if you want the failing value emitted before completing. Why others fail: that is `filter` behavior. the default is exclusive, so 7 is not emitted. it takes all leading passing values, not just the first.',
  },
  {
    id: 261,
    type: 'predict-output',
...
console.log(c(), runs);   // (2)`,
    ...
    topicPath: 'signals',
    explanation:
      'A `computed` does not execute until it is READ. So after two `a.set(...)` calls with no read in between, `runs` is still `0` (log 1). The first `c()` triggers a single computation using the latest value `a() = 3` → returns `6` and increments `runs` to `1` (log 2 prints `6 1`). This is laziness + memoization: intermediate values are never computed, and the result is cached until a dependency changes. Why others fail: it does not recompute per set. it computed once, not per set. computed is lazy, not eager at creation.',
  },
  {
    id: 264,
    type: 'predict-output',
...
// ...synchronous code finishes, then the scheduler flushes`,
    ...
    topicPath: 'signals-advanced',
    explanation:
      'An `effect` does not run synchronously at creation — its first execution is SCHEDULED. Because `a.set(1)` and `a.set(2)` both happen synchronously before the scheduler flushes, the writes coalesce and the effect runs a single time, reading the latest value `a() = 2`, logging `run 2`. This "glitch-free, coalesced" model means you never see intermediate values `0` or `1`. Why others fail: writes are batched, not one-run-per-write. there is no separate initial `run 0` because the flush happens after both sets. effects are driven by their own scheduler, not manual change detection.',
  },
  {
    id: 265,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    topicPath: 'i18n',
    explanation:
      'Pluralization rules differ wildly between languages (some have separate forms for 0, 1, 2, few, many). Angular supports ICU MessageFormat inside i18n text: `{ count, plural, =0 {...} =1 {...} other {...} }`, and a sibling `{ gender, select, male {...} female {...} other {...} }` for enumerations. Translators can then supply the plural categories their language needs. Why others fail: hand-rolled `=== 1` logic only works for English-like languages and breaks for others. manual string selection reimplements ICU badly and is not translator-friendly. using one form is grammatically wrong in most locales.',
  },
  {
    id: 267,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    explanation:
      "Angular's animation DSL composes: `trigger('name', [...])` names an animation and is attached in the template via `[@name]=\"expr\"`; `state('open', style({...}))` declares the styles for a named state; and `transition('open <=> closed', animate('200ms ease'))` describes the timed change between states (optionally with `keyframes`, `group`, `query`, `stagger`). Requires `provideAnimationsAsync()` (or the animations provider). Why others fail: it is a full state machine, not a class toggle. a lone `animate()` has no trigger/state binding. keyframes live inside the trigger DSL, not (only) global CSS.",
  },
  {
    id: 270,
    type: 'multiple-choice',
    ...
    topicPath: 'animations',
    explanation:
      "When an element is added to or removed from the DOM (via `@if`, `@for`, `*ngIf`, etc.), Angular represents those with the special `void` state. `:enter` is shorthand for `transition(':enter', ...)` = `void => *` (appearing), and `:leave` = `* => void` (disappearing). Angular keeps a leaving element in the DOM until its `:leave` animation finishes. Wrapping a list in a trigger and using `query(':enter', stagger(50, animate(...)))` animates children in sequence. Why others fail: those are `mouseenter`/`mouseleave` DOM events, unrelated. enter/leave apply to any element insertion/removal, not just routes. focus/blur are different events.",
  },
  {
    id: 271,
    type: 'multiple-choice',
    ...
    explanation:
      '`withViewTransitions()` hooks the router into the native View Transitions API: on navigation Angular calls `document.startViewTransition()` so the browser snapshots the old and new DOM and cross-fades (or morphs shared elements via `view-transition-name`) between them. It is a progressive enhancement — where the API is unsupported the navigation just happens instantly — and you customize the animation with `::view-transition-old/new` CSS pseudo-elements. Why others fail: preloading is a separate feature (`withPreloading`). SSR is `provideServerRendering`, unrelated. it animates the transition, it is not a spinner.',
  },
  {
    id: 272,
    type: 'multiple-choice',
    ...
    explanation:
      'Browsers run a pipeline of layout → paint → composite. `transform` and `opacity` can be handled purely in the COMPOSITE step on the GPU, so they animate at 60fps without recalculating geometry. Animating layout-affecting properties (`width`, `height`, `top`, `left`, `margin`) forces the browser to re-run layout and paint on every frame, which drops frames on complex pages. Prefer `transform: translate()/scale()` over positional/size changes, and hint with `will-change` sparingly. Why others fail: width/height trigger layout — the opposite of smooth. margin/padding also trigger layout. property choice is one of the biggest factors in animation smoothness.',
  },
  {
    id: 273,
    type: 'multiple-choice',
    ...
    explanation:
      'A STRUCTURAL directive changes DOM structure — it is desugared from the `*` shorthand (`*ngIf="x"`) into an `<ng-template>` and works by injecting `TemplateRef` and `ViewContainerRef` and calling `createEmbeddedView`/`clear`. An ATTRIBUTE directive has no `*`, sits on an existing element like a normal attribute (`[ngClass]`, `[ngStyle]`, or your own `appHighlight`), and alters that element via `ElementRef`/`Renderer2` or host bindings. Why others fail: reverses the two definitions. the `*` is meaningful sugar, not optional. attribute directives apply to any element.',
  },
  {
    id: 274,
    type: 'multiple-choice',
    ...
    explanation:
      "`@HostBinding('class.active') isActive = true` mirrors a field onto a DOM class/attr/style/property of the HOST, and `@HostListener('mouseenter') onEnter() {}` binds a host (or window/document) event to a method. A classic highlight directive combines them: listen for `mouseenter`/`mouseleave` and toggle a `@HostBinding('style.background')`. The modern equivalent is the `host: {}` metadata object. Why others fail: neither performs DI. both work in `@Directive` — that is their primary home. the host is implicit, so no `ViewChild` is needed.",
  },
  {
    id: 275,
    type: 'spot-the-bug',
    ...
    category: 'components',
    question:
      'This custom structural directive delays showing content. What bug appears if `appDelay` is re-set (bound to a changing value)?',
...
// used as: <div *appDelay="delay">Hi</div>`,
  ...
  },
  {
    ...
    topicPath: 'structural-directives',
    explanation:
      'The `*` prefix is shorthand: Angular lifts the host element into an `<ng-template>` and moves the directive onto it. `*ngIf="user as u"` becomes roughly `<ng-template [ngIf]="user" let-u="ngIf"><div>{{ u.name }}</div></ng-template>`. The `as` clause captures the directive exported context value into a template input variable — which is why the element is NOT rendered (and `u` does not exist) when the condition is falsy. Why others fail: `ngIf` adds/removes from the DOM rather than hiding with CSS (that is `[hidden]`). it is a runtime template, not a compile-time ternary. no component merging occurs.',
  },
  {
    id: 277,
    type: 'multiple-choice',
    ...
    topicPath: 'structural-directives',
    explanation:
      'Each `*` structural directive claims the element by lifting it into its own `<ng-template>`; two on the same element would need competing templates, so the compiler forbids it. The classic pre-v17 fix is `<ng-container *ngIf="show"><li *ngFor="let x of items">…</li></ng-container>` (a grouping element that renders no extra DOM). Modern control flow sidesteps it entirely because `@if`/`@for` are block syntax that nests: `@if (show) { @for (x of items; track x.id) { <li>…</li> } }`. Why others fail: ordering is not the issue. both still exist, though `@if`/`@for` are now preferred. importing does not lift the one-structural-directive rule.',
  },
  {
    id: 278,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      "What is the difference between `@Injectable({ providedIn: 'root' })` and listing a service in a component's `providers: [Svc]`?",
    options: [
    ...
    topicPath: 'di-providers',
    explanation:
      "`providedIn: 'root'` puts the provider in the root injector, so the whole app shares a single lazily-created instance, and if nothing injects it the class is tree-shaken away. Declaring `providers: [Svc]` on a component creates a new instance in that component element injector — a distinct copy handed to the component and every descendant, overriding the root instance for that subtree (useful for per-feature or per-dialog state). Why others fail: their scoping differs fundamentally. root is a singleton, not per-component. reversed — root is app-wide, component providers are local.",
  },
  {
    id: 279,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      "Why use `new InjectionToken<Config>('app.config')` instead of a string or interface as a DI key?",
    options: [
    ...
    topicPath: 'di-providers',
    explanation:
      "Angular DI keys must exist at runtime. Classes qualify because they are runtime values, but a TypeScript `interface`/`type` is erased during compilation and cannot be a key. `new InjectionToken<Config>('desc')` creates a unique, collision-proof token that also carries `<Config>` so `inject(APP_CONFIG)` is typed. Tokens also support `{ multi: true }` to gather many providers into an array (as `HTTP_INTERCEPTORS` does). Why others fail: performance is not the reason. tokens hold any value, including objects and functions. a token adds no observability.",
  },
  {
    id: 280,
    type: 'multiple-choice',
    ...
    explanation:
      'Angular resolves a dependency by walking up the injector tree, and the resolution modifiers change where that walk starts and stops: `@Self()` looks ONLY in the current (element) injector and throws if the token is absent there; `@SkipSelf()` ignores the current injector and begins at its parent; `@Optional()` returns `null` instead of throwing; `@Host()` stops the search at the host component boundary. They compose — `@Optional() @SkipSelf()` is the classic guard against a directive resolving a token from itself. Why others fail: singleton-ness depends on where you provide, not these flags. unrelated to change detection. they are not access modifiers.',
  },
  {
    id: 281,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      'What advantage does the `inject()` function have over constructor parameter injection?',
    options: [
    ...
    topicPath: 'services-di',
    explanation:
      '`inject(Foo)` reads from the current injection context instead of a constructor signature, so it works in property initializers (`private http = inject(HttpClient)`), in functional guards/resolvers/interceptors, and in shared functions — and it removes the `super(...)` ceremony when extending a base class. The catch: it must be called in an injection context (constructor, field initializer, or `runInInjectionContext`), otherwise it throws `NG0203`. Why others fail: constructor injection still works. it uses the same injector, no bypass. you still pass `{ optional: true }` for optional dependencies.',
  },
  {
    id: 282,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      'How does `{ provide: Logger, useExisting: BetterLogger }` differ from `useClass: BetterLogger`?',
    options: [
    ...
    topicPath: 'di-providers',
    explanation:
      '`useExisting` constructs nothing — it points one token at another provider so both resolve to the identical instance. `{ provide: Logger, useClass: BetterLogger }` instead instantiates a distinct `BetterLogger` for the `Logger` token, so you would have two objects if `BetterLogger` is also provided directly. The provider recipes are: `useClass` (instantiate a class), `useValue` (a ready object/constant), `useExisting` (alias to another token), and `useFactory` (call a function, optionally with `deps`). Why others fail: they differ in instance sharing. it reuses, never news up. it can alias any token type.',
  },
  {
    id: 283,
    type: 'multiple-choice',
    ...
    topicPath: 'ssr',
    explanation:
      'Without hydration a server-rendered app throws away the server DOM on bootstrap and rebuilds it, causing a visible flash. `provideClientHydration()` enables non-destructive hydration: Angular matches the server markup to the component tree, reuses the existing nodes, and wires up event listeners and bindings in place — improving Core Web Vitals (no re-paint, lower CLS) and killing the flicker. Why others fail: that is precisely the non-hydrated behavior it fixes. it enables interactivity, not disables it. Angular has no such router mode — that is a Next.js notion.',
  },
  {
    id: 284,
    type: 'multiple-choice',
...
export class WidthComponent {
  width = window.innerWidth;   // read at construction
    ...
    topicPath: 'ssr',
    explanation:
      'Server-side rendering runs your component code in Node, where browser globals (`window`, `document`, `navigator`, `localStorage`) do not exist — referencing them during construction or `ngOnInit` throws and breaks the render. Safe patterns: gate the code with `isPlatformBrowser(inject(PLATFORM_ID))`, move it into `afterNextRender`/`afterRender` (which never execute on the server), or use Angular abstractions (`DOCUMENT` token, `Renderer2`) instead of raw globals. Why others fail: it is undefined on the server, not 0. `innerWidth` is a property, not a method. the template is fine — the crash is the global access.',
  },
  {
    id: 286,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'What problem does the HttpClient transfer cache (part of `provideClientHydration()`) solve in SSR?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'ssr',
    explanation:
      'SSR (Angular Universal, now built into `@angular/ssr`) renders the initial view to HTML on the server so the browser paints meaningful content immediately — improving perceived load (FCP/LCP) and giving crawlers and link-unfurlers fully rendered markup for SEO. The trade-offs: you run a Node server (or prerender), and your code must be SSR-safe (no unguarded browser globals). Hydration then makes that server HTML interactive without a re-render. Why others fail: the client still downloads and runs Angular to become interactive. change detection still runs in the browser. SSR is not a service worker — offline needs a PWA/caching strategy.',
  },
  {
    id: 288,
    type: 'predict-output',
  ...
  catchError(() => of(99)),
    ...
    topicPath: 'rxjs-operators',
    explanation:
      'An RxJS error is terminal: when `map` throws on `2`, the source errors and stops — values after the failure (`3`) are never emitted. `catchError` intercepts the error and switches to the returned observable (`of(99)`), which emits `99` then completes. So the output is `1, 99`. To keep processing later values you must move the try/handling INSIDE a per-item inner observable (e.g. `mergeMap(x => of(x).pipe(map(...), catchError(...)))`) so one failure does not kill the outer stream. Why others fail: ignores the throw. `3` cannot appear — the source already errored. `1` is emitted before the error.',
  },
  {
    id: 289,
    type: 'multiple-choice',
    ...
    topicPath: 'rxjs-advanced',
    explanation:
      '`retry` responds to an error by unsubscribing and re-subscribing to the SOURCE — for an HttpClient call that fires the request again. The object form `{ count, delay }` bounds the attempts and inserts a wait (a number for fixed backoff, or a function returning an observable for exponential/conditional backoff). After the final failed attempt the error passes through to the consumer. Why others fail: it does not cache. retries are sequential, not parallel. `retry` retries ALL errors unless your `delay` callback filters by status.',
  },
  {
    id: 290,
    type: 'multiple-choice',
    ...
    topicPath: 'rxjs-advanced',
    explanation:
      'A Scheduler is a policy for dispatching work: it decides the clock and context in which an observable emits. `observeOn(asyncScheduler)` pushes emissions onto the macrotask queue (setTimeout-like), `asapScheduler` onto the microtask queue, `queueScheduler` runs synchronously in a trampoline, and `animationFrameScheduler` aligns with `requestAnimationFrame`. Operators like `delay`, `throttleTime`, and `interval` accept a scheduler, and `TestScheduler` uses virtual time so marble tests run instantly and deterministically. Why others fail: it schedules delivery, not operator selection. most observables run synchronously with no explicit scheduler. it governs timing broadly, not just errors.',
  },
  {
    id: 291,
    type: 'spot-the-bug',
  ...
  error: () => this.showError(),   // never called
    ...
    topicPath: 'rxjs-advanced',
    explanation:
      '`catchError` REPLACES the errored stream with whatever observable you return. Returning `EMPTY` yields a stream that emits nothing and completes normally, so downstream the error is gone — `next` and `error` both stay silent and the observable just completes. If the UI must react to failure, either handle it inside `catchError` (call `this.showError()` there and return a fallback value) or re-throw with `return throwError(() => err)` so the subscriber error callback still runs. Why others fail: `EMPTY` completes, it does not throw. the object-form observer is valid. HTTP errors are ordinary RxJS errors and are catchable.',
  },
  {
    id: 292,
    type: 'multiple-choice',
    ...
    topicPath: 'rxjs-operators',
    explanation:
      '`finalize` registers teardown that fires once the subscription ends by ANY path: normal completion, an error, or the consumer unsubscribing (including via `takeUntil` or an async pipe tearing down). That makes it ideal for `finalize(() => this.loading.set(false))` so a spinner clears whether the call succeeded or failed. Note it runs AFTER the error/complete notification propagates. Why others fail: it runs on teardown, not per value. it runs at the end, not before emissions. it also runs on error and unsubscribe — that is its entire purpose.',
  },
  {
    id: 293,
    type: 'multiple-choice',
    ...
    topicPath: 'ts-utility-types',
    explanation:
      "`Omit<T, K>` builds a type from `T` minus the keys in `K` (it is defined as `Pick<T, Exclude<keyof T, K>>`). `Omit<User, 'password'>` is `User` without the `password` field — handy for API responses or form models that must not carry a secret. Its complement `Pick<T, K>` keeps only the listed keys. Both are compile-time only. Why others fail: making one field optional is `Partial`-like, not removal. that is `Pick<User, 'password'>`. types are erased — no runtime object is created.",
  },
  {
    id: 294,
    type: 'multiple-choice',
    ...
    explanation:
      "`K extends keyof T` constrains `key` to the union of `T`'s property names, and the indexed access type `T[K]` returns exactly the type stored at that key. So for `user: { name: string; age: number }`, `get(user, 'age')` is `number`, `get(user, 'name')` is `string`, and `get(user, 'missing')` fails to compile — full type safety with no casts or `any`. Why others fail: arbitrary strings are rejected by the constraint. the return is precisely typed, not `any`. `K` is a key of `T`, not equal to `T`.",
  },
  {
    id: 295,
    type: 'multiple-choice',
    ...
    category: 'typescript',
    question:
      "What makes a discriminated (tagged) union like `{ kind: 'circle'; r: number } | { kind: 'square'; side: number }` powerful?",
    options: [
    ...
    topicPath: 'ts-narrowing',
    explanation:
      "A discriminated union pairs each member with a common singleton-typed field (the discriminant, here `kind`). When you branch on that field, TypeScript narrows to the matching member — inside `case 'circle'` only `r` is accessible, inside `case 'square'` only `side`. Assigning the value in the `default` branch to `const _exhaustive: never = shape` turns a forgotten case into a compile error, so adding a new shape forces you to handle it. Why others fail: the members stay separate, narrowed by the tag. plain object types suffice — no classes needed. it strengthens checking, not disables it.",
  },
  {
    id: 296,
    type: 'multiple-choice',
    ...
    category: 'typescript',
    question:
      'What does a mapped type with template-literal key remapping, like `{ [K in keyof T as \`on\${Capitalize<string & K>}\`]: () => void }`, generate?',
    options: [
    ...
    explanation:
      'A mapped type `{ [K in keyof T]: … }` iterates every key of `T`; the optional `as` clause REMAPS each key to a new one, and template literal types build that new key from string pieces plus helpers like `Capitalize`. So the shown type turns `{ name: string; age: number }` into `{ onName: () => void; onAge: () => void }`. These are pure type-level computations — ideal for deriving handler or getter types from a model. Why others fail: it is a compile-time type, not a runtime object. it maps any object type, not just arrays. no decorator or runtime is involved.',
  },
  {
    id: 297,
    type: 'multiple-choice',
    ...
    topicPath: 'a11y',
    explanation:
      '`alt` communicates what a sighted user gets from the image: a product photo needs a description, an icon that duplicates adjacent text or a decorative flourish should carry `alt=""` — the EXPLICIT empty value tells assistive tech "intentionally skippable". Leaving `alt` off is worse than empty: many screen readers then read the src file name ("IMG-2024-final-v3.png"). Keep alt concise and purposeful, not exhaustive. Why others fail: a missing attribute is not skipped — it degrades to the file name. alt should convey purpose briefly, not pixel-level detail. alt affects only assistive tech; the image still renders.',
  },
  {
    id: 298,
    type: 'multiple-choice',
    ...
    topicPath: 'a11y',
    explanation:
      'A visible, associated `<label for="email">` (matching the input `id`) or a wrapping label gives the field a persistent, accessible name: it stays visible while typing, screen readers announce it, and clicking it focuses the input (a bigger touch target). Placeholders disappear on input, are usually low-contrast grey, and are not reliably treated as the accessible name. Use placeholder only for a format EXAMPLE (a name@example.com style hint), never as the sole label. Why others fail: placeholder is not deprecated — just misused. all form controls need labels. capitalization changes nothing about the problems.',
  },
  {
    id: 299,
    type: 'fill-blank',
  ...
  },
  {
    ...
    topicPath: 'a11y',
    explanation:
      'Headings form the document outline that screen-reader users jump through (a heading list is one of the most-used navigation methods). The hierarchy must be semantic: a single `h1` describing the page, `h2` sections, `h3` inside those — never jumping h1 → h4 because it "looks right". If a level looks too big, restyle it with CSS; the tag encodes STRUCTURE, the stylesheet encodes appearance. Why others fail: choosing tags by font size breaks the outline. multiple h1s dilute structure and modern SEO does not reward it. a styled div is invisible to heading navigation.',
  },
  {
    id: 301,
    type: 'multiple-choice',
    ...
    topicPath: 'a11y',
    explanation:
      'WCAG 2.x AA sets 4.5:1 minimum contrast for normal body text and relaxes to 3:1 for LARGE text (roughly 24px regular or 18.5px bold), because bigger glyphs are legible at lower contrast. Non-text UI essentials (input borders, focus indicators, icons) need 3:1 under 1.4.11. Grey-on-grey aesthetics commonly fail — check with the browser DevTools color picker, which shows the ratio and AA/AAA pass marks. Why others fail: contrast is a measurable requirement, not a taste call. many palettes pass — black/white is just the extreme. the rule covers all meaningful text and UI parts.',
  },
  {
    id: 302,
    type: 'fill-blank',
  ...
  <a routerLink="/home" routerLinkActive="active"
    ...
    topicPath: 'a11y',
    explanation:
      'The visual `.active` class means nothing to a screen reader — the semantic equivalent is `aria-current="page"` on the current nav link. Export the directive (`#rla="routerLinkActive"`) and bind `[attr.aria-current]="rla.isActive ? \'page\' : null"`: binding null REMOVES the attribute when the route is inactive. Recent Angular versions can set aria-current automatically via routerLinkActive, but the manual wiring shows the mechanism. Why others fail: aria-selected belongs to tabs/options, not nav links — and hardcoding true on every link is wrong anyway. disabling removes it from tab order; users should still be able to reach it. title tooltips are unreliably exposed.',
  },
  {
    id: 303,
    type: 'multiple-choice',
    ...
    category: 'a11y',
    question:
      'After a client-side route change in an Angular SPA, what accessibility gap must you close that a normal page load handles for free?',
    options: [
    ...
    topicPath: 'a11y',
    explanation:
      'In a classic multi-page site, navigation reloads the document: focus returns to the top and the screen reader announces the new page title. An SPA replaces router-outlet content with NO focus change and NO announcement — a screen-reader user may not know anything happened. Fixes: on NavigationEnd, focus the new view\'s main heading (give it `tabindex="-1"`, then call `.focus()`) or announce via an `aria-live` region (the CDK LiveAnnouncer helps), and set per-route titles with the route `title` property / TitleStrategy. Why others fail: the router swaps DOM only — the semantics are your job. styles persist fine. Angular bindings survive navigation; that is not the issue.',
  },
  {
    id: 304,
    type: 'multiple-choice',
    ...
    topicPath: 'security',
    explanation:
      'The browser downloads your entire compiled bundle — minified, but fully readable. Any string in it (API keys, tokens, credentials) is extractable in seconds from the Network or Sources tab. `environment.ts` merely selects build-time constants; it offers zero secrecy. The pattern: the SPA calls YOUR backend, and the backend holds the secret and makes the privileged call. Public-by-design keys (e.g. a maps key locked to your domain) are the only exception. Why others fail: nothing is encrypted — it is compiled in as plain text. irrelevant to build speed. repo privacy does not matter once the bundle is served to browsers.',
  },
  {
    id: 305,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'A route guard (`canActivate`) hides the /admin page from non-admins. Is the data behind it safe?',
    options: [
  ...
  return next(authReq);
    ...
    topicPath: 'security',
    explanation:
      "`HttpRequest` is immutable — `req.headers.set(...)` returns a NEW headers object and does not change the outgoing request, a classic silent bug. The pattern is `req.clone({ setHeaders: { Authorization: 'Bearer ' + token } })` and forwarding the clone. Functional interceptors (`HttpInterceptorFn`, registered via `provideHttpClient(withInterceptors([...]))`) can use `inject()` for dependencies. Consider skipping the header for non-API or public URLs. Why others fail: mutation is a no-op on an immutable request. rebuilding loses the body, params, and method generality. next() takes only the request.",
  },
  {
    id: 307,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      "How does a Content-Security-Policy (CSP) header complement Angular's built-in sanitization?",
    options: [
    ...
    topicPath: 'security',
    explanation:
      "They operate at different layers. Angular sanitizes interpolated/bound values so script tags or javascript: payloads are neutralized in templates. CSP is an HTTP response header (`Content-Security-Policy: script-src 'self'; …`) the BROWSER enforces: forbidding inline script execution, eval, and loading from unlisted origins. If an XSS slips through anyway, a strict CSP usually prevents the payload from running or phoning home. Angular supports strict CSP via nonces (`ngCspNonce` / the `CSP_NONCE` token) for its inline styles. Why others fail: both stay active — layers, not alternatives. CSP governs scripts, styles, frames, connects, and more. it is a server/header concern, not a bundle setting.",
  },
  {
    id: 308,
    type: 'spot-the-bug',
...
onLoginSuccess() {
  ...
  window.location.href = returnUrl;   // redirect wherever it says
    ...
    topicPath: 'security',
    explanation:
      'An open redirect lets an attacker craft a link on YOUR trusted domain that bounces victims to theirs — ideal for phishing ("please log in again") right after a real login. Because the value comes from the URL, treat it as hostile: accept only relative internal paths (starts with `/`, reject `//` and protocol-relative tricks, or match against a route allowlist) and use `router.navigateByUrl(returnUrl)`, which stays inside the SPA rather than performing a raw browser navigation to an arbitrary absolute URL. Why others fail: queryParamMap reads it fine — that is the problem. performance is irrelevant. ?? correctly defaults on null.',
  },
  {
    id: 309,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'How do you prevent your Angular app from being loaded in a hostile iframe (clickjacking)?',
    options: [
    ...
    topicPath: 'security',
    explanation:
      "Clickjacking overlays your real UI (in an invisible iframe) under fake controls, tricking users into clicking privileged buttons. The defense is response headers the BROWSER enforces before your code runs: `Content-Security-Policy: frame-ancestors 'none'` (modern, supersedes) or `X-Frame-Options: DENY/SAMEORIGIN` (legacy). Configure them on the web server/CDN serving index.html. JavaScript frame-busting is bypassable and not recommended alone. Why others fail: no such Angular API — by the time Angular runs, the frame already loaded. obfuscation does not affect framing. HTTPS secures transport, not embedding.",
  },
  {
    id: 310,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'What is the practical risk of outdated npm dependencies in an Angular project, and the first-line tooling?',
    options: [
    ...
    topicPath: 'security',
    explanation:
      "A frontend app is mostly other people's code: hundreds of transitive packages execute in your CI and ship to every user. Known vulnerabilities (XSS in a widget library, prototype pollution in a util) and supply-chain attacks (malicious patch versions) flow straight in. Baseline hygiene: `npm audit` in CI, regular `ng update` for the framework, a committed lockfile so installs are reproducible, and skepticism toward tiny unmaintained packages. Why others fail: the risk is security, not just speed. npm warns but does not block. runtime deps ship to users; dev deps threaten your build pipeline — both matter.",
  },
  {
    id: 311,
    type: 'fill-blank',
    ...
    category: 'i18n',
    question:
      "Complete the component code so this string is translatable with Angular's built-in i18n:",
...
export class Banner {
  ...
  },
  {
    ...
    category: 'i18n',
    question:
      'How do you translate an element ATTRIBUTE like title or placeholder, not just its text content?',
    options: [
  ...
  },
  {
    ...
    category: 'i18n',
    question:
      'In `<h1 i18n="site header|Greeting on the landing page@@homeGreeting">Hello</h1>`, what are the three parts of the i18n value?',
    options: [
    ...
    topicPath: 'i18n',
    explanation:
      'The full syntax is `i18n="meaning|description@@id"`. The MEANING partitions translation units: two "Hello" texts with different meanings become separate units (a greeting vs. a button label may translate differently); texts with the SAME meaning share one translation. The DESCRIPTION is free-form context for the human translator. The `@@id` pins a custom stable identifier — without it Angular generates an id from the content, which CHANGES whenever the text changes, orphaning existing translations. Why others fail: translations live in the locale files, not inline. locale data is a separate mechanism. nothing here touches CSS.',
  },
  {
    id: 314,
    type: 'fill-blank',
...
<!-- goal: "She replied" / "He replied" / "They replied" -->`,
    ...
    topicPath: 'i18n',
    explanation:
      'ICU messages have two flavors: `plural` for NUMBERS (with locale-aware categories like one/few/many plus exact `=0` matches) and `select` for arbitrary STRING values: `{gender, select, female {She replied} male {He replied} other {They replied}}`. The `other` branch is required as the fallback. Translators receive the whole ICU block and can restructure it per language — which per-branch *ngIf markup would prevent. Why others fail: plural matches numeric categories, not strings like "female". there is no switch keyword in ICU. select exists precisely for this.',
  },
  {
    id: 315,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'state',
    question:
      'Two sibling components each keep their own `cartItems` copy and they keep disagreeing. What is the fix?',
    options: [
    ...
    topicPath: 'state-management',
    explanation:
      'Duplicated state ALWAYS drifts — each copy updates on its own schedule. The remedy is one owner: a root-provided CartStore whose `items = signal<Item[]>([])` is mutated only through store methods like `add(item)`. Components inject the store, render `computed` projections, and call its methods to change data. This is the core of every state-management pattern: single source of truth, derive do not store. Why others fail: polling is a race-condition band-aid. sibling-to-sibling @Input/@Output chains must route through a common parent and re-introduce copies. window globals are untracked, untyped, and invisible to change detection.',
  },
  {
    id: 317,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      'A value must travel from a component to its great-great-grandchild. What is the alternative to passing @Input through every layer ("prop drilling")?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'state-management',
    explanation:
      'A facade is an injectable that exposes WHAT the feature can do (queries as signals/observables, commands as methods) and hides HOW state is managed (dispatching actions, selecting slices, calling APIs). Components become thin — inject the facade, bind its view model, invoke intents — and gain painless testing (mock one facade instead of a store) plus freedom to migrate the state layer later. The risk to watch: a lazy facade that just re-exports the store 1:1 adds indirection without abstraction. Why others fail: not a change-detection tool. that describes a shell/container route component. unrelated to HTTP caching.',
  },
  {
    id: 319,
    type: 'multiple-choice',
    ...
    category: 'state',
    question:
      'Why store collections NORMALIZED — entities keyed by id plus an ids array — instead of a plain nested array?',
    options: [
    ...
    topicPath: 'state-management',
    explanation:
      'Normalization treats client state like a tiny relational DB: each entity type keyed by id (`entities: Record<id, Item>`), relations stored AS ids, plus an `ids: []` array preserving order. Updating one todo is a single spread on `entities[id]` — no deep array surgery; and because an author embedded in 40 posts exists ONCE, an author rename cannot miss stale copies. Selectors then join/denormalize for the view, and @ngrx/entity / withEntities generate the adapters. Why others fail: arrays store fine — they just scale badly for updates. memory use is similar; correctness and update ergonomics are the win. the shape helps regardless of transport.',
  },
  {
    id: 320,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    topicPath: 'template-forms',
    explanation:
      '`[( )]` is pure sugar: `[(x)]="prop"` expands to `[x]="prop" (xChange)="prop = $event"`. Knowing the desugared form matters because you can use the halves independently — e.g. `[ngModel]="name" (ngModelChange)="name = $event.trim()"` to normalize input, something the combined syntax cannot express. The same convention powers custom two-way bindings: an input `value` paired with an output `valueChange` (or the `model()` signal API). Why others fail: it is mechanical sugar, fully expressible longhand. it binds through the directive, not the raw attribute. ngModel belongs to template-driven forms; it does create internal state but not a reactive-forms FormControl you declare.',
  },
  {
    id: 322,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'forms',
    question:
      'How do you disable a control in a reactive form, and why not `[disabled]` in the template?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'form-validation',
    explanation:
      'The second FormControl argument accepts a single validator or an array; all run on every value change and their errors MERGE into one object — e.g. an empty field gives `{ required: true }`, a short password `{ minlength: { requiredLength: 8, actualLength: 3 } }`. Note some built-ins are factories you CALL (`Validators.minLength(8)`, `Validators.pattern(/…/)`) while others are used bare (`Validators.required`, `Validators.email`) — passing `Validators.minLength` uncalled is a classic bug. Why others fail: arrays of validators are standard. validators do not chain fluently. attributes only feed template-driven forms; reactive validators live in the class.',
  },
  {
    id: 326,
    type: 'multiple-choice',
    ...
    category: 'forms',
    question:
      'Why bind submission with `<form [formGroup]="form" (ngSubmit)="save()">` rather than `(click)="save()"` on the button?',
    options: [
    ...
    topicPath: 'reactive-forms',
    explanation:
      "Native forms submit on submit-button click OR Enter in an input — `(ngSubmit)` hooks that unified event, so keyboard users are covered for free, and Angular's form directives suppress the browser's default page-reloading POST. A `(click)` on the button captures only mouse/keyboard activation of that one button and leaves Enter-in-field either dead or triggering an unhandled native submit. Still guard inside: `if (this.form.invalid) { this.form.markAllAsTouched(); return; }` — nothing blocks invalid submission automatically. Why others fail: it is best practice, not enforced. click handlers work, they are just incomplete. validation gating is your code's job.",
  },
  {
    id: 327,
    type: 'multiple-choice',
    ...
    category: 'forms',
    question:
      'With nested groups — form = fb.group({ address: fb.group({ street: [""] }) }) — how do you reach the street control from the class?',
    options: [
    ...
    topicPath: 'reactive-forms',
    explanation:
      "`AbstractControl.get()` accepts a dot-delimited path (`'address.street'`) or array path (`['address', 'street']`) and returns the control or null — the classic access pattern. With the typed forms API, chaining `form.controls.address.controls.street` is fully type-safe and non-null, which many teams now prefer. In the template, nesting is declared structurally: a container with `formGroupName=\"address\"` scopes the inner `formControlName=\"street\"`. Why others fail: controls are not promoted to properties of the group instance. there is no find-by-name search. nesting is a core, fully supported feature.",
  },
  {
    id: 328,
    type: 'multiple-choice',
    ...
    category: 'forms',
    question:
      'What is the canonical pipeline for a search box that queries an API as the user types?',
    options: [
    ...
    topicPath: 'reactive-forms',
    explanation:
      'Each operator solves a real defect: `debounceTime(300)` stops a request per keystroke; `distinctUntilChanged()` skips re-querying when the settled value is the same (e.g. type + delete a char); `switchMap` maps to the HTTP call AND cancels the previous in-flight request, killing the out-of-order-response bug where slow results for "ang" overwrite fresh results for "angular". Render with the async pipe or `toSignal`. Why others fail: raw keyup floods the API and races responses. polling wastes requests and is not type-ahead. `mergeMap` keeps stale requests alive — their late responses can clobber newer ones.',
  },
  {
    id: 329,
    type: 'fill-blank',
...
// usage: new FormControl('', [Validators.required, noSpaces])`,
    ...
    topicPath: 'form-validation',
    explanation:
      "The `ValidatorFn` contract: take the control, return `null` for VALID or a `ValidationErrors` object (`{ noSpaces: true }`, or richer like `{ noSpaces: { position: 3 } }`) for invalid. The key becomes addressable in templates (`errors?.['noSpaces']`) and merges with other validators' errors. Guard against null values (after `reset()`). Factories that need parameters return a ValidatorFn: `forbidden(name: string): ValidatorFn => (c) => …`. Why others fail: booleans are not the contract — Angular expects errors-object-or-null. validators return, never throw. calling setErrors inside a validator fights the forms engine, which overwrites errors from validator returns each run.",
  },
  {
    id: 330,
    type: 'multiple-choice',
    ...
    category: 'forms',
    question:
      'What is the difference between `formControlName="email"` and `[formControl]="emailCtrl"` in a template?',
    options: [
    ...
    topicPath: 'reactive-forms',
    explanation:
      '`formControlName="email"` is a lookup: it requires an ancestor `[formGroup]="form"` (or `formGroupName`) and resolves the string against that group\'s controls — using it without a parent group throws the classic "formControlName must be used with a parent formGroup directive" error. `[formControl]="emailCtrl"` binds an actual FormControl reference, perfect for standalone inputs (search fields, filters) that do not belong to any form. Both connect the same way once resolved. Why others fail: formControlName outside a group errors. both are current API. neither creates controls — you create them in the class; the directives just connect.',
  },
  {
    id: 331,
    type: 'spot-the-bug',
  ...
  <input formControlName="email"
    ...
    topicPath: 'reactive-forms',
    explanation:
      '`ngModel` (template-driven) and `formControlName`/`formControl` (reactive) are two different state-management systems; putting both on one input gives the value two competing owners with different update timing. Angular deprecated the combination in v6 with a prominent console warning, and modern versions reject it — the documented migration is to choose one API. If a component field must mirror the value, derive it: subscribe to `valueChanges` or read the control, do not attach ngModel. Why others fail: the sync is exactly what is NOT defined. neither cleanly wins — behavior was undefined-ish, which is why it was banned. the conflict exists from the first render, not on submit.',
  },
  {
    id: 332,
    type: 'multiple-choice',
    ...
    category: 'forms',
    question:
      'What does the unified `control.events` observable (Angular 18+) give you that valueChanges/statusChanges do not?',
    options: [
  ...
  },
  {
    ...
    category: 'routing',
    question:
      'Why must route order in the Routes array be considered, and where does the wildcard go?',
    options: [
    ...
    topicPath: 'routing-basics',
    explanation:
      'The router walks the config in array order and takes the first route whose path matches — there is no specificity scoring. Practical consequences: `products/new` must precede `products/:id` (otherwise "new" is captured as an id), and `{ path: \'**\' }` is the universal fallback that must sit at the very END; anything after it is unreachable. The same logic applies within each children array. Why others fail: no automatic specificity — that is CSS thinking. nothing is alphabetical. a leading wildcard would match EVERY url and hide your entire app.',
  },
  {
    id: 334,
    type: 'multiple-choice',
    ...
    topicPath: 'routing-basics',
    explanation:
      "Route matching consumes url segments by PREFIX by default. The empty path `''` is a prefix of every url — `/products/42` starts with nothing — so a prefix-matched empty redirect would fire on every navigation (Angular throws at config time for exactly this misconfiguration). `pathMatch: 'full'` restricts the match to when no unconsumed segments remain, i.e. the user is genuinely at the root. Non-empty redirects usually keep prefix matching. Why others fail: omitting it on an empty redirect is a config error. client-side redirects have no HTTP status. query params are unrelated (they are preserved by default on redirects).",
  },
  {
    id: 335,
    type: 'fill-blank',
    ...
    category: 'routing',
    question:
      'Complete the link so each product card navigates to its own detail page (/products/42):',
  ...
  },
  {
    ...
    topicPath: 'routing-basics',
    explanation:
      'The entire point of an SPA is client-side navigation: `routerLink` prevents the default anchor behavior and asks the Router to swap views — no page reload, state (stores, form data, scroll positions per strategy) survives, and navigation is near-instant. Crucially it still renders an actual `href` on the anchor, so middle-click/ctrl-click open-in-new-tab, link previews, and crawlers all work. Reserve raw `href` for EXTERNAL urls. Why others fail: href is fine — for external links. it applies to all internal routes. the rendered anchor looks similar but the click behavior differs completely.',
  },
  {
    id: 337,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'routing',
    question:
      'A list page holds filters in query params. How do you navigate without losing them, and what is the merge/preserve difference?',
    options: [
    ...
    topicPath: 'route-params',
    explanation:
      "By default a navigation replaces the query string with whatever `queryParams` you pass (or nothing) — filters vanish. `queryParamsHandling: 'preserve'` keeps the existing params and IGNORES newly passed ones; `'merge'` unions them with your new `queryParams` taking precedence — the right choice when adding a tab/sort key on top of live filters. The same option exists on `[routerLink]` via `queryParamsHandling`. Why others fail: the default drops them — that is the very problem. localStorage works but reinvents a built-in and breaks shareable urls. resolvers fetch data; they do not restore urls.",
  },
  {
    id: 339,
    type: 'multiple-choice',
    ...
    topicPath: 'routing-basics',
    explanation:
      "`router.navigate(['products', id], { relativeTo: this.route, queryParams: {...} })` builds a UrlTree from parts and supports RELATIVE navigation — its power tool. `router.navigateByUrl('/products/42?tab=specs')` treats the string as the complete new url (always absolute; extras like queryParams are ignored because they belong in the string). Both return `Promise<boolean>` and both run guards/resolvers. The login `returnUrl` pattern is the classic navigateByUrl use. Why others fail: both are current, different-shaped APIs. both are async promises. nothing bypasses guards — they run for every router-initiated navigation.",
  },
  {
    id: 340,
    type: 'multiple-choice',
    ...
    category: 'routing',
    question:
      'For /products/42?sort=price — what is the difference between `paramMap` and `queryParamMap` on ActivatedRoute?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'routing-basics',
    explanation:
      'Since v14 the Router owns titles: `title: \'Products · Shop\'` on the route, or `title: resolveProductTitle` (a ResolveFn returning a string, so /products/:id can show the product name). For app-wide formatting, extend `TitleStrategy` and provide it — one class appends " · MyApp" everywhere instead of repeating it per route. This also fixes the SPA accessibility gap where screen readers announce the new title on navigation. Why others fail: works but scatters the concern and misses non-component routes; the Title service predates the route-based API. index.html only sets the initial title. a <title> tag inside body markup is invalid HTML and ignored.',
  },
  {
    id: 342,
    type: 'multiple-choice',
    ...
    category: 'routing',
    question:
      'Deep links like /products/42 return 404 from the web server (but work when navigating in-app). What is going on?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'router-children-lazy',
    explanation:
      'By default the router destroys the outgoing component and creates the incoming one (except param-only changes on the same route, which reuse the instance). A custom `RouteReuseStrategy` overrides five hooks — `shouldDetach`, `store`, `shouldAttach`, `retrieve`, `shouldReuseRoute` — so you can stash a detached component tree (typically keyed by route path in a Map) and reattach it later: the back-to-search-results-without-refetching pattern. Watch memory (stored trees are alive) and lifecycle expectations (no ngOnInit on reattach). Why others fail: mapping one component to many paths is just route config. HTTP caching is interceptors/services. recreation is already the default.',
  },
  {
    id: 344,
    type: 'multiple-choice',
    ...
    category: 'routing',
    question:
      'A "refresh" button re-navigates to the CURRENT url, but guards and resolvers do not re-run. Which router options fix this?',
    options: [
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'You need a writable "selected item" signal that automatically RESETS whenever the options list it depends on changes, but that the user can also set freely in between. Which primitive fits?',
    options: [
    ...
    topicPath: 'signals-advanced',
    explanation:
      '`linkedSignal` exists precisely for "local state that follows a source": it behaves like a computed on source changes (re-running the computation, resetting the selection) yet stays writable for user interaction. The advanced form — `linkedSignal({ source, computation: (src, previous) => … })` — even receives the previous value so you can PRESERVE the current selection when it still exists in the new list. Why others fail: computed is read-only, so the user could never change the selection. an effect that writes signals to derive state is the classic anti-pattern — it introduces an extra change-detection turn and a glitch window where list and selection disagree. signals are not observables; there is no subscription API, and hand-wiring one via toObservable adds complexity linkedSignal removes.',
  },
  {
    id: 346,
    type: 'predict-output',
...
console.log(double());`,
  ...
  constructor() {
    ...
    topicPath: 'signals-advanced',
    explanation:
      'Anything that is a pure function of other signals belongs in `computed` — it is synchronous (no frame where `items` is new but `total` is stale), lazy, memoized, and cannot loop. The effect version updates `total` only after the effect flushes, so any computed or template reading both during that window sees inconsistent state, and the second signal write triggers another round of change detection. The Angular docs call this out directly: do not use effects to propagate state; use them for OUTWARD side effects — logging, localStorage, imperative DOM/chart APIs. Why others fail: reading signals is exactly how effects track dependencies. any plain JS is fine inside reactive contexts. there is no loop here — the effect does not read `total` — and untracked changes tracking, not writability.',
  },
  {
    id: 348,
    type: 'spot-the-bug',
...
// template: @for (t of todos(); track t) { <li>{{ t }}</li> }`,
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'What must you know before writing `readonly ticks = toSignal(interval(1000))` in a component?',
    options: [
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'Complete the signal-input declarations: the parent MUST provide a user, and size accepts "sm"/"md" strings but is stored as a number:',
  ...
  size = input(16, { ____: (v: 'sm' | 'md') => (v === 'sm' ? 12 : 16) });
    ...
    topicPath: 'inputs',
    explanation:
      'Signal inputs come in two flavors: `input<T>(default)` and `input.required<T>()` — required inputs take no default (there is nothing to read before the parent binds) and missing bindings are a template TYPE-CHECK error, not a runtime surprise. The `transform` option converts the value the parent binds into the value the component stores — the classic examples being booleanAttribute/numberAttribute coercion or, as here, mapping a size keyword to pixels; the input signal then reads as the TRANSFORMED type. Why others fail: signal.required does not exist — signals always need initial values; inputs are the special case. decorator @Input is the legacy API and its option is `required: true`, not bare `required` — and it gives you a plain property, not a signal. input.mandatory and parse are invented names.',
  },
  {
    id: 351,
    type: 'multiple-choice',
    ...
    category: 'signals',
    question:
      'What does the resource()/httpResource() API give you over calling HttpClient yourself in ngOnInit?',
    options: [
  ...
  },
  {
    ...
    category: 'signals',
    question:
      'A filter signal holds an object and is set to a structurally identical NEW object on every keystroke, re-running an expensive downstream computed each time. Best fix?',
    options: [
    ...
    topicPath: 'signals-advanced',
    explanation:
      'Every signal (and computed) accepts `{ equal }`: when a write produces a value the function deems equal to the current one, the signal keeps the OLD value and notifies nobody — the dependency graph downstream never wakes up. That is precisely the tool for object-valued signals where semantic equality differs from reference equality. Note the flip side: with a custom equal, in-place mutations are even more invisible, so immutability discipline still applies. Why others fail: untracked would DETACH the computed from its dependency — it stops updating even for real changes. debouncing reduces frequency but every surviving write still spuriously propagates; and equality IS customizable. JSON.stringify per write is a slow, fragile (key-order-dependent) way to get what `equal` expresses directly.',
  },
  {
    id: 353,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'How does a STANDALONE component enter TestBed, and how do you replace one of its heavyweight child components in a test?',
    options: [
  ...
  console.log(msg);
    ...
    topicPath: 'testing-components',
    explanation:
      'Inside `fakeAsync`, time is a number Angular controls: `setTimeout` registers into a virtual queue and nothing fires until you move the clock. `tick(2000)` moves it to t=2000 — the 1000ms callback runs synchronously during the call (msg = "first"), the 3000ms one stays pending. `flush()` says "run whatever timers remain, however far ahead they are", executing the second callback (msg = "second"). One more rule: a fakeAsync test that ENDS with pending timers throws — flush(), discardPeriodicTasks(), or enough tick() calls must clean the queue. Why others fail: timers fire fine — under your control. flushMicrotasks() is the microtask-only variant; flush() targets the timer queue. mixing tick and flush is routine.',
  },
  {
    id: 355,
    type: 'spot-the-bug',
    ...
    category: 'testing',
    question:
      'This HttpTestingController test fails with "Expected one matching request, found none". Why?',
    code: `it('loads users', () => {
  ...
  req.flush([{ id: 1 }, { id: 2 }]);
    ...
    topicPath: 'testing-services-http',
    explanation:
      'Cold observables do work per subscriber, and HttpClient is strictly cold: `getUsers()` merely BUILDS the request description. Until something subscribes, `HttpTestingController` has recorded zero requests, so `expectOne` correctly reports none. The working shape is: subscribe (stashing results or using firstValueFrom + await after flush), THEN expectOne, THEN req.flush(body) — flush makes the response arrive and the subscriber assertion run. Finish with httpTesting.verify() in afterEach so stray unexpected requests fail loudly. Why others fail: relative URLs match fine — expectOne matches what the app requested. flush belongs to the TestRequest that expectOne returns; the order cannot invert. flush takes the deserialized body directly.',
  },
  {
    id: 356,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'A component test asserts DOM immediately after clicking a button and passes with zone.js but shows STALE DOM in a zoneless app. What is the robust pattern?',
    options: [
    ...
    topicPath: 'zoneless',
    explanation:
      'Zoneless removes the "zone saw your click and synchronously ran app-wide CD" behavior tests silently relied on; instead a click handler notifies the scheduler, which renders shortly after. Tests should mirror that: automatic change detection plus `await fixture.whenStable()` gives you the post-render DOM deterministically, and works identically in zone and zoneless apps (which also makes it the right migration-proof style). Manual `detectChanges()` still works but can PAPER OVER real scheduling bugs — the test forces a render the app might never perform. Why others fail: sleeping is flaky and slow — whenStable resolves exactly when work settles. double detectChanges is cargo-culting; the reliable primitive is awaiting stability. DOM testing is fully supported zoneless — only the flush mechanism changed.',
  },
  {
    id: 357,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'A service effect() persists a signal to localStorage. The unit test sets the signal and asserts localStorage on the next line — and fails. Why, and what is the fix?',
    options: [
    ...
    topicPath: 'testing-components',
    explanation:
      'An effect marks itself dirty when a dependency changes and runs at the next FLUSH point — during change detection for component effects, at scheduler ticks for root/injector effects. A bare `TestBed.inject(MyService)` test has no fixture and never renders, so the flush never comes; the assertion runs before the effect body ever has. `TestBed.tick()` triggers application synchronization (running pending effects); in component tests, `fixture.detectChanges()`/`whenStable` does it as a side effect of rendering. The same scheduling rule explains why the effect also did not run at construction time in the test. Why others fail: effects run fine in TestBed once something flushes. localStorage works normally (jsdom/browser). there is no such option — every signal is trackable.',
  },
  {
    id: 358,
    type: 'fill-blank',
...
expect(fixture.nativeElement.textContent).toContain('Ada');`,
    ...
    topicPath: 'testing-components',
    explanation:
      '`input()` produces an `InputSignal` — reading it calls it like a function, and there is no setter on the instance, so `componentInstance.user = value` is a type error (and with decorator inputs it silently skipped transforms and OnPush marking anyway). `fixture.componentRef.setInput(name, value)` goes through the real input pipeline: transform functions run, `markForCheck`-equivalent dirtying happens, and ngOnChanges fires for decorator inputs. Then the DOM assertion needs a render — `detectChanges()` or auto-detect + `await whenStable()`. Why others fail: setProperty targets DOM properties via the renderer, and autoDetect is enabled BEFORE interactions, not called as a flush. input signals expose no .set — only the component template/parent or setInput feed them. assignment fails on read-only signal inputs — exactly the situation setInput exists for.',
  },
  {
    id: 359,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'Why drive UI-library widgets through component test harnesses (MatButtonHarness etc.) instead of querying their DOM directly?',
    options: [
    ...
    topicPath: 'testing-components',
    explanation:
      'The CDK harness contract has three payoffs. Stability: the harness ships WITH the component, so when mat-select\'s internal markup changes, the harness is updated by the library and your `selectHarness.clickOptions(…)` keeps working, where a `.mat-select-trigger` query silently rots. Portability: `TestbedHarnessEnvironment` and e2e environments implement the same `HarnessLoader` interface, so page-object logic is written once. Determinism: each harness interaction flushes change detection and awaits stability before returning, killing a whole class of "assert before render" flakes — including in zoneless apps. Why others fail: environment choice is orthogonal — TestBed harness tests run in the same runner as other unit tests. rendering still happens; nothing is skipped. direct queries work, they are just brittle — harnesses are a convention, not an enforcement.',
  },
  {
    id: 360,
    type: 'spot-the-bug',
    ...
    category: 'testing',
    question:
      'This test is green today — and would STAY green if the filtering logic broke completely. What is the flaw?',
  ...
  service.activeItems$.subscribe(items => {
    ...
    topicPath: 'testing-services-http',
    explanation:
      'The assertion lives inside a callback whose execution is OPTIONAL. If `activeItems$` becomes async (a delay, a switchMap to HTTP), or errors, or never emits, the subscribe callback does not run before the synchronous test body ends, no expectation executes, and the runner reports success. `await firstValueFrom(obs)` inverts the contract: no emission → rejected promise → failed test; the value comes back to the test body where assertions are guaranteed to run. Alternatives with the same property: done callbacks (fail by timeout), expect.assertions(1) (Vitest/Jest, fails when the count is off), or marble testing. Why others fail: subscribing in tests is legal, just assertion-unsafe as shown. any boolean expression works in expect. one dangling test subscription is untidy but will not fail anything — which is exactly the problem.',
  },
  {
    id: 361,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'A heavy chart component sits below the fold. Which @defer configuration loads it best?',
    options: [
    ...
    topicPath: 'deferrable-views',
    explanation:
      'And the separation of the two triggers is the senior detail. `on viewport` (an IntersectionObserver on the @placeholder content) gates INSTANTIATION, keeping main-thread work out of startup. `prefetch on idle` gates only the network FETCH, so by the time the user scrolls down, the JavaScript is usually already in cache — no spinner at the moment of visibility. The placeholder both reserves layout (avoiding CLS) and provides the element the observer watches. Dependencies must be otherwise-unreferenced (no @ViewChild into the deferred block, no eager import elsewhere) or they stay in the main bundle. Why others fail: on immediate still splits the chunk but downloads AND instantiates at startup — the work you were avoiding. hand-rolled visibility tracking recreates `on viewport` worse, and @defer (when …) never code-splits reactively better than triggers. @if hides rendering but the import keeps the chart in the initial bundle.',
  },
  {
    id: 362,
    type: 'spot-the-bug',
    ...
    category: 'performance',
    question:
      'Sorting this list makes row components lose their expanded/edit state, and profiling shows every row re-rendering. The data items have stable ids. What is wrong?',
  ...
  <app-user-row [user]="user" />
    ...
    topicPath: 'control-flow-for',
    explanation:
      'The track expression is the identity key for DOM reuse. With `$index`, a reorder looks like "every position kept its identity but all the DATA changed" — Angular rebinds N inputs, child components re-run their input-driven logic, and any state living inside a row (an open accordion, a half-typed input, focus) stays at its old POSITION while its data moves elsewhere: the classic wrong-row bug. With `track user.id`, the same reorder is "identities moved" — Angular moves the existing DOM nodes, zero rebinds, state travels with its item. $index is only right when items truly have no identity (static lists, primitive duplicates). Why others fail: @for renders components fine. property access is the normal form — trackBy FUNCTIONS were the old NgFor API. mutability of the array is irrelevant to tracking.',
  },
  {
    id: 363,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'What do you get by switching <img src="hero.jpg"> to <img ngSrc="hero.jpg" width="800" height="400" priority>?',
    options: [
    ...
    topicPath: 'performance',
    explanation:
      'The directive encodes the Core-Web-Vitals checklist a performance reviewer would apply by hand. Dimensions (or fill mode) prevent layout shift; below-the-fold images get loading="lazy" decoding="async" for free; the ONE above-the-fold hero gets `priority`, which flips it to eager + fetchpriority=high and (with SSR) emits a <link rel="preload">, directly attacking LCP. Loader functions integrate image CDNs so srcset/sizes emit automatically, and dev-mode diagnostics warn about oversized or distorted images. Client-side format conversion is not something a directive can do — that is the CDN\'s job via the loader. Why others fail: format negotiation happens at the server/CDN. nothing is inlined; URLs are rewritten. any host works — loaders exist for generic and custom CDNs.',
  },
  {
    id: 364,
    type: 'multiple-choice',
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'With SSR + incremental hydration (withIncrementalHydration and @defer hydrate triggers), what happens to a section marked @defer (hydrate on interaction)?',
    options: [
    ...
    topicPath: 'hydration',
    explanation:
      'Incremental hydration decouples "visible" from "interactive": full HTML arrives from the server (SEO and perceived performance keep their SSR benefits), but the deferred subtree\'s code is excluded from the initial client work — less JS parsed, less hydration CPU, better TTI/INP on the pages that need it most. `hydrate on interaction/viewport/idle` etc. choose the wake-up moment; hydration happens IN PLACE against the existing DOM (the non-destructive kind), and event replay (withEventReplay) captures the interaction that arrived before hydration finished and re-dispatches it after, so the triggering click actually does something. Why others fail: blank-until-clicked describes plain @defer WITHOUT SSR hydration triggers — incremental hydration exists to avoid exactly that. hydrate triggers are the production feature, not a dev toggle. re-rendering from scratch is destructive hydration, the legacy fallback this replaces.',
  },
  {
    id: 366,
    type: 'multiple-choice',
    ...
    category: 'performance',
    question:
      'A template shows {{ calculateTotal() }} where calculateTotal is a component METHOD looping over the cart. Why do performance guides flag this, and what replaces it?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'A zone-based app draws on a canvas from a mousemove listener and profiling shows change detection running hundreds of times per second. The drawing touches no template state. Fix?',
    options: [
  ...
  },
  {
    ...
    category: 'performance',
    question:
      'Users report the app getting slower the longer they navigate around. This dashboard component is on the busiest route. Find the leak:',
    code: `@Component({ /* … */ })
export class Dashboard {
  ...
  other {{{count}} messages}
    ...
    topicPath: 'i18n',
    explanation:
      "The point of ICU plurals is that plural GRAMMAR is locale-specific. English has two forms (one/other), Arabic has six, Polish needs few/many; CLDR defines the categories per language and the ICU message carries all branches as ONE translation unit, so the translator supplies whatever forms their language needs. `=0` style exact matches layer on top for special-casing. An @if chain freezes the source language's rules into the template: a Polish translator has nowhere to put the `few` form because the branching lives in code, not in the translatable message. ICU `select` does the same for non-numeric variants (gender, status). Why others fail: the outputs differ fundamentally — one is a single translatable unit with locale-driven branching, the other is untranslatable control flow around fragments. ICU has no change-detection semantics. ICU is the current, supported mechanism; there is no plural pipe in Angular.",
  },
  {
    id: 371,
    type: 'spot-the-bug',
    ...
    category: 'i18n',
    question:
      'The template is fully marked with i18n, yet German users still see English text in the toast. Where is the leak?',
    code: `@Component({ /* … */ })
export class Cart {
  ...
  remove(item: CartItem): void {
    ...
    topicPath: 'i18n',
    explanation:
      'Template `i18n` attributes cover only what lives in templates; any user-facing string born in TypeScript — toasts, dialog titles, validation messages, document.title — must be tagged with the `$localize` template literal (from @angular/localize) to enter the extraction pipeline. `$localize` supports the same `:meaning|description@@id:` prefix and NAMED placeholders (`\${expr}:name:`), which matters because translators must be able to REORDER the placeholder: German might say "…aus dem Warenkorb entfernt: NAME". The original concatenation is doubly broken — invisible to extraction AND word-order-locked. Why others fail: injector scope has nothing to do with extraction. concatenation is legal everywhere; it is just untranslatable. there is no i18n field on @Component — marking happens per string, not per class.',
  },
  {
    id: 372,
    type: 'multiple-choice',
    ...
    category: 'i18n',
    question:
      'How does built-in Angular i18n actually DELIVER multiple languages to production, and what is the main trade-off?',
    options: [
    ...
    topicPath: 'i18n',
    explanation:
      'With `"localize": true` (or an array of locales) in angular.json, the build compiles the app once, then generates a per-locale variant by substituting translation units into the templates — the docs call this build-time inlining. Deployment is directory-per-locale behind content negotiation (Accept-Language redirect, path prefix like /de/, or user preference cookie). The strengths and the constraint are two sides of one fact: because strings are inlined, there is no translation lookup at runtime and dead branches per locale can be optimized, but the running app knows only its own language — in-place switching requires a full navigation to the other build. Teams that need live switching typically reach for runtime libraries (Transloco/ngx-translate) and accept dictionary lookups instead. Why others fail: describes runtime i18n libraries, not the built-in pipeline. same — built-in i18n fetches nothing at startup. locale granularity is per-app, not per-route; lazy chunks within the German build are still all-German.',
  },
  {
    id: 373,
    type: 'fill-blank',
    ...
    category: 'i18n',
    question:
      'Complete the template so the title ATTRIBUTE (not just the text content) gets translated:',
    code: `<button
  ...
  },
  {
    ...
    category: 'i18n',
    question:
      'A Swiss-German build (locale de-CH) shows dates as 3/15/2026 and currency as $ instead of CHF formats. The DatePipe and CurrencyPipe are used correctly. What is missing?',
    options: [
    ...
    topicPath: 'i18n',
    explanation:
      "formatting is a two-part contract: LOCALE_ID tells pipes WHICH locale to use, and registered CLDR locale data tells them HOW that locale formats dates, numbers and currencies (day order, separators, currency symbol placement — de-CH uses apostrophe-grouped numbers like 1'234.50). Angular ships only en-US data by default to keep bundles lean, so an unregistered locale silently degrades to US formats. The i18n build pipeline wires both for you per locale build; hand-rolled setups (or runtime-i18n apps) must call `registerLocaleData(localeDeCh)` (data imported from @angular/common/locales/de-CH) and provide LOCALE_ID — and CurrencyPipe additionally defaults its currency CODE from DEFAULT_CURRENCY_CODE, which you set to CHF. Why others fail: per-call locale arguments work but still require the locale data to be registered, and repeating the locale at every call site is exactly what LOCALE_ID exists to avoid. the pipes are Intl/CLDR-driven and support every registered locale. HTTP headers never reach pipe internals.",
  },
  {
    id: 375,
    type: 'multiple-choice',
    ...
    category: 'a11y',
    question:
      'A code review flags this "button". What is actually broken about it for keyboard and screen-reader users?',
    code: `<div class="btn" (click)="save()">Save</div>`,
    options: [
  ...
  },
  {
    ...
    category: 'a11y',
    question:
      'The toolbar looks perfect, but a screen-reader user hears only "button… button… button". Fix the root cause:',
...
<button type="button" (click)="remove()">
  ...
  },
  {
    ...
    category: 'a11y',
    question:
      'You are building a custom modal dialog (no Material/CDK Dialog). Which keyboard/focus behavior is REQUIRED, and what does the Angular CDK give you for it?',
    options: [
    ...
    topicPath: 'a11y',
    explanation:
      'This is the WAI-ARIA dialog pattern, and every piece exists because of what happens without it: if focus stays on the trigger, a screen-reader user "opens" a dialog they cannot find; if Tab escapes the dialog, keyboard users interact with controls they cannot see behind the overlay; if focus is not restored on close, the user is dumped at the document top and loses their place. The CDK ships the hard parts: `cdkTrapFocus` (with cdkTrapFocusAutoCapture to capture and later restore focus) implements the wrap-around trap, and pairing it with role="dialog", aria-modal="true" and an aria-labelledby pointing at the title completes the semantics. Escape-to-close and click-outside remain yours to wire. (Using CDK Dialog/Overlay gets all of this pre-assembled — the exam point is knowing WHAT the contract is.) Why others fail: without focus management the dialog is unusable non-visually, not merely less polished. hiding the background from the TAB ORDER is what the trap does, but you also need aria-modal/inert semantics — hand-walking the DOM misses dynamically added content and is exactly the wheel CDK invented. autofocus addresses only the entry half of the contract, and unreliably.',
  },
  {
    id: 378,
    type: 'multiple-choice',
    ...
    category: 'a11y',
    question:
      'After an async save, a toast appears for 4 seconds: <div class="toast">Saved!</div>. Sighted users see it; screen-reader users get nothing. What is the correct mechanism?',
    options: [
  ...
  },
  {
    ...
    category: 'a11y',
    question:
      'In an SPA, clicking a routerLink swaps the page content — but a screen-reader user hears NOTHING and their focus is still on the old nav link. What is the established remediation?',
    options: [
    ...
    topicPath: 'a11y',
    explanation:
      'A real page load gives screen-reader users two signals: the new title is announced and the reading cursor resets to document top. Router navigation gives neither — the DOM under the outlet changed but focus and the reading position still sit on the clicked link, so the user must explore to discover anything happened. The pattern: give the main content region (or its h1) tabindex="-1", subscribe to NavigationEnd (or use an afterNextRender hook in the routed component), and call .focus() on it — tabindex="-1" is the key detail, making the element focusable by SCRIPT but not adding a Tab stop for keyboard users. Angular sets the document title per route via the title property; announcing it through a LiveAnnouncer complements the focus move. Why others fail: title changes alone are not reliably announced by screen readers during SPA navigation. firing assertive announcements of entire page contents on every navigation is unusable noise, and router-outlet\'s replaced content is not a well-formed live-region update. SPAs are perfectly accessible WITH focus management — that is the remediation, not surrender.',
  },
  {
    id: 380,
    type: 'fill-blank',
    ...
    category: 'a11y',
    question:
      'Complete the template so the input has a proper, clickable, screen-reader-announced label:',
...
<input id="email" type="email" [formControl]="email" />`,
    ...
    topicPath: 'a11y',
    explanation:
      '`for`/`id` pairing is the foundational form-accessibility association (wrapping the input inside the label is the equivalent implicit form). It does three jobs at once: the accessible NAME of the input becomes the label text, clicking/tapping the label focuses the control (a real usability win on mobile), and voice-control users can target the field by its visible name. Common Angular-specific slips: generating non-unique ids in @for loops (association silently binds to the FIRST match) and placeholder-as-label designs, which fail because placeholders vanish on input and are not reliably announced. When a visible label is truly impossible, aria-label on the INPUT is the fallback — but the visible+associated label is strictly better. Why others fail: there is no label binding to FormControl; reactive forms and accessibility wiring are separate concerns. aria-label goes on the control itself and replaces, not references, a visible label. name participates in form submission, not labeling.',
  },
  {
    id: 381,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'A user saves their display name as <img src=x onerror="alert(1)">. The template renders {{ user.displayName }}. What happens?',
    options: [
  ...
  },
  {
    ...
    category: 'security',
    question:
      'This renders user-submitted forum posts "with formatting support". Find the vulnerability:',
    code: `@Component({
...
export class Post {
  ...
  get trustedBody(): SafeHtml {
    ...
    topicPath: 'security',
    explanation:
      'The bypass* family exists for app-CONTROLLED values that the sanitizer would mangle — an iframe URL you construct yourself, SVG markup you author — never for content that originated in another user\'s hands. The name is the warning: you are asserting trust Angular cannot verify, and with forum posts you are asserting it about your attackers. Stored XSS like this is worse than reflected: the payload persists and fires for every visitor, harvesting sessions or acting as the victim. The fix is deleting code — plain [innerHTML] binding sends the string through sanitization, keeping <b>, <a href="https://…"> etc. while stripping event handlers and script vectors. If posts are markdown, render to HTML server-side with a strict allowlist and still bind without bypass. Defense in depth: a Content-Security-Policy and Trusted Types catch the bypasses code review misses. Why others fail: getters returning SafeHtml are legal (though a computed would memoize) — style, not security. [innerHTML] is correct casing. signal inputs vs decorator inputs is irrelevant to the vulnerability.',
  },
  {
    id: 383,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      "Your API uses cookie-based sessions. How does Angular's HttpClient participate in CSRF/XSRF defense, and what must the SERVER do for it to work?",
    options: [
    ...
    topicPath: 'security',
    explanation:
      "The mechanism only clicks once you see WHY reading matters more than sending: CSRF exists because browsers attach cookies to cross-site requests automatically, so a hostile page can trigger authenticated POSTs blind. The token cookie is deliberately NOT HttpOnly — page JavaScript (Angular's HttpXsrfInterceptor) must read it to mirror it into the header, and same-origin policy stops the attacker's page from doing that read. Server-side responsibilities: issue the token cookie (e.g. Spring Security's CookieCsrfTokenRepository.withHttpOnlyFalse()), and reject mutating requests where header and cookie disagree. Angular skips GET/HEAD (safe methods should have no side effects to forge) and absolute URLs to other origins (mirroring the token there would LEAK it). Why others fail: HttpClient enforces no such blocking — CORS is a server-declared browser policy and does not stop simple form POSTs anyway. header-borne JWTs are not auto-attached by browsers, so classic CSRF does not apply — the trade is XSS theft risk instead. withCredentials makes cross-origin requests CARRY cookies — closer to enabling the attack surface than defending it.",
  },
  {
    id: 384,
    type: 'predict-output',
    ...
    category: 'security',
    question:
      'A user profile contains website: "javascript:stealCookies()". What does the rendered link actually do?',
...
<a [href]="profile.website">My site</a>`,
    ...
    topicPath: 'security',
    explanation:
      'This demonstrates CONTEXT-AWARE sanitization. Angular\'s template compiler knows [href] is a URL security context, so bound values pass through URL sanitization at render: safe schemes (https, http, mailto, tel…) pass untouched; javascript: and data:text/html get the `unsafe:` prefix, defusing them while leaving forensic evidence in the DOM. Each context has its own rules — [innerHTML] gets HTML sanitization, [style.xyz] historically style checks, and RESOURCE URL contexts like <iframe src> accept NO sanitization at all (there is no safe way to sanitize an arbitrary document to embed), which is why iframes demand either a static string or an explicit bypassSecurityTrustResourceUrl. The correct app-level fix for user "website" fields is validating/normalizing to https?:// at input time, not trusting the defusal. Why others fail: attribute/property bindings in security contexts ARE sanitized — that is the core mechanism. the attribute survives with the prefixed value, not removed. sanitization degrades gracefully; no error is thrown for plain URL contexts.',
  },
  {
    id: 385,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'Security wants defense-in-depth so that even a bypassSecurityTrustHtml misuse cannot inject script. Which platform mechanism does Angular integrate with for this?',
    options: [
    ...
    topicPath: 'security',
    explanation:
      "Trusted Types moves XSS defense from convention to platform enforcement: with the CSP directive active, `element.innerHTML = someString` THROWS unless the value was produced by a registered policy, no matter how the string got there. Angular's renderer routes its sanitized output through its `angular` policy, so a normal app runs cleanly with `trusted-types angular; require-trusted-types-for 'script'` — and because the bypass APIs use the separate `angular#unsafe-bypass` policy name, OMITTING that name from the header means every bypassSecurityTrust* call site fails loudly in the browser: the exact backstop asked for. Rollout is observable via Content-Security-Policy-Report-Only. Third-party libs that write raw HTML will also surface, which is a feature. Why others fail: X-Frame-Options addresses clickjacking (framing), unrelated to script injection. strictTemplates checks TYPES; a string is a string whether malicious or not. change-detection strategy has zero relationship to DOM injection sinks.",
  },
  {
    id: 386,
    type: 'multiple-choice',
    ...
    category: 'security',
    question:
      'Where should an SPA keep its session credential — localStorage JWT or HttpOnly cookie — and what is the actual trade-off?',
    options: [
  ...
  },
  {
    ...
    category: 'state',
    question:
      'A cart count must show in the header, the product page, and a checkout badge. Passing it through inputs is getting painful. What is the idiomatic Angular solution?',
    options: [
  ...
  },
  {
    ...
    category: 'state',
    question:
      'Review this store. Why is this shape — private writable, public readonly, mutations as methods — the recommended discipline?',
    code: `@Injectable({ providedIn: 'root' })
export class TodoStore {
  ...
  },
  {
    ...
    category: 'state',
    question:
      'Clicking "complete" updates the database, but the remaining-count in the header never changes. The store method is below. Why does nothing react?',
  ...
  },
  {
    ...
    category: 'state',
    question:
      'When does adopting a state-management LIBRARY (NgRx Store / SignalStore) genuinely pay for itself over the signal-in-a-service pattern?',
    options: [
    ...
    topicPath: 'state-management',
    explanation:
      'This is an engineering-judgment question, and the honest answer names the FORCES rather than a team allegiance. Event-sourced stores (NgRx Store) shine when you need to answer "what sequence of events produced this state?" — devtools time-travel, action logs for debugging production incidents, strict unidirectionality that survives 30 contributors. Those benefits are real and so is their price: every trivial write becomes action + reducer case + possibly effect + selector, files multiply, and newcomers learn a meta-framework. The signal-service pattern covers a surprising share of real apps; NgRx SignalStore is the middle path (structured, opinionated, signal-native, less boilerplate). Choosing SMALLEST-tool-that-solves-the-problem is the senior signal here. Why others fail: cargo-culting infrastructure ahead of need is how simple apps rot. SSR works fine with plain services (TransferState is orthogonal). signals changed the REACTIVITY substrate; coordination, auditability and convention problems still exist at scale — SignalStore existing is NgRx agreeing.',
  },
  {
    id: 391,
    type: 'predict-output',
...
function run(): void {
  ...
  },
  {
    ...
    category: 'state',
    question:
      'This component copies store state into local fields "so the template is simpler" — and now search results go stale when another tab of the app adds a product. What principle was violated?',
  ...
  get results(): Product[] {
    ...
    topicPath: 'state-management',
    explanation:
      "`this.store.products()` OUTSIDE a reactive context returns a plain array value — a photograph, not a subscription. From that line on the app has two versions of the truth, and every stale-data bug is some flavor of this fork. The rewrite eliminates the copy rather than synchronizing it: `readonly query = signal('')` and `readonly results = computed(() => this.store.products().filter(p => p.name.includes(this.query())))` — the computed re-derives when EITHER dependency changes, there is nothing to refresh, and the template reads results(). The rule generalizes: store MINIMAL source state; everything expressible as a function of it should be computed, not copied (the getter here also re-filtered on every CD cycle without caching — computeds memoize). Why others fail: any lifecycle hook that copies produces the same fork, just timed differently. signal reads work anywhere; only TRACKING requires a reactive context (computed/effect/template) — which is exactly why the snapshot did not subscribe. push-refresh reinvents change propagation manually and scales as O(components × stores) wiring; deriving gets it from the reactive graph for free.",
  },
  ...
  // --- TEMPLATES (HTML in Angular) ---
  {
    ...
    category: 'templates',
    question:
      'What is the difference between binding `[src]="url"` and writing `src="{{ url }}"` on an <img>?',
    options: [
...
<td [colspan]="span">Total</td>`,
    ...
    topicPath: 'property-binding',
    explanation:
      'Angular property binding writes to DOM properties, and it is a compile-time error (or silent miss) when no such property exists. colspan lives only as an HTML attribute (the property is actually colSpan, with different casing — the classic tell that attributes ≠ properties). The reliable form is `[attr.colspan]="span"`, which calls setAttribute under the hood. The same rule covers aria-* (`[attr.aria-label]`) and data-* attributes. Why others fail: [colSpan] happens to exist as a property, but the general fix — and what the exam wants — is attr. binding for attribute-only targets. numbers bind fine; attr binding stringifies them. every element supports bindings.',
  },
  {
    id: 395,
    type: 'multiple-choice',
    ...
    topicPath: 'structural-directives',
    explanation:
      '<ng-container> is purely logical: Angular uses it to anchor directives and then renders only its CHILDREN. The two canonical uses: grouping several elements under one *ngIf without a wrapper <div> (which could break flex/grid layout), and putting control flow inside markup where extra elements are INVALID — e.g. between <table> and <tr>, where a <div> would be ejected by the parser. Why others fail: no element is emitted at all, so no display: contents trick is needed. it leaves at most a comment anchor — negligible. no custom element reaches the DOM.',
  },
  {
    id: 396,
    type: 'multiple-choice',
    ...
    category: 'templates',
    question:
      'What does a template reference variable point at — #box on a <div> versus #search on an <app-search> component?',
    options: [
    ...
    topicPath: 'view-queries',
    explanation:
      'The reference resolves by what it sits on: plain element → element instance (so #input lets you write input.value in an event handler), component host → the component instance (so #search exposes its public API, e.g. search.clear()). When several directives share the host, `#x="ngForm"`-style exportAs picks WHICH one you get — that is exactly how template-driven forms hand out the NgForm. viewChild(\'box\') reads the same reference from the class. Why others fail: (A)/the target depends on the host, and they are real object references, not id strings.',
  },
  {
    id: 397,
    type: 'predict-output',
...
<p>{{ user!.email }}</p>  <!-- non-null assertion -->`,
    ...
    explanation:
      '`?.` is a RUNTIME guard: when user is null/undefined the whole expression evaluates to undefined and interpolation renders it as an empty string — no error. `!` is the opposite kind of tool: a COMPILE-TIME promise to the type-checker that user is non-null, erased entirely from the emitted code — so at runtime the property read executes on undefined and throws. The exam point: ?. changes behavior, ! only changes type-checking. Why others fail: the first line is explicitly safe. interpolation renders null/undefined as empty string, not the word "undefined". reversed.',
  },
  {
    id: 398,
    type: 'multiple-choice',
    ...
    category: 'templates',
    question:
      'A CMS sends HTML strings that you render with [innerHTML]="post.html". A <script> tag and an onclick attribute are in the payload. What happens?',
    options: [
    ...
    topicPath: 'security',
    explanation:
      'Angular treats values crossing into dangerous sinks (innerHTML, style, URLs) by CONTEXT: an innerHTML binding runs through the built-in sanitizer, which whitelists safe tags and removes executable content — <script>, on* handlers, javascript: URLs. The page shows the formatting but cannot execute the payload. bypassSecurityTrustHtml exists for content you can vouch for (already sanitized server-side) and is the audit-me keyword in reviews. Why others fail: the sanitizer is precisely what prevents this. it strips rather than throws. escaping-to-text is what INTERPOLATION does — {{ post.html }} would show the tags; [innerHTML] renders them.',
  },
  {
    id: 399,
    type: 'multiple-choice',
    ...
    category: 'templates',
    question:
      'What problem does the template @let block solve, e.g. @let total = items().length * price();?',
    options: [
    ...
    topicPath: 'let-block',
    explanation:
      "@let gives a template-local name to any expression: subsequent bindings read `total` instead of re-writing the expression, and it re-evaluates when its dependencies change — it is a live alias, not a snapshot. It especially shines for `@let user = user$ | async` (one subscription, one name, no *ngIf-as-a-variable hack) and for narrowing long optional chains. Scope is lexical: available after its declaration within the same block. Why others fail: it never touches the class. it stays reactive across change detection. it declares a value and renders nothing conditionally — that is @if's job.",
  },
  {
    id: 400,
    type: 'multiple-choice',
    ...
    category: 'templates',
    question:
      'Why does the Angular style guide favor <button (click)="save()"> over <div (click)="save()"> for a save action?',
    options: [
    ...
    topicPath: 'a11y',
    explanation:
      'The template is still HTML, and element choice carries behavior contracts: <button> participates in the tab order, fires click for Enter/Space presses, exposes an implicit role="button" to assistive tech, and honors the disabled attribute. The div version silently works for mouse users only — the accessibility bugs surface later as keyboard traps and silent screen readers. Recreating it needs tabindex="0", role="button", a keydown handler and aria-disabled — four attributes to badly imitate one element. Why others fail: any element can bind (click). rendering cost is identical. event bindings compile on every element.',
  },
  ...
  // --- STYLING (CSS in Angular) ---
  {
    ...
    category: 'styling',
    question:
      "A rule `p { color: red; }` sits in one component's styles. Why do <p> elements in OTHER components stay uncolored?",
    options: [
    ...
    topicPath: 'components',
    explanation:
      "With ViewEncapsulation.Emulated (the default) the compiler stamps the component's DOM with a generated attribute and rewrites `p { … }` into `p[_ngcontent-xxx] { … }`. The effect: styles cannot leak OUT of the component (and outside page styles that do not use such attributes still cascade IN unless more specific). This is why each component can use generic selectors fearlessly. Why others fail: tags are untouched; attributes are added. ngStyle is an unrelated binding API. component styles are fully real CSS, just scoped.",
  },
  {
    id: 402,
    type: 'multiple-choice',
    ...
    topicPath: 'class-style-binding',
    explanation:
      '`[class.name]="boolExpr"` is the surgical form — one class, toggled by one boolean, coexisting peacefully with the element\'s static class attribute. The object-map form ([class]="{…}") and [ngClass] handle multiple conditional classes. Why others fail: interpolates the literal text "active=true" INTO the class attribute value — a garbage class name. className is not an Angular binding and the string "isActive" would be applied literally. parentheses are event-binding syntax; classes are not events.',
  },
  {
    id: 403,
    type: 'multiple-choice',
    ...
    topicPath: 'components',
    explanation:
      "None removes the scoping step: the component's CSS lands in a plain <style> tag with the selectors untouched, exactly as if written in styles.css. Legitimate uses: a design-system root that intentionally publishes global rules, or styling content Angular cannot attribute-stamp. The risk is selector collision — `button { … }` now restyles every button in the app, loading-order dependent. ShadowDom is the third mode: real shadow-root isolation where outside styles cannot cascade in at all. Why others fail: styles still render — globally. that is ViewEncapsulation.ShadowDom, the opposite direction. encapsulation is a CSS concern; inputs are unaffected.",
  },
  {
    id: 404,
    type: 'multiple-choice',
    ...
    category: 'styling',
    question:
      'In component CSS, what does the :host selector target — and what does :host(.compact) add?',
    options: [
  ...
  },
  {
    ...
    category: 'styling',
    question:
      "A parent tries to restyle a child component's internals and it does not work. What is the accepted fix?",
...
app-fancy-list .item-title {
  color: purple;      /* never applies */
    ...
    topicPath: 'components',
    explanation:
      "The parent's scoped rule compiles to `app-fancy-list[_ngcontent-parent] .item-title[_ngcontent-parent]` — but the child's internals carry the CHILD's attribute, so the selector can never match. That is encapsulation working as designed. The maintainable escape hatch is a contract, not a crowbar: CSS custom properties INHERIT through encapsulation boundaries, so the child declares which knobs exist (var(--item-title-color)) and any ancestor sets them (`app-fancy-list { --item-title-color: purple; }`). ::ng-deep pierces everything, is deprecated, and leaks the rule to every descendant forever. Why others fail: specificity is irrelevant — the attribute requirement makes the match impossible. load order does not change scoping attributes. ShadowDom on the parent isolates it MORE, not less.",
  },
  {
    id: 406,
    type: 'multiple-choice',
    ...
    category: 'styling',
    question:
      "What is the difference between the styles in a component's `styles`/`styleUrls` and the ones in the global `src/styles.css`?",
    options: [
    ...
    topicPath: 'class-style-binding',
    explanation:
      'Two delivery mechanisms with different scopes: styles.css (registered under "styles" in angular.json) is document-wide plain CSS — the right home for resets, design tokens (:root { --brand: … }), and typography. Component styles are code-split with their component (a lazily-loaded page\'s CSS arrives only when visited) and are attribute-scoped so they cannot leak. The idiomatic architecture: global tokens + component-scoped consumption via var(). Why others fail: they are bundled and injected differently. the cascade still follows normal specificity and order — scoping ADDS specificity but does not create a special override tier. globals are where shared CSS variables usually LIVE.',
  },
  {
    id: 407,
    type: 'multiple-choice',
    ...
    category: 'styling',
    question:
      'Why are CSS custom properties (var(--x)) THE theming mechanism that plays well with view encapsulation?',
    options: [
      ...
      'Angular compiles each var() reference into a real component @Input binding automatically',
      'They bypass the browser cascade entirely, which is what makes them fast',
    ],
    answer: 1,
    topicPath: 'class-style-binding',
    explanation:
      "Encapsulation works by rewriting selectors so parent rules cannot MATCH child internals — but it does not (and cannot) stop value INHERITANCE, and custom properties inherit by definition. So the theme lives at the top (`:root { --surface: #fff }`, flipped by `[data-theme=dark] { --surface: #111 }`), and every component writes `background: var(--surface)` in its own scoped CSS. Components keep their internals private while still following the theme — this exact app's light/dark toggle works this way. Why others fail: sanitization concerns bindings, not stylesheet features. no such compilation exists. they participate in the cascade normally; the win is inheritance, not bypass.",
  },
  {
    id: 408,
    type: 'predict-output',
    ...
    topicPath: 'class-style-binding',
    explanation:
      'Style bindings accept unit suffixes — [style.width.px], [style.font-size.em], [style.margin.%] — appending the unit to a plain number, which keeps component code numeric and free of string concatenation. Each binding targets one property; on conflicts, the more specific [style.prop] binding wins over a [style] object binding, which wins over the static style attribute. Why others fail: (A)/the suffix syntax is real and exists precisely so numbers work. [ngStyle] is an alternative for maps, not a requirement.',
  },
  ...
  // --- TOOLING (angular.json, tsconfig, package.json) ---
  {
    ...
    category: 'tooling',
    question:
      'In package.json, why is @angular/core under "dependencies" while @angular/cli sits in "devDependencies"?',
    options: [
  ...
  },
  {
    ...
    topicPath: 'terminal-and-npm',
    explanation:
      'scripts are the project\'s command palette: `npm start` → ng serve, `npm test` → ng test, `npm run build` → ng build. npm prepends node_modules/.bin to PATH for the child shell, so the locally-installed ng binary resolves — every collaborator and CI machine runs the exact pinned CLI version, no global install needed (that is also why `npx ng` works ad hoc). Why others fail: script/style INJECTION is angular.json\'s "scripts"/"styles" arrays — a classic name collision the exam probes. (C)/unrelated to npm scripts.',
  },
  {
    id: 411,
    type: 'multiple-choice',
    ...
    explanation:
      'angular.json is how `ng build` knows WHAT to do: for each project it maps target names to a builder (e.g. @angular/build:application) plus options — entry points, index.html, tsconfig to use, assets to copy, the global "styles"/"scripts" arrays, size budgets, and named configurations like production/development that ng build -c selects. package.json says what to install; angular.json says how to build; tsconfig says how to compile TS. Why others fail: that is package.json. that is tsconfig.json. it is consumed at build time only — nothing in the browser reads it.',
  },
  {
    id: 412,
    type: 'multiple-choice',
    ...
    category: 'tooling',
    question:
      'In package.json, what is the difference between "^20.1.0" and "~20.1.0" — and what does package-lock.json add?',
    options: [
  ...
  },
  {
    ...
    category: 'tooling',
    question:
      'You install a CSS framework and add its stylesheet to the "styles" array in angular.json. What did that do, and why did the dev server need a restart?',
    options: [
  ...
  },
  {
    ...
    category: 'tooling',
    question:
      'A fresh workspace has tsconfig.json, tsconfig.app.json and tsconfig.spec.json. How do they relate?',
    options: [
    ...
    topicPath: 'cli-project-structure',
    explanation:
      'The "extends" chain keeps one source of truth for compiler strictness while letting each COMPILATION UNIT define its own file set: the app build must not compile test files (they would bloat and could break the build — and spec-only types like describe/it do not exist there), while the test build needs exactly those globals via its "types" entry. angular.json points each architect target at the right tsconfig — build → app, test → spec. Add a shared option in the base; add file-set concerns in the leaf. Why others fail: extends exists precisely to avoid manual sync. all three are active. environments are angular.json configurations, not tsconfigs.',
  },
  {
    id: 416,
    type: 'multiple-choice',
    ...
    category: 'tooling',
    question:
      'What does "strictTemplates": true under angularCompilerOptions add on top of TypeScript\'s "strict": true?',
    options: [
  ...
  },
  {
    ...
    category: 'tooling',
    question:
      'How does `ng build --configuration production` change what gets built, mechanically?',
    options: [
    ...
    topicPath: 'cli-project-structure',
    explanation:
      '"configurations" are named option-override sets on a target; -c production merges that set over the target\'s base options. Modern defaults: production turns on optimization (minify, tree-shake, CSS optimization), outputHashing: "all" (immutable-cacheable file names), extractLicenses and budget enforcement; development keeps fast rebuilds with sourcemaps. fileReplacements is the traditional environment mechanism (environment.ts → environment.prod.ts). serve borrows via "buildTarget". defaultConfiguration decides what plain `ng build` means — production in recent CLIs. Why others fail: it is declarative config selection, not env vars. (C)/one build, one framework.',
  },
  {
    id: 418,
    type: 'spot-the-bug',
    ...
    category: 'tooling',
    question:
      'CI builds fail with "Cannot find name describe" — locally everything compiles. The developer recently "cleaned up" a tsconfig. What was the likely change?',
    code: `// tsconfig.app.json (after the cleanup)
{
  ...
  // "exclude": ["src/**/*.spec.ts"]  <- removed as "redundant"
    ...
    topicPath: 'cli-project-structure',
    explanation:
      'The two leaf tsconfigs exist to keep two DIFFERENT file sets compiling with two different ambient type sets: the app compilation must never see *.spec.ts (their globals come from the test runner\'s type package, listed only in tsconfig.spec.json\'s "types"). With the exclude gone, spec files enter the app program — and every describe/it/expect is an unknown identifier there. It often "works locally" because the dev only ran ng serve with a warm cache or an editor using the spec config. Why others fail: it is a TypeScript program-membership issue, not Node. test globals are ambient, not framework imports. they combine fine — files pins entry points, extends inherits options.',
  },
  ...
  // --- TESTING (.spec.ts files) ---
  {
    ...
    category: 'testing',
    question:
      'ng generate component user-card created user-card.spec.ts alongside the .ts/.html/.css. What is that file?',
    options: [
    ...
    topicPath: 'testing-components',
    explanation:
      'The .spec.ts suffix is the test-file convention tsconfig.spec.json and the test runner are wired to pick up. Colocation is deliberate: the test sits next to the component (not in a parallel test/ tree), so renames, moves and reviews keep them together — the file structure itself nags you when a component has no spec. The generated seed compiles the component via TestBed and asserts it constructs; your real behavioral tests replace that placeholder. Why others fail: nothing CLI-generated is a backup. the type-checker uses types, not spec files. e2e tests live in a separate project/tooling; specs are fast, isolated unit tests.',
  },
  {
    id: 420,
    type: 'multiple-choice',
    ...
    topicPath: 'testing-components',
    explanation:
      'This trio is the near-universal test grammar (Jasmine, Jest and Vitest all share it): suites nest via describe for organization, it names ONE observable behavior ("clamps at zero on -"), and expect(...).matcher(...) performs the check — .toBe (Object.is), .toEqual (deep), .toContain, .toThrow. They are global functions provided by the runner, whose type declarations come in via tsconfig.spec.json\'s "types" — which is why they exist in specs and are compile errors in app code. Why others fail: they predate and sit outside Angular. they control test execution and reporting. TypeScript knows them only through those type declarations, not as language keywords.',
  },
  {
    id: 421,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'Why does the generated spec create the component inside beforeEach with TestBed.configureTestingModule?',
    options: [
...
console.log('2:', fixture.nativeElement.querySelector('h1').textContent);`,
  ...
  },
  {
    ...
    category: 'testing',
    question:
      'What does wrapping a test in fakeAsync(() => { … }) with tick() enable, versus using real async/await?',
    options: [
    ...
    topicPath: 'testing-components',
    explanation:
      'Time is the classic source of slow, flaky tests. Inside a fakeAsync zone, scheduled timers and microtasks land in a controlled queue; tick(300) says "pretend 300ms elapsed" and runs exactly what is due — the debounced search test asserts nothing fired at tick(299) and everything at tick(1) more, in microseconds of real time. flush() runs all pending timers; forgetting to drain them fails the test with "timers still in queue" — a feature, catching leaked intervals. Real async/await tests wait wall-clock time and can race. Why others fail: no workers involved. promises resolve via the controlled microtask queue, not synchronously-everywhere. HttpTestingController is synchronous already — fakeAsync is about TIME, not HTTP.',
  },
  {
    id: 424,
    type: 'multiple-choice',
    ...
    category: 'testing',
    question:
      'How does provideHttpClientTesting() + HttpTestingController let a service spec verify HTTP behavior without a network?',
    options: [
    ...
    topicPath: 'testing-services-http',
    explanation:
      'The providers pair (provideHttpClient(), provideHttpClientTesting()) replaces the real backend, so the service under test runs its genuine request-building code — URL, params, headers, interceptors — while nothing touches the network. The controller inverts control: the TEST decides when and how each request resolves, including error cases via req.flush(body, { status: 500, statusText: … }) that are nearly impossible to produce reliably against a live API. expectOne doubles as an assertion (two identical requests = failure = caught duplicate-call bug), and httpMock.verify() closes the loop on requests your code made that the test never expected. Why others fail: (A)/no server, no recording — pure in-memory doubles. requests are captured, not nulled.',
  },
];
