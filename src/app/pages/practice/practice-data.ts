...
fixture.detectChanges();`,
  ...
  },
  {
    ...
    answer: 2,
    explanation: 'ngOnChanges fires before ngOnInit on the first render (if the component has @Input properties), then fires again whenever any @Input reference changes. It receives a SimpleChanges object showing previous and current values. Note: it does NOT fire for signal inputs — those are reactive by default and do not use ngOnChanges. Why others fail: Fires on first render but also on subsequent @Input changes. Fires on @Input changes, not on every CD pass. Fires throughout the component lifecycle, not just at end.',
  },
...
const total = computed(() => {
  return price + tax;  // ???
    ...
    answer: 1,
    explanation: 'Signals are getter functions — to read their current value you must call them: price() and tax(). Writing price + tax tries to add the signal function objects together (resulting in NaN), not their values. The fix is: return price() + price() * tax() for the calculated total. Why others fail: const is the correct declaration for computed. Addition works on numbers regardless of integer or decimal type. computed() returns a Signal<T>, not an Observable.',
  },
  {
...
cartStore.items.set([]);   // can mutate store directly`,
  ...
  // --- RxJS ---
  {
    ...
    answer: 0,
    explanation: 'map() transforms each emitted value using a synchronous function (like Array.map for streams). switchMap() projects each value to a new inner Observable and automatically cancels/unsubscribes from the previous inner Observable, "switching" to the new one. This makes switchMap() essential for HTTP requests where you want to discard stale responses (e.g., typeahead search). Why others fail: switchMap() is not for filtering; use filter() for that. They are completely different operators with distinct use cases. Both work with Observables; switchMap() is actually designed for Observable-returning functions.',
...
export class DataComponent implements OnInit {
  ...
  map(n => n * 10)
    ...
    answer: 2,
    explanation: 'filter(n => n % 2 !== 0) keeps only odd numbers: 1 and 3. map(n => n * 10) multiplies each by 10: 10 and 30. So console.log is called twice with 10, then 30. Why others fail: Filter removes the even number (2). Filter removes 2 before map runs. Map is definitely applied to filtered values.',
  },
...
of(1, 2, 3).pipe(
  ...
  },
  {
    ...
    answer: 1,
    explanation: '`takeUntilDestroyed()` (Angular 16+) is the cleanest pattern for services and components. Inject `DestroyRef` (or use the overload `takeUntilDestroyed(inject(DestroyRef))`). The stream auto-completes when the context is destroyed — no `ngOnDestroy` boilerplate. A works but is verbose. C works for templates but not service-side subscriptions. D only works for single-emission Observables.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    explanation: "`router.navigate([\"/products\", 42], { queryParams: { sort: \"price\" }, fragment: \"reviews\" })` is the clean, structured way. Passing `router.navigate([\"/products/42?sort=price#reviews\"])` is wrong — cramming the query string and fragment into the path segment itself isn't valid `navigate()` syntax; those belong in the extras object. `router.navigateByUrl(\"/products/42\", { queryParams: { sort } })` is wrong too — `navigateByUrl` takes a complete URL string as its first argument, and query params can't be layered on top via an extras object the way they can with `navigate()`. There's also no `router.go()` method on Angular's `Router` — that's not part of its API.",
  },
  {
...
const p = config.port;`,
    ...
    answer: 1,
    topicPath: 'ts-utility-types',
    explanation: '`satisfies` checks that the value is assignable to `Record<string, string | number>` but keeps the NARROW inferred type of the literal. So `config.port` stays `number` (and `config.host` stays `string`), giving you both validation and precise types. Contrast with a type ANNOTATION `const config: Record<string, string | number> = {...}`, which widens `config.port` to `string | number`. Why others fail: that is what an annotation would do, not satisfies. satisfies never produces `any`. object property inference widens `3000` to `number` (only `as const` keeps the literal `3000`).',
  },
  {
...
await button.click();`,
    ...
    answer: 1,
    topicPath: 'testing-components',
    explanation: 'A harness wraps a component behind an intent-based API. Your test says `button.click()` / `await input.setValue(\'x\')` instead of querying `.mat-button` and dispatching events. When the internal DOM structure or CSS classes change, only the harness implementation updates — the tests stay green because they assert behavior, not markup. Angular Material ships harnesses for every component; you can build your own by extending `ComponentHarness`. Why others fail: speed is comparable. harnesses work for any component, not just forms. harness methods trigger change detection internally but the concept is about stable, semantic queries.',
  },
  {
    ...
    answer: 1,
    topicPath: 'testing-services-http',
    explanation: '`TestBed.inject` goes through the configured injector, so the service is constructed with its real (or overridden/mocked) dependencies and you get the SAME singleton the components in that TestBed use. `new UserService(...)` forces you to manually build every constructor dependency, ignores any `{ provide, useValue }` mocks you set up, and produces an instance disconnected from the component tree. Why others fail: `new` is legal, just usually wrong here. inject returns the DI singleton, not a fresh instance each call. they differ in dependency wiring and override behavior.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'a11y',
    explanation: 'An accessible modal takes focus on open, confines Tab/Shift+Tab to elements inside it so keyboard and screen-reader users cannot wander into the inert background, and returns focus to the element that opened it when it closes. Semantically it needs `role="dialog"` with `aria-modal="true"` and a label (`aria-labelledby`/`aria-label`). Angular CDK\'s `a11y` module (`cdkTrapFocus` / `FocusTrap`, `FocusMonitor`) implements this. Why others fail: a backdrop is visual only, not a focus boundary. disabling the keyboard breaks all keyboard users. a plain `<div>` gets none of this for free.',
  },
  {
...
export class NameField {
  name = '';
    ...
    explanation: '`ngModel` is provided by `FormsModule`. In a standalone component you must add it to `imports: [FormsModule]`; otherwise Angular does not recognize the `ngModel` directive and reports "Can\'t bind to \'ngModel\' since it isn\'t a known property of \'input\'". Why others fail: template-driven `ngModel` is fully supported, just not imported here. `ngModel` works with a plain string property. `[(ngModel)]` is valid banana-in-a-box sugar; splitting it does not fix the missing import.',
  },
  {
...
export class Profile {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'state-management',
    explanation: '`createSelector(inputA, inputB, (a, b) => derive(a, b))` caches its last inputs and output. If the input selectors return the same references, it skips the projector and hands back the memoized value — so expensive derivations (filtering, sorting, joining slices) run only when their inputs actually change, and components bound to the selector do not re-render needlessly. Why others fail: it runs on the main thread. selectors CAN be plain functions; createSelector adds memoization. it does not persist anything.',
  },
  {
...
b$.next('b2');`,
  ...
  distinctUntilChanged()
    ...
    answer: 1,
    topicPath: 'rxjs-operators',
    explanation: '`distinctUntilChanged` compares each value to the PREVIOUS emitted one and drops it only if they are equal. So the runs `1,1` → `1`, `2,2` → `2`, then `3`, then a `1` again — which is NOT equal to the immediately preceding `3`, so it passes. Result: `1, 2, 3, 1`. To remove ALL duplicates ever seen (globally), you would need `distinct()` instead. Why others fail: that is `distinct()`, not distinctUntilChanged. consecutive dupes ARE removed. it filters, it does not stop the stream.',
  },
  {
...
// ...synchronous code finishes, then the scheduler flushes`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'animations',
    explanation: 'When an element is added to or removed from the DOM (via `@if`, `@for`, `*ngIf`, etc.), Angular represents those with the special `void` state. `:enter` is shorthand for `transition(\':enter\', ...)` = `void => *` (appearing), and `:leave` = `* => void` (disappearing). Angular keeps a leaving element in the DOM until its `:leave` animation finishes. Wrapping a list in a trigger and using `query(\':enter\', stagger(50, animate(...)))` animates children in sequence. Why others fail: those are `mouseenter`/`mouseleave` DOM events, unrelated. enter/leave apply to any element insertion/removal, not just routes. focus/blur are different events.',
  },
  {
...
// used as: <div *appDelay="delay">Hi</div>`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'structural-directives',
    explanation: 'Each `*` structural directive claims the element by lifting it into its own `<ng-template>`; two on the same element would need competing templates, so the compiler forbids it. The classic pre-v17 fix is `<ng-container *ngIf="show"><li *ngFor="let x of items">…</li></ng-container>` (a grouping element that renders no extra DOM). Modern control flow sidesteps it entirely because `@if`/`@for` are block syntax that nests: `@if (show) { @for (x of items; track x.id) { <li>…</li> } }`. Why others fail: ordering is not the issue. both still exist, though `@if`/`@for` are now preferred. importing does not lift the one-structural-directive rule.',
  },
  {
...
export class WidthComponent {
  ...
  error: () => this.showError(),   // never called
    ...
    answer: 1,
    topicPath: 'rxjs-advanced',
    explanation: '`catchError` REPLACES the errored stream with whatever observable you return. Returning `EMPTY` yields a stream that emits nothing and completes normally, so downstream the error is gone — `next` and `error` both stay silent and the observable just completes. If the UI must react to failure, either handle it inside `catchError` (call `this.showError()` there and return a fallback value) or re-throw with `return throwError(() => err)` so the subscriber error callback still runs. Why others fail: `EMPTY` completes, it does not throw. the object-form observer is valid. HTTP errors are ordinary RxJS errors and are catchable.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'ts-narrowing',
    explanation: 'A discriminated union pairs each member with a common singleton-typed field (the discriminant, here `kind`). When you branch on that field, TypeScript narrows to the matching member — inside `case \'circle\'` only `r` is accessible, inside `case \'square\'` only `side`. Assigning the value in the `default` branch to `const _exhaustive: never = shape` turns a forgotten case into a compile error, so adding a new shape forces you to handle it. Why others fail: the members stay separate, narrowed by the tag. plain object types suffice — no classes needed. it strengthens checking, not disables it.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'a11y',
    explanation: 'A visible, associated `<label for="email">` (matching the input `id`) or a wrapping label gives the field a persistent, accessible name: it stays visible while typing, screen readers announce it, and clicking it focuses the input (a bigger touch target). Placeholders disappear on input, are usually low-contrast grey, and are not reliably treated as the accessible name. Use placeholder only for a format EXAMPLE (a name@example.com style hint), never as the sole label. Why others fail: placeholder is not deprecated — just misused. all form controls need labels. capitalization changes nothing about the problems.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'a11y',
    explanation: 'Headings form the document outline that screen-reader users jump through (a heading list is one of the most-used navigation methods). The hierarchy must be semantic: a single `h1` describing the page, `h2` sections, `h3` inside those — never jumping h1 → h4 because it "looks right". If a level looks too big, restyle it with CSS; the tag encodes STRUCTURE, the stylesheet encodes appearance. Why others fail: choosing tags by font size breaks the outline. multiple h1s dilute structure and modern SEO does not reward it. a styled div is invisible to heading navigation.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'security',
    explanation: 'They operate at different layers. Angular sanitizes interpolated/bound values so script tags or javascript: payloads are neutralized in templates. CSP is an HTTP response header (`Content-Security-Policy: script-src \'self\'; …`) the BROWSER enforces: forbidding inline script execution, eval, and loading from unlisted origins. If an XSS slips through anyway, a strict CSP usually prevents the payload from running or phoning home. Angular supports strict CSP via nonces (`ngCspNonce` / the `CSP_NONCE` token) for its inline styles. Why others fail: both stay active — layers, not alternatives. CSP governs scripts, styles, frames, connects, and more. it is a server/header concern, not a bundle setting.',
  },
  {
...
onLoginSuccess() {
  ...
  window.location.href = returnUrl;   // redirect wherever it says
    ...
    answer: 1,
    topicPath: 'security',
    explanation: 'An open redirect lets an attacker craft a link on YOUR trusted domain that bounces victims to theirs — ideal for phishing ("please log in again") right after a real login. Because the value comes from the URL, treat it as hostile: accept only relative internal paths (starts with `/`, reject `//` and protocol-relative tricks, or match against a route allowlist) and use `router.navigateByUrl(returnUrl)`, which stays inside the SPA rather than performing a raw browser navigation to an arbitrary absolute URL. Why others fail: queryParamMap reads it fine — that is the problem. performance is irrelevant. ?? correctly defaults on null.',
  },
  {
    ...
    answer: 1,
    topicPath: 'security',
    explanation: 'Clickjacking overlays your real UI (in an invisible iframe) under fake controls, tricking users into clicking privileged buttons. The defense is response headers the BROWSER enforces before your code runs: `Content-Security-Policy: frame-ancestors \'none\'` (modern, supersedes) or `X-Frame-Options: DENY/SAMEORIGIN` (legacy). Configure them on the web server/CDN serving index.html. JavaScript frame-busting is bypassable and not recommended alone. Why others fail: no such Angular API — by the time Angular runs, the frame already loaded. obfuscation does not affect framing. HTTPS secures transport, not embedding.',
  },
  {
...
<!-- goal: "She replied" / "He replied" / "They replied" -->`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'form-validation',
    explanation: 'The second FormControl argument accepts a single validator or an array; all run on every value change and their errors MERGE into one object — e.g. an empty field gives `{ required: true }`, a short password `{ minlength: { requiredLength: 8, actualLength: 3 } }`. Note some built-ins are factories you CALL (`Validators.minLength(8)`, `Validators.pattern(/…/)`) while others are used bare (`Validators.required`, `Validators.email`) — passing `Validators.minLength` uncalled is a classic bug. Why others fail: arrays of validators are standard. validators do not chain fluently. attributes only feed template-driven forms; reactive validators live in the class.',
  },
  {
    ...
    answer: 1,
    topicPath: 'reactive-forms',
    explanation: 'Native forms submit on submit-button click OR Enter in an input — `(ngSubmit)` hooks that unified event, so keyboard users are covered for free, and Angular\'s form directives suppress the browser\'s default page-reloading POST. A `(click)` on the button captures only mouse/keyboard activation of that one button and leaves Enter-in-field either dead or triggering an unhandled native submit. Still guard inside: `if (this.form.invalid) { this.form.markAllAsTouched(); return; }` — nothing blocks invalid submission automatically. Why others fail: it is best practice, not enforced. click handlers work, they are just incomplete. validation gating is your code\'s job.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'reactive-forms',
    explanation: 'Each operator solves a real defect: `debounceTime(300)` stops a request per keystroke; `distinctUntilChanged()` skips re-querying when the settled value is the same (e.g. type + delete a char); `switchMap` maps to the HTTP call AND cancels the previous in-flight request, killing the out-of-order-response bug where slow results for "ang" overwrite fresh results for "angular". Render with the async pipe or `toSignal`. Why others fail: raw keyup floods the API and races responses. polling wastes requests and is not type-ahead. `mergeMap` keeps stale requests alive — their late responses can clobber newer ones.',
  },
  {
...
// usage: new FormControl('', [Validators.required, noSpaces])`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'routing-basics',
    explanation: 'The router walks the config in array order and takes the first route whose path matches — there is no specificity scoring. Practical consequences: `products/new` must precede `products/:id` (otherwise "new" is captured as an id), and `{ path: \'**\' }` is the universal fallback that must sit at the very END; anything after it is unreachable. The same logic applies within each children array. Why others fail: no automatic specificity — that is CSS thinking. nothing is alphabetical. a leading wildcard would match EVERY url and hide your entire app.',
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'routing-basics',
    explanation: '`router.navigate([\'products\', id], { relativeTo: this.route, queryParams: {...} })` builds a UrlTree from parts and supports RELATIVE navigation — its power tool. `router.navigateByUrl(\'/products/42?tab=specs\')` treats the string as the complete new url (always absolute; extras like queryParams are ignored because they belong in the string). Both return `Promise<boolean>` and both run guards/resolvers. The login `returnUrl` pattern is the classic navigateByUrl use. Why others fail: both are current, different-shaped APIs. both are async promises. nothing bypasses guards — they run for every router-initiated navigation.',
  },
  {
...
<a [href]="profile.website">My site</a>`,
    ...
    answer: 1,
    topicPath: 'security',
    explanation: 'This demonstrates CONTEXT-AWARE sanitization. Angular\'s template compiler knows [href] is a URL security context, so bound values pass through URL sanitization at render: safe schemes (https, http, mailto, tel…) pass untouched; javascript: and data:text/html get the `unsafe:` prefix, defusing them while leaving forensic evidence in the DOM. Each context has its own rules — [innerHTML] gets HTML sanitization, [style.xyz] historically style checks, and RESOURCE URL contexts like <iframe src> accept NO sanitization at all (there is no safe way to sanitize an arbitrary document to embed), which is why iframes demand either a static string or an explicit bypassSecurityTrustResourceUrl. The correct app-level fix for user "website" fields is validating/normalizing to https?:// at input time, not trusting the defusal. Why others fail: attribute/property bindings in security contexts ARE sanitized — that is the core mechanism. the attribute survives with the prefixed value, not removed. sanitization degrades gracefully; no error is thrown for plain URL contexts.',
  },
  {
...
export class TodoStore {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'state-management',
    explanation: 'This is an engineering-judgment question, and the honest answer names the FORCES rather than a team allegiance. Event-sourced stores (NgRx Store) shine when you need to answer "what sequence of events produced this state?" — devtools time-travel, action logs for debugging production incidents, strict unidirectionality that survives 30 contributors. Those benefits are real and so is their price: every trivial write becomes action + reducer case + possibly effect + selector, files multiply, and newcomers learn a meta-framework. The signal-service pattern covers a surprising share of real apps; NgRx SignalStore is the middle path (structured, opinionated, signal-native, less boilerplate). Choosing SMALLEST-tool-that-solves-the-problem is the senior signal here. Why others fail: cargo-culting infrastructure ahead of need is how simple apps rot. SSR works fine with plain services (TransferState is orthogonal). signals changed the REACTIVITY substrate; coordination, auditability and convention problems still exist at scale — SignalStore existing is NgRx agreeing.',
  },
  {
...
<p>{{ user!.email }}</p>  <!-- non-null assertion -->`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'let-block',
    explanation: '@let gives a template-local name to any expression: subsequent bindings read `total` instead of re-writing the expression, and it re-evaluates when its dependencies change — it is a live alias, not a snapshot. It especially shines for `@let user = user$ | async` (one subscription, one name, no *ngIf-as-a-variable hack) and for narrowing long optional chains. Scope is lexical: available after its declaration within the same block. Why others fail: it never touches the class. it stays reactive across change detection. it declares a value and renders nothing conditionally — that is @if\'s job.',
  },
  {
  ...
  // --- STYLING (CSS in Angular) ---
  {
    ...
    answer: 1,
    topicPath: 'components',
    explanation: 'With ViewEncapsulation.Emulated (the default) the compiler stamps the component\'s DOM with a generated attribute and rewrites `p { … }` into `p[_ngcontent-xxx] { … }`. The effect: styles cannot leak OUT of the component (and outside page styles that do not use such attributes still cascade IN unless more specific). This is why each component can use generic selectors fearlessly. Why others fail: tags are untouched; attributes are added. ngStyle is an unrelated binding API. component styles are fully real CSS, just scoped.',
  },
  {
...
app-fancy-list .item-title {
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'cli-project-structure',
    explanation: '"configurations" are named option-override sets on a target; -c production merges that set over the target\'s base options. Modern defaults: production turns on optimization (minify, tree-shake, CSS optimization), outputHashing: "all" (immutable-cacheable file names), extractLicenses and budget enforcement; development keeps fast rebuilds with sourcemaps. fileReplacements is the traditional environment mechanism (environment.ts → environment.prod.ts). serve borrows via "buildTarget". defaultConfiguration decides what plain `ng build` means — production in recent CLIs. Why others fail: it is declarative config selection, not env vars. (C)/one build, one framework.',
  },
  {
...
console.log('2:', fixture.nativeElement.querySelector('h1').textContent);`,
  ...
  },
  {
    ...
    answer: 1,
    topicPath: 'testing-services-http',
    explanation: 'The providers pair (provideHttpClient(), provideHttpClientTesting()) replaces the real backend, so the service under test runs its genuine request-building code — URL, params, headers, interceptors — while nothing touches the network. The controller inverts control: the TEST decides when and how each request resolves, including error cases via req.flush(body, { status: 500, statusText: … }) that are nearly impossible to produce reliably against a live API. expectOne doubles as an assertion (two identical requests = failure = caught duplicate-call bug), and httpMock.verify() closes the loop on requests your code made that the test never expected. Why others fail: (A)/no server, no recording — pure in-memory doubles. requests are captured, not nulled.',
  },
];
