...
const EXAM: ExamLevel[] = [
  {
    ...
    blurb:
      'Core concepts and basic implementations — the language, templates, binding and the building blocks.',
    topics: [
      {
      ...
      {
        topic: 'Angular basics & the CLI',
        lessonIds: ['what-is-angular', 'cli-project-structure'],
      },
      { topic: 'Components & templates', lessonIds: ['components', 'interpolation', 'lifecycle'] },
      {
        topic: 'Data binding',
        lessonIds: ['property-binding', 'event-binding', 'two-way-binding', 'class-style-binding'],
      },
      {
        topic: 'Directives & control flow',
        lessonIds: [
          'control-flow-if',
          'control-flow-for',
          'control-flow-switch',
  ...
  },
  {
    ...
    blurb:
      'Advanced concepts and real-world applications — RxJS, advanced forms & routing, custom directives, DI, testing and change detection.',
    topics: [
      {
      ...
      {
        topic: 'Reactive forms & validation',
        lessonIds: ['reactive-forms', 'form-validation', 'async-validators', 'form-arrays'],
      },
      {
        topic: 'Advanced routing',
        lessonIds: ['router-children-lazy', 'route-guards', 'resolvers', 'route-params'],
      },
      { topic: 'HTTP in depth', lessonIds: ['http-crud', 'http-interceptors'] },
      {
        topic: 'RxJS',
        lessonIds: ['rxjs-observables', 'rxjs-operators', 'rxjs-subjects', 'rxjs-interop'],
      },
      {
        topic: 'Custom directives & pipes',
        lessonIds: ['custom-pipes', 'attribute-directives', 'structural-directives'],
      },
      {
        topic: 'Advanced components',
        lessonIds: ['content-projection', 'view-queries', 'ng-template-outlet'],
      },
  ...
  },
  {
    ...
    format: '135 min · 40 questions + 105 min coding tasks',
    blurb:
      'Architecture and large-scale applications — performance, rendering, security, advanced DI/RxJS and reusable libraries.',
    topics: [
      ...
      { topic: 'Advanced type system', lessonIds: ['ts-mapped-conditional'] },
      {
        ...
        lessonIds: ['performance', 'deferrable-views', 'after-render', 'zoneless'],
      },
      ...
      { topic: 'Custom form controls (CVA)', lessonIds: ['control-value-accessor'] },
      {
        ...
        lessonIds: ['security', 'i18n', 'a11y', 'animations', 'view-transitions'],
      },
...
const EXAM_QS: ExamQ[] = [
  // ──── JUNIOR ────
  {
    ...
    answer: 1,
    explanation:
      'standalone: true means the component owns its dependency declarations. It imports directives, pipes, and other components directly in its imports: [] array — no NgModule needed. This is the default in Angular 17+.',
  },
  {
    level: 'junior',
    ...
    answer: 2,
    explanation:
      'update(fn) reads the current value, passes it to your function, and sets the result. set(v) replaces with a literal value. mutate() and next() do not exist on writable signals.',
  },
  {
    level: 'junior',
    ...
    answer: 1,
    explanation:
      'Square brackets [] create a property binding that evaluates the expression. Without brackets, "pageTitle" is treated as a string literal, not a variable. Interpolation inside an attribute is only valid for native HTML attributes, not component inputs.',
  },
  {
    level: 'junior',
    ...
    answer: 3,
    explanation:
      'Both @Output() with EventEmitter (classic) and output() (modern signal API, Angular 17+) produce an output named "clicked". The parent binds to it with (clicked)="save()" in either case.',
  },
  {
    level: 'junior',
    ...
    answer: 2,
    explanation:
      'href="/shop" tells the browser to fetch a new page from the server — Angular is destroyed and restarted. routerLink="/shop" uses Angular Router to swap components in-place, preserving all service singletons and avoiding the network round-trip.',
  },
  {
    level: 'junior',
    ...
    answer: 1,
    explanation:
      '@if is the built-in control flow syntax introduced in Angular 17. It requires no import, supports @else if / @else chaining, and provides better TypeScript type narrowing. *ngIf still works but @if is the modern standard.',
  },
  {
    level: 'junior',
    q: "Which lifecycle hook runs exactly once, right after Angular first sets the component's inputs?",
    opts: ['ngAfterViewInit', 'ngOnChanges', 'ngOnInit', 'constructor'],
    answer: 2,
    explanation:
      'ngOnInit runs once after the first ngOnChanges call and before the first render. It is the right place to kick off data loading that depends on @Input() values. The constructor runs before inputs are set.',
  },
  {
    level: 'junior',
    q: 'What is the correct two-way binding syntax with ngModel?',
    opts: ['[ngModel]="name"', '(ngModel)="name"', '[(ngModel)]="name"', '{{ngModel}}="name"'],
    answer: 2,
    explanation:
      '[(ngModel)] is the "banana-in-a-box" syntax — it combines property binding [ngModel] (parent → input) and event binding (ngModelChange) (input → parent) into one shorthand. FormsModule must be imported for ngModel to work.',
  },
  {
    level: 'junior',
    ...
    answer: 1,
    explanation:
      'providedIn: "root" registers the service with Angular\'s root injector. One instance is created and reused everywhere the service is injected. This is the default and correct choice for app-wide services like AuthService or ApiService.',
  },
  {
    level: 'junior',
    ...
    answer: 1,
    explanation:
      'The built-in DatePipe accepts format strings. "MMMM d, y" produces "January 15, 2025". Pipe syntax is value | pipeName:"arg". DatePipe is available automatically in standalone components that import CommonModule or just use it directly.',
  },
  ...
  // ──── MID ────
  {
    ...
    answer: 1,
    explanation:
      'switchMap is "cancel previous" — ideal for typeahead search where stale HTTP requests should be abandoned. mergeMap is "run all in parallel" — ideal for independent parallel operations. concatMap queues; exhaustMap ignores new while one is in flight.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      'OnPush skips the component unless: (1) an @Input() reference changes, (2) a signal the template reads emits a new value, (3) an event handler runs inside the component, or (4) markForCheck()/detectChanges() is called manually. This drastically reduces unnecessary DOM diffing.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      'setValue() is strict: it throws "Must supply a value for form control with name: X" if any control is omitted. patchValue() is lenient — it only updates the keys you provide and ignores missing ones. Use setValue() for full model replacement; patchValue() for partial updates.',
  },
  {
    level: 'mid',
    ...
    answer: 2,
    explanation:
      'HttpRequest is immutable. req.clone() creates a new request with the specified overrides. headers.set() returns a new HttpHeaders object (also immutable). The original request is never modified — this is a requirement because interceptors can run in parallel.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      'CanMatchFn returning false makes the router treat this route as non-existent and fall through to the next route definition for the same URL — enabling role-based routing. CanActivateFn returning false blocks the navigation entirely, with no fallback to other routes.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      "computed() is for pure derivation: lazy, memoized, synchronous, returns a Signal<T>. effect() is for side effects (logging, localStorage writes, DOM updates) — it runs after render when dependencies change. Never use effect() to synchronize one signal's value to another; use computed() or linkedSignal() instead.",
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      '@defer separates "when to prefetch" from "when to render". Prefetch on idle downloads the chunk in the background before the user needs it. On interaction defers rendering until a click or focus event. The result: near-instant render on first interaction with no download latency.',
  },
  {
    level: 'mid',
    ...
    answer: 2,
    explanation:
      'Component providers create a new service instance for each component instance (and its view children). When the component is destroyed, so is its scoped service instance. This is ideal for features that need isolated state — for example, an edit form with its own unsaved-changes service.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      'fakeAsync() patches timer APIs (setTimeout, setInterval) and Promises to run under a virtual clock. tick(500) advances time 500ms instantly; flushMicrotasks() drains all pending Promise callbacks. This makes debounce, retry delays, and async validation fully deterministic in tests.',
  },
  {
    level: 'mid',
    ...
    answer: 1,
    explanation:
      'Angular sets FormControl.status = "PENDING" while any async validator Observable or Promise is in flight. Check this in templates to show a spinner: @if (ctrl.status === "PENDING") { <span>Checking...</span> }. When the Observable completes, the status resolves to "VALID" or "INVALID".',
  },
  ...
  },
  {
    ...
    answer: 2,
    explanation:
      'A → B → A is a construction deadlock. The cleanest fix is extracting the shared logic into SharedStateService that both A and B depend on. Alternatively, defer one dependency: inject ServiceB lazily inside a method body (not the constructor) using inject(), which resolves past construction time.',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'hostDirectives: [CdkDrag, FocusTrapDirective] in @Component attaches these directive behaviors to the component without any class-level coupling. Inputs and outputs can be selectively forwarded. This replaces inheritance and mixin patterns for composable behaviors in design systems.',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'http.get() is cold — each subscribe makes a new request. pipe(shareReplay(1)) converts it to a hot multicasting Observable with a buffer of 1. All current and future subscribers receive the same response without triggering duplicate network requests. Essential for shared data streams (user profile, config).',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'Signals live in a single JavaScript context per tab. BroadcastChannel.postMessage("logout") instantly notifies all same-origin tabs. Each tab\'s service listens with channel.onmessage and clears its local auth state. Without this, logout in Tab A is invisible to Tab B until it makes an authenticated request and gets a 401.',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'Zone.js works by monkey-patching async APIs and triggering change detection globally. Without it, Angular cannot know when async operations complete. Every state change must be tracked by signals, async pipe, or resource() so Angular knows exactly which components to re-render and when.',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'Classes are their own DI tokens. For non-class values (API base URL string, feature-flag object, factory function), you create a typed token: const API_URL = new InjectionToken<string>("API_URL"). Provide it: { provide: API_URL, useValue: "https://api.example.com" }. Inject it: inject(API_URL).',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'CVA is Angular\'s protocol for custom form controls. Implementing it (writeValue, registerOnChange, registerOnTouched, setDisabledState) lets the control work with [formControl]="ctrl", formControlName="x", and [(ngModel)] — exactly as a native <input> would. Without CVA, your component is invisible to Angular\'s form model.',
  },
  {
    level: 'senior',
    ...
    answer: 1,
    explanation:
      'Any JavaScript running on your page — including injected scripts from XSS — can read localStorage. An attacker who injects JS steals the token and impersonates the user. HttpOnly cookies are set by the server and cannot be read by JavaScript at all. Access tokens should be in-memory (lost on refresh, XSS-safe) with an HttpOnly refresh token cookie for silent re-auth.',
  },
  {
    level: 'senior',
  ...
  // ──── JUNIOR (continued — 20 additional) ────
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `track item.id` (or `track $index`) gives Angular a stable key per item. When the array is updated, Angular can reuse existing DOM nodes for matching keys rather than recreating them — a critical performance optimisation for large lists. A is wrong: filtering is done by the array itself or a computed value, not `track`. C is wrong: `track` does not sort. D is wrong: it does not limit count.',
  },
  {
    level: 'junior',
  ...
  },
  {
    ...
    answer: 2,
    explanation:
      'C is correct: `[class.className]="expression"` is Angular\'s class binding — the class is added if the expression is truthy and removed if falsy. A is wrong: without `[]` it would be a static string, but `[class.active]` is conditional. B is wrong: it only toggles the one named class. D is wrong: CSS custom property bindings use `[style.--var]` syntax, not `[class]`.',
  },
  {
    level: 'junior',
  ...
  },
  {
    ...
    answer: 2,
    explanation:
      'C is correct: `@Input({ required: true })` causes Angular to throw a runtime error if the binding is missing at component initialisation. This is checked at runtime (not always at compile time) and is enforced even in non-strict builds. A is wrong: the error is thrown, not silently ignored. B is wrong: it is a runtime, not compile-time, check (though templates with static analysis may catch it earlier). D is wrong: it throws, not warns.',
  },
  {
    level: 'junior',
    ...
    answer: 1,
    explanation:
      'B is correct: `async` pipe handles the full subscription lifecycle — subscribe on init, update the DOM on each emission, and unsubscribe on destroy. This eliminates memory leak risk and manual subscription boilerplate. A is wrong: the Observable is not converted to a Promise. C is wrong: there is no count threshold. D is wrong: it executes in the same thread.',
  },
  {
    level: 'junior',
    ...
    answer: 2,
    explanation:
      'C is correct: `HttpClient` methods return cold Observables. No HTTP request is made until something subscribes — either manually with `.subscribe()`, with `async` pipe in the template, or via `toSignal()`. A is wrong: it is an Observable, not a Promise (though you can convert with `firstValueFrom()`). B is wrong: HTTP is asynchronous by nature. D is wrong: it is a regular Observable, not a Subject.',
  },
  {
    level: 'junior',
    q: 'You have `count = signal(0)`. What is the WRONG way to read the value in a template?',
    opts: ['{{ count() }}', '[value]="count()"', '{{ count }}', '@if (count() > 0) { ... }'],
    answer: 2,
    explanation:
      'C is wrong (and therefore the correct answer): `{{ count }}` prints the string representation of the signal object itself (a function), not its value. You must call the signal as a function: `{{ count() }}`. A, B, and D are all correct ways to read a signal — they call it as a function.',
  },
  {
    level: 'junior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      "B is correct: `@ContentChild` accesses elements that a parent inserts into the component via `<ng-content>`, available in `ngAfterContentInit`. `@ViewChild` accesses elements defined in the component's own template, available in `ngAfterViewInit`. A is wrong: it reverses the two. C is wrong: they have distinct scopes. D is wrong: both can query directives, components, and template references.",
  },
  {
    level: 'junior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `?.` is optional chaining — it short-circuits to `undefined` if any part of the chain is nullish (null or undefined) instead of throwing "Cannot read properties of null". A is wrong: it is an expression operator, not a type cast. C is wrong: optional chaining is for reading, not writing. D is wrong: ternary is `condition ? a : b` — optional chaining is distinct.',
  },
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `BehaviorSubject(initialValue)` always has a current value (`getValue()`) and immediately emits it to any new subscriber. `ReplaySubject(n)` buffers the last n emissions and replays them to late subscribers — no mandatory initial value. A is wrong: BehaviorSubject only replays 1 (the current value), not all history. C is wrong: they are not interchangeable. D is wrong: both work in any context.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `distinctUntilChanged()` compares each emission to the previous one using strict equality (or a custom comparator). In a typeahead, if the user types "ang" → "angu" → "ang" (backspace), without it you\'d fire three searches; with it, only "ang" and "angu" trigger requests (the second "ang" is suppressed). A is wrong: it compares with the previous, not the first. C is wrong: use `filter(v => v != null)` for that. D is wrong: that is `last()`.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 0,
    explanation:
      'A is correct: `withComponentInputBinding()` (Angular 16+) makes the router automatically pass route params, query params, and resolve data as component inputs matching the same name. For example, `{ path: "product/:id" }` maps to `readonly id = input<string>()`. B, C, and D do not describe real Angular features.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 3,
    explanation:
      'D is correct: `input<string>()` creates an optional signal input (value is `string | undefined` unless a default is given), while `input.required<string>()` creates a required signal input that throws if the parent does not bind it. Both replace `@Input()`. A is wrong: `signal()` creates internal state, not a component input.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `<ng-container>` is invisible in the final DOM — Angular renders its children in-place. This is essential when you need to apply structural directives (`@if`, `@for`, `*ngIf`) to a group of elements without wrapping them in a `<div>` that would break CSS layout (flex/grid rows, table rows, etc.). A, C, D do not describe `ng-container`.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `debounceTime(300)` suppresses emissions until 300ms of silence — if a new value arrives within 300ms of the previous, the timer resets. This prevents an HTTP request on every keystroke for a user typing quickly. A, C, D all misrepresent the operator.',
  },
  {
    level: 'mid',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `concatMap` queues inner Observables and starts the next only when the previous completes — perfect for sequential file uploads or ordered API calls. A is wrong: `mergeMap` runs all concurrently, losing order guarantees. C is wrong: `switchMap` cancels the previous, so you would lose all but the last operation. D is wrong: `exhaustMap` ignores new values while one is in flight — useful for preventing double-submit, not for sequential processing of all values.',
  },
  ...
  // ──── SENIOR (continued — 20 additional) ────
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `withHttpTransferCache()` serialises HTTP responses made during SSR into the `<script type="application/json">` block in the HTML. On the client, `HttpClient` reads from this transfer state cache first, serving the responses instantly without network calls. Once the cache is consumed, subsequent requests go to the network normally. A, C, D misrepresent what this API does.',
  },
  {
    level: 'senior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: the browser\'s `document` global throws on the server (Node.js) during SSR. `inject(DOCUMENT)` is platform-agnostic — Angular substitutes a DOM-compatible implementation server-side. This is part of the "platform abstraction" pattern for writing components that work in both environments. A, C, D are incorrect descriptions of `DOCUMENT`.',
  },
  {
    level: 'senior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `inject()` must be called during component/directive/service construction (the injection context). For asynchronous callbacks, store the injected service in a class field or use `runInInjectionContext(this.injector, () => inject(SomeService))` to temporarily enter an injection context. A is wrong: `inject()` outside context throws "inject() must be called from an injection context". C is wrong: `@Inject` is a parameter decorator for constructors, not standalone functions. D is too absolute — `runInInjectionContext` is the solution.',
  },
  {
    level: 'senior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `readonly value = model<string>("")` creates a writable signal that acts as both an input and an output. The parent binds with `[(value)]="name"` (two-way). Inside the component, `this.value()` reads it and `this.value.set(x)` writes it — automatically emitting to the parent. A, C, D are all incorrect descriptions.',
  },
  {
    level: 'senior',
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'B is correct: `LOCALE_ID` (default "en-US") determines how locale-aware pipes format their output. Provide a different locale: `{ provide: LOCALE_ID, useValue: "de-DE" }` and `{{ 1234.5 | number }}` outputs "1.234,5" instead of "1,234.5". You must also call `registerLocaleData(localeDe)` for Angular to have the locale data. A, C, D misstate its role.',
  },
];
...
export class Certification implements OnDestroy {
      ...
      const topics: ResolvedTopic[] = lvl.topics.map((t) => ({
        ...
        lessons: t.lessonIds
          .map((id) => {
            ...
            return lesson
              ? { id: lesson.id, title: lesson.title, built: !!lesson.loadComponent }
              : null;
          })
  ...
  protected readonly passThreshold = computed(() => Math.ceil(this.examQs().length * 0.7));
